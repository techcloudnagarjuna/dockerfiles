 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojthemeutils', 'ojs/ojlogger'], function (exports, oj, $, DomUtils, ThemeUtils, Logger) {
  'use strict';
  ...
  oj._registerLegacyNamespaceProp('AnimationUtils', AnimationUtils); // this is the variable name that the AMD module will return in the require callback (used in a no-require environment)
  ...
  AnimationUtils._animate = function (element, fromState, toState, options, transProps, persistProps) {
    ...
    var doAnimate = function doAnimate(resolve, reject) {
      ...
      toState.css.transition = AnimationUtils._createTransitionValue(element, transProps, options); // Save the orignal style so that we can restore it later if needed

      var effectCount = AnimationUtils._saveStyle(element, fromState, toState, options, persistProps || transProps);
  ...
  AnimationUtils._saveStyle = function (element, fromState, toState, options, persistProps) {
    ...
    AnimationUtils._saveCssValues(element, toStateCss, savedStyle, _persistProps); // eslint-disable-next-line no-param-reassign
    ...
    if (state) {
      if (state.css) {
        ...
        if (state.css[transformPropName]) {
          // eslint-disable-next-line no-param-reassign
          state.css[transformPropName] = AnimationUtils._applyTransform(element, state.css[transformPropName]);
        }
  ...
  AnimationUtils.startAnimation = function (element, action, effects, component) {
    ...
    function (resolve, reject) {
      ...
      AnimationUtils._saveStyle(element, null, null, null, null); // Trigger ojanimatestart event so that app can prevent default animation
      ...
      var defaultPrevented = AnimationUtils._triggerEvent(element, 'animateStart', ui, component); // Continue animation handling if app didn't preventDefault
      ...
      if (!defaultPrevented) {
        ...
        for (var i = 0; i < effectArray.length; i++) {
          ...
          if (animationEffect != null && animationEffect !== 'none') {
            ...
            AnimationUtils._fillEmptyOptions(effectOptions, lastOptions); // Remember the last set of options
  ...
  AnimationUtils._mergeOptions = function (effect, options) {
    ...
    return $.extend({
      duration: '400ms'
    }, AnimationUtils._defaultOptions ? AnimationUtils._defaultOptions[effect] : null, options);
  };
  ...
  AnimationUtils._expandCollapse = function (element, _options, isExpand) {
    ...
    if (direction === 'both' || direction === 'height') {
      var startMaxHeight = AnimationUtils._getSizeLimit(element, style, options.startMaxHeight, isExpand, false);

      var endMaxHeight = AnimationUtils._getSizeLimit(element, style, options.endMaxHeight, !isExpand, false);
    ...
    if (direction === 'both' || direction === 'width') {
      var startMaxWidth = AnimationUtils._getSizeLimit(element, style, options.startMaxWidth, isExpand, true);

      var endMaxWidth = AnimationUtils._getSizeLimit(element, style, options.endMaxWidth, !isExpand, true);
  ...
  AnimationUtils.ripple = function (element, options) {
    ...
    return AnimationUtils._animate(rippler[0], fromState, toState, _options, [transformPropName, 'opacity']).then(function () {
      container.remove();
    });
  };
  ...
  AnimationUtils._flip = function (element, options, effect, startAngle, endAngle) {
    ...
    return AnimationUtils._animate(element, fromState, toState, options, [transformPropName], [transformPropName, backfaceVisPropName, transformOriginPropName]);
  };
  /**
  ...
  AnimationUtils._defaultHeroAnimateClonedElement = function (context) {
    return new Promise(function (resolve) {
      ...
      requestAnimationFrame(function () {
        ...
        var waitTime = AnimationUtils._getTimingValue(context.delay) + AnimationUtils._getTimingValue(context.duration);
  ...
  AnimationUtils._doAnimateHero = function (fromElement, toElementSelector, resolvedOptions, toElementElapsedTime, resolve, reject) {
    ...
    if (toElement == null) {
      ...
      if (toElementElapsedTime + interval > resolvedOptions.toElementWaitTime) {
        ...
        setTimeout(function () {
          AnimationUtils._doAnimateHero(fromElement, toElementSelector, resolvedOptions, toElementElapsedTime + interval, resolve, reject);
        }, interval);
      }
  ...
  AnimationUtils.animateHero = function (element, options) {
    ...
    return new Promise(function (resolve, reject) {
      if (!fromElement) {
        ...
        AnimationUtils._doAnimateHero(fromElement, options.toElementSelector, resolvedOptions, toElementElapsedTime, resolve, reject);
      }
    });
  };

  var startAnimation = AnimationUtils.startAnimation;
  var fadeIn = AnimationUtils.fadeIn;
  var fadeOut = AnimationUtils.fadeOut;
  var expand = AnimationUtils.expand;
  var collapse = AnimationUtils.collapse;
  var zoomIn = AnimationUtils.zoomIn;
  var zoomOut = AnimationUtils.zoomOut;
  var slideIn = AnimationUtils.slideIn;
  var slideOut = AnimationUtils.slideOut;
  var ripple = AnimationUtils.ripple;
  var flipIn = AnimationUtils.flipIn;
  var flipOut = AnimationUtils.flipOut;
  var addTransition = AnimationUtils.addTransition;
  var animateHero = AnimationUtils.animateHero;
