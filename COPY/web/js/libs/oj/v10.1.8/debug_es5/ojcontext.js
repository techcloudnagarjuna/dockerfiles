...
define(['ojs/ojcore-base', 'ojs/ojlogger'], function (oj, Logger) {
  ...
  var BusyState = function BusyState(description) {
    /**
     * @ignore
     * @private
     * @type {?}
  ...
  var BusyContext = function BusyContext(hostNode) {
    this.Init(hostNode);
  };
  ...
  BusyContext.prototype.Init = function (hostNode) {
    ...
    this._mediator = {
      ...
      getSlaveTimeoutPromise: function getSlaveTimeoutPromise(master, generateErrorCallback, timeout) {
        ...
        return Promise.race([master, slaveTimeoutPromise]).finally(this._clearAllSlaveTimeouts.bind(this));
      },
  ...
  BusyContext.prototype.clear = function () {
    ...
    for (var i = 0; i < busyStates.length; i++) {
      ...
      Object.defineProperty(busyState, BusyContext._OJ_RIP, {
        value: true,
        enumerable: false
      });
    }
  ...
  BusyContext.prototype.whenReady = function (timeout) {
    ...
    var master = mediator.getMasterWhenReadyPromise();
  ...
  BusyContext._BOOTSTRAP_MEDIATOR = new
  /** @constructor */
  function () {
    var _tracking;
  ...
  Context.prototype.getBusyContext = function () {
    if (!this._busyContext) {
      this._busyContext = new BusyContext(this._node);
    }
