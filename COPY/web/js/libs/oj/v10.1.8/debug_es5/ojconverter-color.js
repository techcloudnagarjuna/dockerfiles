 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojtranslation', 'ojs/ojconverter', 'ojs/ojvalidation-error'], function (oj, Translations, Converter, ojvalidationError) {
  'use strict';
  ...
  var ColorConverter = function ColorConverter(options) {
    this.Init(options);
  };
  /**
  ...
  */
  ...
  ColorConverter._toHslString = function (color) {
    ...
    return color._a === 1 ? 'hsl(' + h + ', ' + s + '%, ' + l + '%)' : 'hsla(' + h + ', ' + s + '%, ' + l + '%, ' + color._a + ')';
  };
  /**
   *  Converts an oj.Color object to a 3 or 6 hex character string
   *  @param {Object} color  The oj.Color object to be converted to a hex string.
  ...
  ColorConverter._toHsvString = function (color) {
    ...
    return color._a === 1 ? 'hsv(' + h + ', ' + s + '%, ' + v + '%)' : // "hsva(" + h + ", " + s + "%, " + v + "%, "+ this._roundA + ")";
    'hsva(' + h + ', ' + s + '%, ' + v + '%, ' + color._a + ')';
  };
  /**
  ...
  ColorConverter._rgbToHex = function (r, g, b, allow3Char) {
    var hex = [ColorConverter._pad2(Math.round(r).toString(16)), ColorConverter._pad2(Math.round(g).toString(16)), ColorConverter._pad2(Math.round(b).toString(16))]; // Return a 3 character hex if possible
  ...
  ColorConverter._rgbToHsl = function (r, g, b) {
    // eslint-disable-next-line no-param-reassign
    r = ColorConverter._bound01(r, 255); // eslint-disable-next-line no-param-reassign

    g = ColorConverter._bound01(g, 255); // eslint-disable-next-line no-param-reassign
  ...
  ColorConverter._bound01 = function (n, max) {
    ...
    var processPercent = ColorConverter._isPercentage(n); // eslint-disable-next-line no-param-reassign
