...
define(['exports', 'ojs/ojlogger', 'ojs/ojconverterutils-i18n', 'ojs/ojlocaledata', 'ojs/ojconverter', 'ojs/ojtranslation', 'ojs/ojconfig', 'ojs/ojcore-base', 'ojs/ojvalidation-error'], function (exports, Logger, __ConverterUtilsI18n, LocaleData, Converter, Translations, Config, oj$1, ojvalidationError) {
  ...
  var OraTimeZone = function () {
    ...
    function _packBase60Fraction(fraction, precision) {
      ...
      while (precision > 0) {
        ...
        precision -= 1; // eslint-disable-next-line no-param-reassign

        fraction *= 60;
        var current = Math.floor(fraction + _EPSILON);
        buffer += __BASE60[current]; // eslint-disable-next-line no-param-reassign
    ...
    function Zone(name, tzData) {
      var data = tzData.zones[name]; // Try  if name matches Etc/GMT offset
      ...
      if (_GMT_REGEXP.test(name)) {
        ...
        hours += hours >= 0 ? minutes : -minutes; // offset must be between -14 and +12
    ...
    Zone.prototype = {
      ...
      parse: function parse(target, dst, ignoreDst, throwException) {
        ...
        for (var i = 0; i < max; i++) {
          ...
          var dupTime = transitionTime - _HOUR; // Transition to dst:
    ...
    function _init() {
      return {
        getZone: function getZone(name, localeElements) {
          ...
          var s = _getZone(name, tzData); // try the links
  ...
  var OraDateTimeConverter = function () {
    var instance;
    ...
    var _TIME_FORMAT_REGEXP = /h|H|K|k/g; // This eslint disable require to make eacs happy
    ...
    function _expandPredefinedStylesFormat(options, localeElements, caller) {
      ...
      var fmtType = getOption('formatType', 'string', ['date', 'time', 'datetime'], 'date');
      var dStyle = getOption('dateFormat', 'string', ['short', 'medium', 'long', 'full'], 'short');
      var tStyle = getOption('timeFormat', 'string', ['short', 'medium', 'long', 'full'], 'short');
      ...
      if (dStyleFmt !== undefined && (fmtType === 'datetime' || fmtType === 'date')) {
        format = dStyleFmt;
      }

      if (tStyleFmt !== undefined && (fmtType === 'datetime' || fmtType === 'time')) {
        if (tzNotSupported) {
    ...
    function _throwInvalidDateFormat(format, options, m) {
      var isDate = options.year !== undefined || options.month !== undefined || options.weekday !== undefined || options.day !== undefined;
      var isTime = options.hour !== undefined || options.minute !== undefined || options.second !== undefined;
      var samplePattern;
      ...
      var msg = 'Unexpected character(s) ' + m + ' encountered in the pattern "' + format + ' An example of a valid pattern is "' + samplePattern + '".';
    ...
    function _basicFormatMatcher(dateTimeKeys, localeElements, isDate, hour12) {
      ...
      var dateTimeFormats = ['weekday', 'era', 'year', 'month', 'day', 'hour', 'minute', 'second', 'timeZoneName'];
      var values = {
        '2-digit': 0,
        numeric: 1,
        narrow: 2,
    ...
    function _toAvailableFormatsKeys(options, localeElements, caller) {
      ...
      var getOption = __ConverterUtilsI18n.OraI18nUtils.getGetOption(options, caller); // date key


      var option = getOption('era', 'string', ['narrow', 'short', 'long']);
      dateKey += _appendToKey(dateOptions, 'era', option, {
        narrow: 'GGGGG',
        short: 'GGG',
        long: 'GGGG'
      });
      option = getOption('year', 'string', ['2-digit', 'numeric']);
      dateKey += _appendToKey(dateOptions, 'year', option, {
        '2-digit': 'yy',
        numeric: 'y'
      });
    ...
    function _expandAvailableDateFormatsPattern(formatTemplate, options, caller) {
      ...
      var getOption = __ConverterUtilsI18n.OraI18nUtils.getGetOption(options, caller); // year
    ...
    function _getLocaleDecimalSeparator(localeElements, locale) {
      var numberingSystemKey = __ConverterUtilsI18n.OraI18nUtils.getNumberingSystemKey(localeElements, locale);
    ...
    function _expandFormat(options, localeElements, mlocale, caller) {
      ...
      var pattern;
      var matcher = getOption('formatMatcher', 'string', ['basic', 'munger'], 'munger');
      ...
      var hOption;
      var mOption; // append millisecnods to pattern
      ...
      if (msOption !== undefined) {
        ...
        mOption = getOption('minute', 'string', ['numeric', '2-digit']); // eslint-disable-next-line no-param-reassign

        options.second = '2-digit';
      }

      var dateTimeKeys = _toAvailableFormatsKeys(options, localeElements, caller); // First try to get the pattern from cache


      if (_localeDataCache[locale] !== undefined) {
        var cachedPattern = _localeDataCache[locale].dates.calendars.gregorian.dateTimeFormats[dateTimeKeys[0] + dateTimeKeys[1]];
      ...
      if (datePattern === undefined && dateTimeKeys[0] !== '') {
        datePattern = _getPatternFromSingleToken(dateTimeKeys[0], dateTimeKeys[2], availableFormats);

        if (datePattern === null) {
          datePattern = _basicFormatMatcher(dateTimeKeys[2], localeElements, true, hour12);
        }

        if (datePattern !== null) {
          if (matcher !== 'basic') {
            datePattern = _expandAvailableDateFormatsPattern(datePattern, options, caller);
          }
        } else {
      ...
      if (timePattern === undefined && dateTimeKeys[1] !== '') {
        timePattern = _getPatternFromSingleToken(dateTimeKeys[1], dateTimeKeys[3], availableFormats);

        if (timePattern === null) {
          timePattern = _basicFormatMatcher(dateTimeKeys[3], localeElements, true, hour12);
        }

        if (timePattern !== null) {
          if (matcher !== 'basic') {
            timePattern = _expandAvailableTimeFormatsPattern(timePattern, options, caller);
          }
        } else {
      ...
      _localeDataCache[locale].dates.calendars.gregorian.dateTimeFormats[dateTimeKeys[0] + dateTimeKeys[1]] = pattern;
    ...
    function _getMetazone(value, zoneName, metazones) {
      var nowObj = new Date(value[0], value[1] - 1, value[2], value[3], value[4], value[5]);
      ...
      for (var i = 0; i < length; i++) {
        ...
        var mzoneName = zone[i].usesMetazone._mzone; // time zone never chnaged. _to and _from undefined
    ...
    function _parseTimezoneOffset(_offset) {
      var offsetParts = _offset.split(':'); // offset is +hh:mm
    ...
    function _formatImpl(localeElements, options, isoStrInfo, locale) {
      ...
      var getOption = __ConverterUtilsI18n.OraI18nUtils.getGetOption(options, 'OraDateTimeConverter.format'); // get the pattern from options


      format = options.pattern || _expandFormat(options, localeElements, locale, 'OraDateTimeConverter.format'); // Start with an empty string
      ...
      function _getPart(date, _part) {
        switch (_part) {
          ...
          case 3:
            var dt = new Date(date[0], date[1] - 1, date[2], date[3], date[4], date[5]);
        ...
        if (timeZone !== undefined) {
          ...
          switch (isoStrInfo.format) {
            case _OFFSET:
              ...
              offset = hoursOffset * 60 + (__ConverterUtilsI18n.OraI18nUtils.startsWith(isoStrInfo.timeZone, '-') ? -minOffset : minOffset);
              break;
      ...
      for (;;) {
        ...
        var index = _TOKEN_REGEXP.lastIndex; // Look for the next pattern

        var ar = _TOKEN_REGEXP.exec(format); // Append the text before the pattern (or the end of the string if
        // not found)
        ...
        if (quoteCount % 2) {
          ...
          switch (currentPart.token) {
            ...
            case 'ampm':
              ...
              part = value[3] < 12 ? cal.dayPeriods.format.wide.am : cal.dayPeriods.format.wide.pm;
              ret.push(part);
              break;
            ...
            case 'tzhm':
              ...
              if (part === 0) {
                ...
                ret.push((part <= 0 ? '-' : '+') + __ConverterUtilsI18n.OraI18nUtils.padZeros(Math.floor(Math.abs(part / 60)), 2) + __ConverterUtilsI18n.OraI18nUtils.padZeros(Math.floor(Math.abs(part % 60)), 2));
              }
            ...
            case 'tzh':
              ...
              if (part === 0) {
                ...
                ret.push((part <= 0 ? '-' : '+') + __ConverterUtilsI18n.OraI18nUtils.padZeros(Math.floor(Math.abs(part / 60)), 2));
              }
    ...
    function _isSameWeek(localeElements, d1, d2) {
      if (d1 > d2) {
        ...
        var tmp = d1; // eslint-disable-next-line no-param-reassign

        d1 = d2; // eslint-disable-next-line no-param-reassign
    ...
    function _convertToLocalDate(d, localeElements, options) {
      ...
      var isoInfoFormat = isoInfo.format; // first test id d is local
      ...
      var zuluDate = _parseImpl(d, localeElements, tzOptions, 'en-US'); // second, convert to local date
    ...
    function _formatRelativeDisplayName(isoNow, isoValue, options, localeElements) {
      ...
      var getOption = __ConverterUtilsI18n.OraI18nUtils.getGetOption(options, 'OraDateTimeConverter.formatRelative');

      var option = getOption('dateField', 'string', ['day', 'week', 'month', 'year', 'hour', 'minute', 'second']);
    ...
    function _formatRelativeImplicit(now, relativeDate, localeElements, field) {
      ...
      if (field === null) {
        // eslint-disable-next-line no-param-reassign
        field = units.second < _THRESHOLDS.s && 'second' || units.minute < _THRESHOLDS.m && 'minute' || units.hour < _THRESHOLDS.h && 'hour' || units.day < _THRESHOLDS.d && 'day' || units.week < _THRESHOLDS.w && 'week' || units.month < _THRESHOLDS.M && 'month' || 'year';
      ...
      var format = fields[field][x][pluralEntry]; // some locales only have other plural entry
    ...
    function _formatRelativeImpl(value, localeElements, options) {
      ...
      var fieldOption = getOption('dateField', 'string', ['day', 'week', 'month', 'year', 'hour', 'minute', 'second']); // eslint-disable-next-line no-param-reassign
      ...
      if (toNow) {
        var tmp = now;
        now = value; // eslint-disable-next-line no-param-reassign

        value = tmp;
      }

      var formatUsing = getOption('formatUsing', 'string', ['displayName', 'calendar'], 'displayName');
    ...
    function _throwDateFormatMismatch(value, format, style) {
      ...
      if (style === 2) {
        msg = 'The value "' + value + '" does not match the expected date-time format "' + format + '"';
    ...
    function _expandYear(start2DigitYear, year) {
      ...
      if (year < 100) {
        var ambiguousTwoDigitYear = start2DigitYear % 100; // eslint-disable-next-line no-param-reassign
    ...
    function _arrayIndexOfDay(daysObj, item) {
      ...
      for (var i = 0; i < daysKeys.length; i++) {
        ...
        if (__ConverterUtilsI18n.OraI18nUtils.trim(__ConverterUtilsI18n.OraI18nUtils.toUpper(daysObj[d])) === __ConverterUtilsI18n.OraI18nUtils.trim(item)) {
          return days[d];
        }
      }
    ...
    function _arrayIndexOfMonth(monthsObj, item) {
      ...
      for (var i = 0; i < monthsKeys.length; i++) {
        ...
        if (__ConverterUtilsI18n.OraI18nUtils.trim(__ConverterUtilsI18n.OraI18nUtils.toUpper(monthsObj[m])) === __ConverterUtilsI18n.OraI18nUtils.trim(item)) {
          return i;
        }
      }
    ...
    function _getDayIndex1(localeElements, value, fmt) {
      ...
      var calDaysStdAlone = mainNode.dates.calendars.gregorian.days['stand-alone'];

      switch (fmt) {
        case 0:
          days = [calDaysFmt.abbreviated, calDaysFmt.wide];
          break;

        case 1:
          days = [calDaysStdAlone.abbreviated, calDaysStdAlone.wide];
          break;
    ...
    function _getMonthIndex(localeElements, value, fmt) {
      ...
      var calMonthsStdAlone = mainNode.dates.calendars.gregorian.months['stand-alone'];

      switch (fmt) {
        case 0:
          months = [calMonthsFmt.wide, calMonthsFmt.abbreviated];
          break;

        case 1:
          months = [calMonthsStdAlone.wide, calMonthsStdAlone.abbreviated];
          break;

        case 2:
          months = [calMonthsFmt.wide, calMonthsFmt.abbreviated, calMonthsStdAlone.wide, calMonthsStdAlone.abbreviated];
          break;
    ...
    function _getParseRegExp(format, options) {
      var re = {}; // expand single digit formats, then escape regular expression
      //  characters.
      ...
      var groups = [];
      var index = 0;
      var quoteCount = 0; // iterate through each date token found.

      var match = _TOKEN_REGEXP.exec(expFormat);

      while (match !== null) {
        var preMatch = expFormat.slice(index, match.index);
        index = _TOKEN_REGEXP.lastIndex; // don't replace any matches that occur inside a string literal.
    ...
    function _validateRange(name, value, low, high, displayValue, displayLow, displayHigh) {
      if (value < low || value > high) {
        var msg = displayValue + ' is out of range.  Enter a value between ' + displayLow + ' and ' + displayHigh + ' for ' + name;
    ...
    function _parseLenienthms(result, timepart, format, localeElements, dtype) {
      ...
      var calPM = mainNode.dates.calendars.gregorian.dayPeriods.format.wide.pm; // hour, optional minutes and optional seconds
      ...
      switch (match[0]) {
        case 'h':
          ...
          idx = __ConverterUtilsI18n.OraI18nUtils.toUpper(timepart).indexOf(__ConverterUtilsI18n.OraI18nUtils.toUpper(calPM));
      ...
      _validateRange('minute', minute, 0, 59, minute, 0, 59); // Seconds.


      _validateRange('second', second, 0, 59, second, 0, 59); // millisec
    ...
    function _getWeekdayName(value, localeElements) {
      ...
      var days = [calDaysFmt.wide, calDaysFmt.abbreviated, calDaysStandAlone.wide, calDaysStandAlone.abbreviated];
    ...
    function _parseLenientyMEd(value, format, options, localeElements, isDateTime) {
      ...
      for (i = 1; i <= 3; i++) {
        var tokenMatch = match[i]; // find year if year is yyy|yyyy
      ...
      var daysInMonth = __ConverterUtilsI18n.OraI18nUtils._getDaysInMonth(year, month); // if both month and day > 12 and swapped, throw exception
      ...
      var parsedDate = new Date(year, month, day); // locate weekday

      var dName = _getWeekdayName(value, localeElements);

      if (dName !== null) {
        var weekDay = _getDayIndex1(localeElements, dName, 0); // day of week does not match date
    ...
    function _parseLenientyMMMEd(value, format, options, localeElements, isDateTime) {
      var origValue = value; // eslint-disable-next-line no-param-reassign

      value = __ConverterUtilsI18n.OraI18nUtils.toUpper(value);

      var mainNode = __ConverterUtilsI18n.OraI18nUtils.getLocaleElementsMainNode(localeElements); // locate month name


      var calMonthsFmt = mainNode.dates.calendars.gregorian.months.format;
      var calMonthsStandAlone = mainNode.dates.calendars.gregorian.months['stand-alone'];
      var months = [calMonthsFmt.wide, calMonthsFmt.abbreviated, calMonthsStandAlone.wide, calMonthsStandAlone.abbreviated];
      ...
      for (i = 0; i < months.length; i++) {
        ...
        for (j = 0; j < reverseMonth.length; j++) {
          ...
          if (value.indexOf(mName) !== -1) {
            foundMatch = true; // eslint-disable-next-line no-param-reassign
      ...
      _validateRange('month', month, 0, 11, month, 1, 12); // locate weekday
    ...
    function _getNameIndex(localeElements, datePart, matchGroup, mLength, style, matchIndex, start1, end1, start2, end2, name) {
      var index;
      ...
      _validateRange(name, index, start1, end1, matchGroup, startName, endName);
    ...
    function _parseExact(value, format, options, localeElements) {
      var mainNode = __ConverterUtilsI18n.OraI18nUtils.getLocaleElementsMainNode(localeElements); // remove spaces from era.


      var cal = mainNode.dates.calendars.gregorian;
      var eraPart = cal.eras.eraAbbr['1'];

      var trimEraPart = __ConverterUtilsI18n.OraI18nUtils.trimNumber(eraPart); // eslint-disable-next-line no-param-reassign
      ...
      var dtStyle = _dateTimeStyle(options, 'OraDateTimeConverter.parse'); // convert date formats into regular expressions with groupings.
      ...
      var start2DigitYear = _get2DigitYearStart(options); // iterate the format groups to extract and set the date fields.
      ...
      for (var j = 0, jl = groups.length; j < jl; j++) {
        ...
        if (matchGroup) {
          ...
          switch (currentGroup.token) {
            case 'months':
              month = _getNameIndex(localeElements, currentGroup.token, matchGroup, currentGroup.mLen, currentGroup.style, currentGroup.matchIndex, 0, 11, '1', '12', 'month name');
              break;

            case 'days':
              weekDayName = matchGroup;
              weekDay = _getNameIndex(localeElements, currentGroup.token, matchGroup, currentGroup.dLen, currentGroup.style, currentGroup.matchIndex, 0, 6, 'sun', 'sat', 'weekday');
              break;
            ...
            case 'dayOfMonth':
              date = matchInt; // try leneient parse for date style only
            ...
            case 'monthIndex':
              // Month.
              month = matchInt - 1; // try leneient parse for date style only
            ...
            case 'ampm':
              pmHour = __ConverterUtilsI18n.OraI18nUtils.toUpper(matchGroup).indexOf(__ConverterUtilsI18n.OraI18nUtils.toUpper(calPM)) !== -1;
              break;
      ...
      _validateRange('month', month, 0, 11, month + 1, 1, 12); // validate day range, depending on the month and year
      ...
      parsedDate.setFullYear(year, month, date); // day of week does not match date
      ...
      var parts = [year, month + 1, date, timeObj.hour, timeObj.minute, timeObj.second, timeObj.millisec];
    ...
    function _getResolvedOptionsFromPattern(locale, numberingSystemKey, pattern) {
      ...
      while (match !== null) {
        var preMatch = expFormat.slice(index, match.index);
        index = _TOKEN_REGEXP.lastIndex; // skip matches that occur inside a string literal.
    ...
    function _dateTimeStyleFromPattern(pattern) {
      ...
      var isDate = result.year !== undefined || result.month !== undefined || result.weekday !== undefined || result.day !== undefined;
      var isTime = result.hour !== undefined || result.minute !== undefined || result.second !== undefined || result.millisecond !== undefined;
    ...
    function _dateTimeStyle(options, caller) {
      ...
      var isTime = getOption('hour', 'string', ['2-digit', 'numeric']) !== undefined || getOption('minute', 'string', ['2-digit', 'numeric']) !== undefined || getOption('second', 'string', ['2-digit', 'numeric']) !== undefined || getOption('millisecond', 'string', ['numeric']) !== undefined;
      var isDate = getOption('year', 'string', ['2-digit', 'numeric']) !== undefined || getOption('month', 'string', ['2-digit', 'numeric', 'narrow', 'short', 'long']) !== undefined || getOption('day', 'string', ['2-digit', 'numeric']) !== undefined || getOption('weekday', 'string', ['narrow', 'short', 'long']) !== undefined;
      ...
      var option = getOption('formatType', 'string', ['date', 'time', 'datetime'], 'date');
    ...
    function _adjustHours(isoStrInfo, options, localeElements) {
      ...
      newOffset = _getStdOffset(zone, value); // 2.adjust utcd to target zone

      utcd -= (newOffset + origOffset) * 60000; // 3.get target zone offset
      ...
      newOffset = -zone.ofset(index); // adjust the offset

      newOffset -= origOffset; // Do the offset math through the Date object.

      var adjustD = new Date(Date.UTC(value[0], value[1] - 1, value[2], value[3], value[4], value[5]));
      var adjustedMin = adjustD.getUTCMinutes() + newOffset; // eslint-disable-next-line no-bitwise
    ...
    function _createISOStrParts(dtStyle, d) {
      ...
      switch (dtStyle) {
        // Date only
        case 0:
          val = __ConverterUtilsI18n.OraI18nUtils.padZeros(d[0], 4) + '-' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[1], 2) + '-' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[2], 2);
          break;
        // Time only

        case 1:
          val = 'T' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[3], 2) + ':' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[4], 2) + ':' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[5], 2);
          ms = d[6];
        ...
        default:
          val = __ConverterUtilsI18n.OraI18nUtils.padZeros(d[0], 4) + '-' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[1], 2) + '-' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[2], 2) + 'T' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[3], 2) + ':' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[4], 2) + ':' + __ConverterUtilsI18n.OraI18nUtils.padZeros(d[5], 2);
          ms = d[6];
    ...
    function _getParseISOStringOffset(tzName, parts, dst, ignoreDst, localeElements, thowException) {
      ...
      var index = _parseZone(zone, parts, dst, ignoreDst, thowException); // hours
    ...
    function _createParseISOStringFromDate(dtStyle, isoStrInfo, options, localeElements) {
      ...
      var isoFormat = getOption('isoStrFormat', 'string', [_ZULU, _OFFSET, _INVARIANT, _LOCAL, _AUTO], _AUTO);
      var dst = getOption('dst', 'boolean', [true, false], false);
      var ignoreDst = true;
      ...
      var val = _createISOStrParts(dtStyle, parts); // do not include timezone if date-only or local
      ...
      switch (isoFormat) {
        ...
        case _ZULU:
          ...
          if (adjustedMin !== 0) {
            ...
            var adjustD = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], parts[3], parts[4], parts[5], parts[6]));
            adjustedMin = adjustD.getUTCMinutes() + adjustedMin; // eslint-disable-next-line no-bitwise
    ...
    function _parseImpl(str, localeElements, options, locale) {
      ...
      var dtStyle;
      var formats; // First try if str is an iso 8601 string
      ...
      if (testIsoStr === true) {
        ...
        formats = options.pattern || _expandFormat(options, localeElements, locale, 'OraDateTimeConverter.parse');
    ...
    function _getPatternResolvedOptions(isoFormat, tz, dst, locale, numberingSystemKey, options, getOption) {
      var result = _getResolvedOptionsFromPattern(locale, numberingSystemKey, options.pattern);
    ...
    function _getPredefinedStylesResolvedOptions(_result, options, localeElements, getOption) {
      ...
      var format = _expandPredefinedStylesFormat(options, localeElements, OraDateTimeConverter.resolvedOptions);
    ...
    function _getRelativeTimeResolvedOptions(getOption, _result) {
      ...
      option = getOption('dateField', 'string', ['day', 'week', 'month', 'year', 'hour', 'minute', 'second']);

      if (option !== undefined) {
        result.dateField = option;
      }

      option = getOption('relativeTime', 'string', ['fromNow', 'toNow']);
    ...
    function _getResolvedOptions(options, getOption, tz, dst, isoFormat, localeElements, numberingSystemKey, locale) {
      var result = {
        locale: locale,
        numberingSystem: numberingSystemKey,
        calendar: 'gregorian'
      ...
      result.lenientParse = getOption('lenientParse', 'string', ['none', 'full'], 'full');
      result.patternFromOptions = _expandFormat(result, localeElements, locale, 'OraDateTimeConverter.resolvedOptions');
      return result;
    }
    ...
    function _getResolvedDefaultOptions(localeElements, locale, numberingSystemKey) {
      ...
      var patternFromOptions = _expandFormat(defaultOptions, localeElements, locale, 'OraDateTimeConverter.resolvedOptions');
    ...
    function _resolvedOptionsImpl(localeElements, options, locale) {
      ...
      if (options !== undefined) {
        isoFormat = getOption('isoStrFormat', 'string', [_ZULU, _OFFSET, _INVARIANT, _LOCAL, _AUTO], _AUTO);
      ...
      if (options !== undefined && options.pattern !== undefined) {
        return _getPatternResolvedOptions(isoFormat, tz, dst, locale, numberingSystemKey, options, getOption);
      }

      if (options !== undefined) {
        return _getResolvedOptions(options, getOption, tz, dst, isoFormat, localeElements, numberingSystemKey, locale);
      }
    ...
    function _availableTimeZonesImpl(localeElements) {
      ...
      var tzData = localeElements.supplemental.timeZoneData;
      var d = new Date();
      var dParts = [d.getFullYear(), d.getMonth() + 1, d.getDate(), d.getHours(), d.getMinutes(), d.getSeconds()];
      ...
      function getLocalizedName(id, offset, _metaZones, _cities) {
        ...
        if (metaZone !== undefined && metaZone !== null && metaZone.long !== undefined) {
          locZone = metaZone.long.generic; // some metazones do not have generic. Use standard
        ...
        if (offset !== 0) {
          locName = __ConverterUtilsI18n.OraI18nUtils.getTimeStringFromOffset(_UTC, offset, true, true);
          locName = '(' + locName + ')';
        }
      ...
      pushZoneNameObject(tzData.zones); // add the links
      ...
      var len = sortedZones.length; // return an array with "display name with offset" instead of the
    ...
    function _init() {
      return {
         ...
         * @export
         */
        formatRelative: function formatRelative(value, localeElements, options) {
         ...
         */
        calculateWeek: function calculateWeek(date) {
          var d = __ConverterUtilsI18n.OraI18nUtils._IsoStrParts(date);

          var time;
          var checkDate = new Date(Date.UTC(d[0], d[1] - 1, d[2])); // Find Thursday of this week starting on Monday
         ...
         */
        getAvailableTimeZones: function getAvailableTimeZones(localeElements) {
         ...
         */
        compareISODates: function compareISODates(isoStr1, isoStr2, localeElements) {
         ...
         * @since 2.2
         */
        isHour12: function isHour12(localeElements) {
         ...
         */
        getTimePositioning: function getTimePositioning(localeElements, options, locale) {
  ...
  var DateTimeConverter = function DateTimeConverter(options) {
    this.Init(options);
  ...
  var IntlDateTimeConverter = function IntlDateTimeConverter(options) {
    this.Init(options);
  };
  /**
  ...
  IntlDateTimeConverter.prototype.format = function (value) {
    ...
    var resolvedOptions = this.resolvedOptions(); // undefined, null and empty string values all return null. If value is NaN then return "".
    ...
    if (value == null || typeof value === 'string' && oj$1.StringUtils.trim('' + value).length === 0) {
      return '';
    }
  ...
  IntlDateTimeConverter.prototype._getHintValue = function () {
    ...
    try {
      // example date
      value = this.format(__ConverterUtilsI18n.IntlConverterUtils.dateToLocalIso(IntlDateTimeConverter._DEFAULT_DATE));
    } catch (e) {
  ...
  IntlDateTimeConverter.prototype.resolvedOptions = function () {
    var localeElements;
    var locale = Config.getLocale();
    var options = this.getOptions(); // options are resolved and cached for a locale
    ...
    if (locale !== this._locale || !this._resolvedOptions) {
      ...
      try {
        ...
        this._resolvedOptions = this._getWrapped().resolvedOptions(localeElements, options, locale);
        this._locale = locale;
      } catch (e) {
  ...
  IntlDateTimeConverter.prototype.calculateWeek = function (value) {
    return this._getWrapped().calculateWeek(value, LocaleData.__getBundle(), Config.getLocale());
  };
  /**
  ...
  IntlDateTimeConverter.prototype.parse = function (value) {
    ...
    try {
      ...
      if (parsed) {
        ...
        // field.
        if (result.warning) {
          Logger.warn('The value ' + value + ' was leniently parsed to represent a date ' + parsed.toString ? parsed.toString() : parsed);
        }
      }
  ...
  IntlDateTimeConverter.prototype._processConverterError = function (e, value) {
    ...
    if (errorInfo) {
      ...
      propName = Translations.getTranslatedString('oj-converter.datetime.datetimeOutOfRange.' + propName); // update the map back so that functions like __getConverterOptionError use new value
      ...
      if (e instanceof TypeError) {
        ...
        if (errorCode === 'optionOutOfRange') {
          ...
          // TODO: NLS should use lower case time
        ...
        if (errorCode === 'dateFormatMismatch') {
          ...
          summary = Translations.getTranslatedString('oj-converter.datetime.invalidTimeZoneID.summary', {
          ...
          summary = Translations.getTranslatedString('oj-converter.datetime.dateToWeekdayMismatch.summary', {
            date: parameterMap.date,
            weekday: parameterMap.weekday
          });
  ...
  IntlDateTimeConverter.prototype._getTimePositioning = function (param) {
    ...
    return this._getWrapped().getTimePositioning(LocaleData.__getBundle(), resolvedOption, Config.getLocale());
  };
