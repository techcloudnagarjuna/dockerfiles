...
define(['ojs/ojcore-base', 'jquery', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojlogger', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojdomutils', 'touchr'], function (oj, $, ThemeUtils, Components, Logger, Context, Config, DomUtils, touchr) {
  ...
  */
  ...
  function ConveyorBeltCommon(elem, options, buttonInfo, callbackInfo, styleInfo, agentInfo) {
  ...
  ConveyorBeltCommon.prototype.setup = function () {
    ...
    this._addResizeListenerFunc(this._contentContainer, this._handleResizeFunc); // notify the child that it's being re-attached to the DOM AFTER attaching it
  ...
  ConveyorBeltCommon.prototype._setCurrScrollHelper = function (scroll, bImmediate) {
    ...
    if (bImmediate || !scrollFunc || scroll === this._getCurrScroll()) {
      ...
      scrollFunc(this._overflowContainer, this._convertScrollLogicalToBrowser(scroll), duration, onEndFunc);
    }
  };
  /**
  ...
  ConveyorBeltCommon.prototype._handleTouchMove = function (event) {
    ...
    var diff = this._touchCurrentCoord - this._touchLastCoord; // in non-RTL, if swiping left or up, scroll next; otherwise scroll prev
    ...
    if (this._bTouch && this._firstTouch.id === currentTouch.id && canScrollInSwipeDirection) {
      ...
      this._touchLastCoord = this._touchCurrentCoord; // if we're under the threshold, but we've already scrolled to the end,
    ...
    oj.__registerWidget('oj.ojConveyorBelt', $.oj.baseComponent, {
      ...
      _setup: function _setup() {
        ...
        if (!this._cbCommon) {
          ...
          this._cbCommon = new ConveyorBeltCommon(elem[0], {
            orientation: orientation,
            contentParent: contentParentElem,
            bRtl: this._bRTL
          }, buttonInfo, callbackInfo, styleInfo, agentInfo);
        }
