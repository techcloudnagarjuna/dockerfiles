 ...
 * @ignore
 */
define(['ojs/ojexpparser', 'ojs/ojkoshared'], function (ojexpparser, ojkoshared) {
  'use strict';
  /**
   * @license
  ...
  var CspExpressionEvaluator = function CspExpressionEvaluator(options) {
    ...
    function _evaluate(node, contexts) {
      switch (node.type) {
        ...
        case 6:
          ...
          return _binops[node.operator](_evaluate(node.left, contexts), _evaluate(node.right, contexts));
        ...
        case 8:
          ...
          return _evaluate(node.test, contexts) ? _evaluate(node.consequent, contexts) : _evaluate(node.alternate, contexts);
    ...
    function _evaluateConstructorExpression(node, contexts) {
      ...
      return new (Function.prototype.bind.apply(constrObj, [null].concat(_evaluateArray(node.arguments, contexts))))();
    }
