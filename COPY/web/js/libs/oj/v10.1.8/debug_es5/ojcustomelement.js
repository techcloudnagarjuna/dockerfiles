 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojlogger', 'customElements', 'ojs/ojcustomelement-utils', 'ojs/ojbootstrap'], function (oj, Logger, customElements, ojcustomelementUtils, Bootstrap) {
  'use strict';
  ...
  BaseCustomElementBridge.proto = {
    getClass: function getClass(descriptor) {
      ...
      BaseCustomElementBridge._enumerateMetadataForKey(proto, metadata, 'methods', this.DefineMethodCallback.bind(this)); // Add additional element methods not defined in metadata, e.g. getNodeBySubId/getSubIdByNode or get/setProperty
    ...
    AttributeChangedCallback: function AttributeChangedCallback(attr, oldValue, newValue) {
      ...
      if (oldValue !== newValue || bridge.State.dirtyProps.has(prop)) {
        bridge.State.dirtyProps.delete(prop.split('.')[0]); // Due to  where IE11 disables child inputs for a parent with the disabled attribute,
        ...
        if (bridge.ShouldHandleAttributeChanged(this)) {
          BaseCustomElementBridge.__CheckOverlappingAttribute(this, attr); // removeAttribute calls return null as the newValue which we want to treat as
          ...
          if (!expression) {
            var propMeta = BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(this));

            if (propMeta) {
              this.setProperty(prop, BaseCustomElementBridge.__ParseAttrValue(this, attr, prop, newValue, propMeta));
    ...
    GetEventListenerProperty: function GetEventListenerProperty(property) {
      var event = ojcustomelementUtils.AttributeUtils.eventListenerPropertyToEventType(property); // Get event listener
    ...
    GetProperty: function GetProperty(element, prop, props) {
      var meta = BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(element)); // For event listener and non component properties, retrieve the value directly stored on the element.
      ...
      return ojcustomelementUtils.CustomElementUtils.getComplexProperty(props, prop);
    },
    ...
    HandlePropertyChanged: function HandlePropertyChanged(element, property, value, previousValue, isOuter, subprop) {
      ...
      BaseCustomElementBridge.__FirePropertyChangeEvent(element, property, value, previousValue, updatedFrom, subprop);
    },
     ...
     * @return {boolean}
     */
    SaveEarlyPropertySet: function SaveEarlyPropertySet(element, prop, value) {
    ...
    SetProperty: function SetProperty(element, prop, value, props, bOuter) {
      ...
      var meta = BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(element));
      ...
      if (ojcustomelementUtils.AttributeUtils.isEventListenerProperty(prop) || !meta) {
        ...
        if (!BaseCustomElementBridge.__CompareOptionValues(prop, meta, value, previousValue)) {
          ...
          if (bOuter) {
            ...
            BaseCustomElementBridge.__SetProperty(this.GetAliasForProperty.bind(this), props, prop, value);
          }
          ...
          if (isSubprop) {
            ...
            this.HandlePropertyChanged(element, topProp, props[topProp], topPropPrevValue, bOuter, subprop);
          }
    ...
    ValidatePropertySet: function ValidatePropertySet(element, property, value) {
      ...
      if (!propMeta) {
        Logger.warn(ojcustomelementUtils.CustomElementUtils.getElementInfo(element) + ": Ignoring property set for undefined property '" + property + "'.");
        return undefined;
    ...
    _connected: function _connected(element) {
      ...
      if (!this._bCreateCalled) {
        ...
        this.InitializeElement(element); // Should be able to call PlaybackEarlyPropertySets inside
        ...
        var createComponentCallback = function createComponentCallback() {
          ...
          if (!state.isDisposed) {
            var createPromise = _this.CreateComponent(element) || Promise.resolve();
            return createPromise.then(_this.PostCreate.bind(_this, element));
          } // Reset the create flag


          _this._bCreateCalled = false;
  ...
  BaseCustomElementBridge.checkEnumValues = function (element, property, value, metadata) {
    ...
    if (typeof value === 'string' && metadata) {
      ...
      if (enums && enums.indexOf(value) === -1) {
        ...
        bridge.State.throwError("Invalid value '" + value + "' found for property '" + property + "'. Expected one of the following '" + enums.toString() + "'.");
      }
    }
  };
  /**
  ...
  BaseCustomElementBridge.checkType = function (element, property, value, metadata) {
    ...
    var supportedTypes = ojcustomelementUtils.ElementUtils.getSupportedTypes(type);

    if (supportedTypes.typeCount === 1 && !supportedTypes.any && !supportedTypes.other) {
      var valueType = _typeof(value);

      if (supportedTypes.array && !Array.isArray(value) || supportedTypes.object && !(valueType === 'object') || supportedTypes.string && !(valueType === 'string') || supportedTypes.number && !(valueType === 'number' && isFinite(value))) {
  ...
  BaseCustomElementBridge.__ThrowTypeError = function (element, property, value, type) {
    var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(element);
    bridge.State.throwError("Invalid type '" + _typeof(value) + "' found for property '" + property + "'. Expected value of type '" + type + "'.");
  };
  /**
  ...
  BaseCustomElementBridge.__InitProperties = function (element, componentProps) {
    ...
    if (metaProps) {
      var elementState = bridge.State;
      elementState.isInitializingProperties = true;
      var attrs = element.attributes; // attrs is a NodeList

      for (var i = 0; i < attrs.length; i++) {
        var attr = attrs[i];
        var property = ojcustomelementUtils.AttributeUtils.attributeToPropertyName(attr.nodeName); // See if attribute is a component property
        ...
        if (meta && !meta.readOnly) {
          ...
          if (!info.expr) {
            var value = BaseCustomElementBridge.__ParseAttrValue(element, attr.nodeName, property, attr.value, meta);
  ...
  BaseCustomElementBridge.__ParseAttrValue = function (elem, attr, prop, val, metadata) {
    ...
    function _coerceVal(value) {
      ...
      try {
        coercedValue = ojcustomelementUtils.AttributeUtils.coerceValue(elem, attr, value, type);
      } catch (ex) {
  ...
  BaseCustomElementBridge.__ProcessEventListeners = function (_metadata) {
    ...
    BaseCustomElementBridge._enumerateMetadataForKey(null, metadata, 'properties', function (proto, property) {
    ...
    BaseCustomElementBridge._enumerateMetadataForKey(null, metadata, 'events', function (proto, event) {
  ...
  BaseCustomElementBridge.__FirePropertyChangeEvent = function (element, name, value, previousValue, updatedFrom, subprop) {
    var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(element); // There are cases where a subproperty set can trigger a top level property set
