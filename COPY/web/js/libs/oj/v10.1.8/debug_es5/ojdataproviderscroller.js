 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojanimation', 'ojs/ojlogger', 'ojs/ojdatacollection-common', 'ojs/ojdomscroller', 'ojs/ojset', 'ojs/ojmap'], function (exports, oj, $, Context, Config, AnimationUtils, Logger, DataCollectionUtils, DomScroller, KeySet, KeyMap) {
  'use strict';
  ...
  DataProviderContentHandler.prototype.FindElementByKey = function (key) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if ((key == this.GetKey(elem) || oj.Object.compareValues(key, this.GetKey(elem))) && !elem.classList.contains('oj-listview-item-remove')) {
        return elem;
      }
    }
  ...
  DataProviderContentHandler.prototype.replaceItem = function (item, index, data, metadata, templateEngine, callback, restoreFocus) {
    ...
    return this._addOrReplaceItem(newItem, position, parentElement, index, data, metadata, templateEngine, callback, restoreFocus);
  };
  /**
  ...
  DataProviderContentHandler.prototype.afterRenderItemForInsertEvent = function (item, context, isCustomizeItem) {
    ...
    var isCardLayout = this.isCardLayout(); // add card style class to wrapper/transforming div

    if (isCardLayout) {
      // eslint-disable-next-line no-param-reassign
      item.className = item.className + ' oj-listview-card-animated ' + this.m_widget.getItemStyleClass();
    }
  ...
  DataProviderContentHandler.prototype._handleAddTransitionEnd = function (context, elem, currentClassName) {
  ...
  DataProviderContentHandler.prototype.handleModelRemoveEvent = function (event) {
    ...
    keys.forEach(function (key) {
      ...
      if (elem != null) {
        ...
        elem.classList.add('oj-listview-item-remove'); // item remove might have been just added (in insert animation phase)
  ...
  DataProviderContentHandler.prototype.animateShowContent = function (parentElem, content, shouldEmptyElem) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (skeletonContainer != null) {
        ...
        var promises = []; // but hide them for now
        ...
        Promise.all(promises).then(function () {
          ...
          AnimationUtils.fadeOut(skeletonContainer, {
            ...
            if (skeletonContainerRoot.classList.contains('oj-listview-initial-skeletons')) {
              ...
              skeletonContainer.querySelectorAll('.oj-listview-skeleton').forEach(function (skeleton) {
  ...
  IteratingDataProviderContentHandler.prototype.fetchRows = function (forceFetch) {
    ...
    if (this.IsReady()) {
      ...
      IteratingDataProviderContentHandler.superclass.fetchRows.call(this, forceFetch); // this is the key of the item that we want to scroll to initially

      var scrollToKeyPromise = this.m_widget.getScrollToKey(); // initiate loading of template engine, note it will not load it unless a template has been specified
      ...
      var helperFunction = function helperFunction(values, currentMetadata, scrollToKey) {
      ...
      Promise.all([promise, enginePromise, scrollToKeyPromise]).then(function (values) {
  ...
  IteratingDataProviderContentHandler.prototype._removeDuplicateItems = function (keys) {
    if (this._hasPendingInsertKeys()) {
      var self = this; // populate initial values using Map instead of KeyMap because we know the keys are unique to avoid
      // the overhead checking logic

      var elemMap = new Map();
      this.m_root.querySelectorAll('.' + this.m_widget.getItemElementStyleClass()).forEach(function (elem) {
        if (elem.tagName !== 'LI') {
  ...
  IteratingDataProviderContentHandler.prototype._handleFetchSuccess = function (data, keys, doneOrMaxLimitReached, templateEngine, isMouseWheel, metadata, isInitialFetch) {
    ...
    for (var i = 0; i < data.length; i++) {
      var row = data[i];
      var key = keys[i];
      var callbackFunc = this._isFetchFromInsert(key) ? this.afterRenderItemForInsertEvent.bind(this) : null; // passing -1 for opt since we know it will be inserted at the end of the parent

      this.addItem(parent, -1, row, this.getMetadata(index, key, row), templateEngine, callbackFunc, metadata != null ? metadata[i] : null);
      index += 1;
    }
  ...
  IteratingDataProviderContentHandler.prototype.handleDomScrollerFetchedData = function (result) {
    if (result != null) {
      ...
      if (result.value && result.value.data) {
        ...
        this._handleScrollerMaxRowCount(); // since fetchEnd would not be called in this case
  ...
  IteratingDataProviderContentHandler.prototype._registerDomScroller = function () {
    ...
    var options = {
      ...
      beforeFetch: function beforeFetch(scrollTop, isCheckViewport) {
  ...
  IteratingDataProviderContentHandler.prototype.handleModelAddEvent = function (event) {
    ...
    var processed = IteratingDataProviderContentHandler.superclass.handleModelAddEvent.call(this, event);
  ...
  IteratingDataProviderContentHandler.prototype._getMaxIndexForInsert = function () {
    var max = Number.MAX_VALUE; // temporary solution for SDP
    ...
    if (this._isLoadMoreOnScroll()) {
      max = $(this.m_root).children('li.' + this.m_widget.getItemElementStyleClass()).length; // todo: maybe can combine to a single selector

      var itemsToAdd = this.m_root.querySelectorAll('.oj-listview-temp-item');
      var itemsToAddButRemove = this.m_root.querySelectorAll('.oj-listview-temp-item.oj-listview-item-remove');
  ...
  IteratingDataProviderContentHandler.prototype.addItemsForModelInsert = function (data, indexes, keys, parentKeys, isBeforeKeys, refKeys, metadata) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (index < max) {
        var promise = this.addItem(this.m_root, index, data[i], this.getMetadata(index, keys[i], data[i]), templateEngine, this.afterRenderItemForInsertEvent.bind(this), metadata != null ? metadata[i] : null);
  ...
  IteratingDataProviderContentHandler.prototype.handleModelRemoveEvent = function (event) {
    this.m_currentEvents.push(event);
    var processed = IteratingDataProviderContentHandler.superclass.handleModelRemoveEvent.call(this, event);
  ...
  IteratingDataProviderContentHandler.prototype.handleRemoveItemsPromises = function (promises) {
    if (promises.length === 0) {
      ...
      var self = this; // check viewport after multiple items are removed
  ...
  IteratingDataProviderContentHandler.prototype.handleModelRefreshEvent = function (event) {
    ...
    this.m_widget.adjustScrollPositionValueOnFetch(); // reset focus if needed
        ...
        fetchPromise.then(function (result) {
          ...
          if (self.m_widget != null) {
            if (result != null) {
              ...
              var _horizontalViewportCheck = function _horizontalViewportCheck() {
