  ...
  * @memberof! oj.ojInputDate
  ...
  oj.__registerWidget('oj.ojInputDate', $.oj.inputBase, {
    ...
    _createDpDiv: function _createDpDiv() {
      ...
      this._dpDiv = bindHover($(dpDiv)); // Append the dpDiv to the CE wrapper and for widgets simply to the body
    ...
    _doCalendarKeyDown: function _doCalendarKeyDown(event) {
      ...
      if (this._datepickerShowing()) {
        switch (event.keyCode) {
          ...
          case kc.ESCAPE:
            this._hide(this._ON_CLOSE_REASON_CANCELLED); // JET-43662 - date picker inside popup is not getting closed on esc key press
    ...
    _doMonthViewKeyDown: function _doMonthViewKeyDown(event) {
      ...
      if (this._datepickerShowing()) {
        switch (event.keyCode) {
          ...
          case kc.ESCAPE:
            this.hide(); // JET-43662 - date picker inside popup is not getting closed on esc key press
            // The ESC key is handled only when the picker is not inline. When in inline
            // mode, we need to allow the event to bubble up as we are not doing anything here.
  ...
  * @memberof! oj.ojInputTime
  ...
  oj.__registerWidget('oj.ojInputTime', $.oj.inputBase, {
    ...
    _getPrependNode: function _getPrependNode() {
      var independentPrependNode = this._IsCustomElement() ? $(this.OuterWrapper) : $('body');
      return this._isIndependentInput() ? independentPrependNode : $('.oj-popup-content', this._datePickerComp.widget._popUpDpDiv.ojPopup('widget'));
    },
    ...
    _destroyHammer: function _destroyHammer() {
      if (this._wheelGroup && this._wheelGroup.length) {
        ...
        for (var index = 0; index < children.length; index++) {
          wheel = children[index]; // This calls Wheel#destroy() and this calls off on each
    ...
    _ReleaseResources: function _ReleaseResources() {
      ...
      this._removeResizePopupBind(); // Note: We do not have a symmetrical 'createHammer' call in SetupResources.
     ...
     * @private
     */
    _removeResizePopupBind: function _removeResizePopupBind() {
      if (this._resizePopupBind) {
        window.removeEventListener('resize', this._resizePopupBind);
      }
    },
  ...
  oj.__registerWidget('oj.ojInputDateTime', $.oj.ojInputDate, {
    ...
    _ReleaseResources: function _ReleaseResources() {
      if (this._IsCustomElement()) {
        ...
        if (this._timePicker) {
          var timePickerInstance = this._timePicker.ojInputTime('instance');
