 ...
 * @ignore
 */
define(['exports', 'ojs/ojthemeutils', 'ojs/ojcore-base', 'jquery'], function (exports, ThemeUtils, oj, $) {
  'use strict';
  ...
  DomUtils.addResizeListener = function (elem, listener, collapseEventTimeout, useResizeObserver) {
    var map = DomUtils._RSZ_TRKR;

    if (useResizeObserver) {
      map = DomUtils._RSZ_TRKR_OBS; // remember that this particular listener is associated with a resize observer-based tracker

      DomUtils._OBS_BASED.set(listener, true);
    }

    var tracker = map.get(elem);
  ...
  DomUtils.removeResizeListener = function (elem, listener) {
    var useResizeObserver = DomUtils._OBS_BASED.get(listener);

    var map = useResizeObserver ? DomUtils._RSZ_TRKR_OBS : DomUtils._RSZ_TRKR;
    var tracker = map.get(elem);
  ...
  DomUtils._ResizeTracker = function (div, useResizeObserver) {
    ...
    function _resizeObserverCallback(entries) {
      ...
      var outer = entries[0].borderBoxSize; // MDN states that borderBoxSize may be either a direct entry (true for Firefox) or
      // an array of entries (true for Chrome)

      var newWidth = outer ? (outer[0] || outer).inlineSize : div.offsetWidth;
      var newHeight = outer ? (outer[0] || outer).blockSize : div.offsetHeight;
  ...
  DomUtils._RSZ_TRKR = new WeakMap(); // a weak element-observer-based-tracker map

  DomUtils._RSZ_TRKR_OBS = new WeakMap(); // a weak map with true values for observer-based resize listeners
  ...
  DomUtils._suppressNativeContextMenu(); // standard duration of a pressHold gesture.  Point of reference: default
  ...
  var isHTMLContent = DomUtils.isHTMLContent;
  var cleanHtml = DomUtils.cleanHtml;
  var isAncestor = DomUtils.isAncestor;
  var isAncestorOrSelf = DomUtils.isAncestorOrSelf;
  var addResizeListener = DomUtils.addResizeListener;
  var removeResizeListener = DomUtils.removeResizeListener;
  var fixResizeListeners = DomUtils.fixResizeListeners;
  var isMetaKeyPressed = DomUtils.isMetaKeyPressed;
  var dispatchEvent = DomUtils.dispatchEvent;
  var isValidIdentifier = DomUtils.isValidIdentifier;
  var isTouchSupported = DomUtils.isTouchSupported;
  var setInKoCleanExternal = DomUtils.setInKoCleanExternal;
  var unwrap = DomUtils.unwrap;
  var isChromeEvent = DomUtils.isChromeEvent;
  var getScrollBarWidth = DomUtils.getScrollBarWidth;
  var getReadingDirection = DomUtils.getReadingDirection;
  var setScrollLeft = DomUtils.setScrollLeft;
  var getCSSLengthAsInt = DomUtils.getCSSLengthAsInt;
  var getCSSLengthAsFloat = DomUtils.getCSSLengthAsFloat;
  var getLogicalParent = DomUtils.getLogicalParent;
  var setLogicalParent = DomUtils.setLogicalParent;
  var isLogicalAncestorOrSelf = DomUtils.isLogicalAncestorOrSelf;
  var validateURL = DomUtils.validateURL;
  var PRESS_HOLD_THRESHOLD = DomUtils.PRESS_HOLD_THRESHOLD;
  var recentTouchEnd = DomUtils.recentTouchEnd;
  var recentTouchStart = DomUtils.recentTouchStart;
  var recentPointer = DomUtils.recentPointer;
  var makeFocusable = DomUtils.makeFocusable;
