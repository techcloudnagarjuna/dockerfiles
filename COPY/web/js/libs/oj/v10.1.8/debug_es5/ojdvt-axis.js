...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var BaseAxisInfo = function BaseAxisInfo() {};

  dvt.Obj.createSubclass(BaseAxisInfo, dvt.Obj);
  /**
   * Minimum buffer for horizontal axis.
   */

  BaseAxisInfo.MINIMUM_AXIS_BUFFER = 10;
  /**
  ...
  BaseAxisInfo.prototype.Init = function (context, options, availSpace) {
    this._context = context; // Figure out the start and end coordinate of the axis
    ...
    this.GlobalMax = null;
    this.DataMin = null;
    this.DataMax = null; // The overflows at the two ends of the axis

    this.StartOverflow = 0;
    this.EndOverflow = 0; // Sets the buffers (the maximum amount the labels can go over before they overflow)

    if (options['leftBuffer'] == null) options['leftBuffer'] = Infinity;
    if (options['rightBuffer'] == null) options['rightBuffer'] = Infinity; // Store the options object
  ...
  BaseAxisInfo.prototype.getValueAt = function (coord) {
    ...
    var maxCoord = Math.max(this.StartCoord, this.EndCoord); // Return null if the coord is outside of the axis

    if (coord < minCoord || coord > maxCoord) return null;
  ...
  BaseAxisInfo.prototype.getCoordAt = function (value) {
    ...
    if (value < this.MinValue || value > this.MaxValue) return null;else return this.getUnboundedCoordAt(value);
  };
  /**
  ...
  BaseAxisInfo.prototype.getBoundedValueAt = function (coord) {
    ...
    if (coord < minCoord) coord = minCoord;else if (coord > maxCoord) coord = maxCoord;
    return this.getUnboundedValueAt(coord);
  };
  /**
  ...
  BaseAxisInfo.prototype.getBoundedCoordAt = function (value) {
    if (value == null) return null;
    if (value < this.MinValue) value = this.MinValue;else if (value >= this.MaxValue) value = this.MaxValue;
  ...
  var DataAxisInfoMixin = function DataAxisInfoMixin(context, options, availSpace) {
    /** @private @const */
    ...
    * */
    ...
    this.MixinInit = function (context, options, availSpace) {
      ...
      this.Converter = null;
      if (options['tickLabel'] != null) this.Converter = options['tickLabel']['converter'];
    ...
    this.GetUnboundedCoordAt = function (value) {
      if (value == null) return null;
      var ratio = this.LinearMaxValue == this.LinearMinValue ? 0 : (value - this.LinearMinValue) / (this.LinearMaxValue - this.LinearMinValue); // : Make sure the the ratio is not way too large so the browser does not fail to render
    ...
    this.CalcMajorMinorIncr = function (scaleUnit) {
      if (!this.MajorIncrement) {
        if (this.MajorTickCount) this.MajorIncrement = (this.LinearMaxValue - this.LinearMinValue) / this.MajorTickCount;else this.MajorIncrement = Math.max(scaleUnit, this._minMajorIncrement);
      }

      if (!this.MajorTickCount) {
        this.MajorTickCount = (this.LinearMaxValue - this.LinearMinValue) / this.MajorIncrement; // : Check if we have a floating point inaccuracy that causes the tick count to be undercalculated
        // within the allowable buffer. If so, tick count is supposed to be the rounded up integer.

        if (Math.ceil(this.MajorTickCount) - this.MajorTickCount < this.MAJOR_TICK_INCREMENT_BUFFER) this.MajorTickCount = Math.ceil(this.MajorTickCount);
      }

      if (!this.MinorTickCount) {
        if (this.MinorIncrement) this.MinorTickCount = this.MajorIncrement / this.MinorIncrement;else if (this.IsLog) this.MinorTickCount = this.MajorIncrement;else this.MinorTickCount = this.MINOR_TICK_COUNT;
      }

      if (!this.MinorIncrement) this.MinorIncrement = this.MajorIncrement / this.MinorTickCount;
    };
    /**
    ...
    this._calcAxisExtents = function () {
      var continuousExtent = this.Options['_continuousExtent'] == 'on'; // Include 0 in the axis if we're scaling from the baseline
      ...
      var scaleUnit = Math.max(this._calcAxisScale(minValue, maxValue), this._minMajorIncrement); // If there's only a single value on the axis, we need to adjust the
      // this._dataMin and this._dataMax to produce a nice looking axis with around 6 ticks.

      if (this._dataMin == this._dataMax) {
        if (this._dataMin == 0) this._dataMax += 5 * scaleUnit;else {
      ...
      if (this.LinearGlobalMin == null) {
        ...
        if (this._dataMin >= 0 && !this.IsLog) this.LinearGlobalMin = Math.max(this.LinearGlobalMin, 0);
      } // Set the default global max


      if (this.LinearGlobalMax == null) {
        if (this.MajorTickCount) {
          this.LinearGlobalMax = this.LinearGlobalMin + this.MajorTickCount * scaleUnit; // JET-28098 - wrong y2 max

          if (this.LinearGlobalMax < this._dataMax) {
            scaleUnit = Math.max(this._calcAxisScale(minValue, maxValue + scaleUnit), this._minMajorIncrement);
        ...
        if (this._dataMax <= 0) this.LinearGlobalMax = Math.min(this.LinearGlobalMax, 0);
      }
      ...
      if (this.LinearMinValue == null) this.LinearMinValue = this.LinearGlobalMin;
      if (this.LinearMaxValue == null) this.LinearMaxValue = this.LinearGlobalMax; // if data min and axis min are too close (less than 10px) in log scale, decrease the axis min by a step
      ...
      if (this.LinearMinValue != this.LinearGlobalMin || this.LinearMaxValue != this.LinearGlobalMax) scaleUnit = this._calcAxisScale(this.LinearMinValue, this.LinearMaxValue);
      if (this.LinearGlobalMin > this.LinearMinValue) this.LinearGlobalMin = this.LinearMinValue;
      if (this.LinearGlobalMax < this.LinearMaxValue) this.LinearGlobalMax = this.LinearMaxValue; // Calculate major and minor gridlines
    ...
    this._calcAxisScale = function (min, max) {
      if (this.MajorIncrement) return this.MajorIncrement;
      var spread = max - min;

      if (this.IsLog) {
        var scaleUnit = Math.floor(spread / 8) + 1; // Store the scaleUnit for aligning log axes

        if (!this.LogScaleUnit || this.LogScaleUnit < scaleUnit) this.LogScaleUnit = scaleUnit;
        return this.LogScaleUnit;
      }

      if (spread == 0) {
        if (min == 0) return 10;else return Math.pow(10, Math.floor(dvt.Math.log10(min)) - 1);
      }
      ...
      if (this.MajorTickCount) {
        ...
        if (firstDigit > 1 && firstDigit <= 1.5) firstDigit = 1.5;else if (firstDigit > 5) firstDigit = 10;else firstDigit = Math.ceil(firstDigit);
        return firstDigit * testVal;
      }

      var t = dvt.Math.log10(spread);
      var testVal = Math.pow(10, Math.ceil(t) - 2);
      var first2Digits = Math.round(spread / testVal); // Aesthetically choose a scaling factor limiting to a max number of steps

      var scaleFactor = 1;
      if (first2Digits >= 10 && first2Digits <= 14) scaleFactor = 2;else if (first2Digits >= 15 && first2Digits <= 19) scaleFactor = 3;else if (first2Digits >= 20 && first2Digits <= 24) scaleFactor = 4;else if (first2Digits >= 25 && first2Digits <= 45) scaleFactor = 5;else if (first2Digits >= 46 && first2Digits <= 80) scaleFactor = 10;else scaleFactor = 20;
    ...
    this.actualToLinear = function (value) {
      if (value == null) return null;
      if (this.IsLog) return value > 0 ? dvt.Math.log10(value) : null;
      return value;
    };
  };
  /**
  ...
  var LinearScaleAxisValueFormatter = function LinearScaleAxisValueFormatter(context, minValue, maxValue, tickStep, scale, autoPrecision, translations) {
    this.Init(context, minValue, maxValue, tickStep, scale, autoPrecision, translations);
  };

  dvt.Obj.createSubclass(LinearScaleAxisValueFormatter, dvt.Obj); // Allowed scales that can be used as formatter scale param values

  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_NONE = 'none';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_AUTO = 'auto';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_THOUSAND = 'thousand';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_MILLION = 'million';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_BILLION = 'billion';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_TRILLION = 'trillion';
  /** @const **/

  LinearScaleAxisValueFormatter.SCALE_QUADRILLION = 'quadrillion';
  /** @const **/
  ...
  LinearScaleAxisValueFormatter.SCALING_FACTOR_DIFFERENCE = 3;
  /**
  ...
  LinearScaleAxisValueFormatter.prototype.Init = function (context, minValue, maxValue, tickStep, scale, autoPrecision, translations) {
    ...
    this._translations = translations; // array of successive scale values

    this._scales = {}; // array of scale values ordered by scale factor asc

    this._scalesOrder = []; // mapping of scale factors to corresponding scale objects
  ...
  LinearScaleAxisValueFormatter.prototype.InitScales = function () {
     ...
     * @param {number} scaleFactor scale factor of corresponding scale, i.e. 'x' such that 10^x represents corresponding scale (e.g. for scale LinearScaleAxisValueFormatter.SCALE_THOUSAND x = 3)
     ...
     */
    var createScale = function createScale(scaleName, scaleFactor, scaleKey) {
      var suffix;
    ...
    var diff = LinearScaleAxisValueFormatter.SCALING_FACTOR_DIFFERENCE;
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_NONE, 0 * diff);
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_THOUSAND, 1 * diff, 'labelScalingSuffixThousand');
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_MILLION, 2 * diff, 'labelScalingSuffixMillion');
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_BILLION, 3 * diff, 'labelScalingSuffixBillion');
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_TRILLION, 4 * diff, 'labelScalingSuffixTrillion');
    createScale.call(this, LinearScaleAxisValueFormatter.SCALE_QUADRILLION, 5 * diff, 'labelScalingSuffixQuadrillion'); // sort _scalesOrder array
  ...
  LinearScaleAxisValueFormatter.prototype.InitFormatter = function (minValue, maxValue, tickStep, scale, autoPrecision) {
    var findScale = false,
        decimalPlaces,
        scaleFactor,
        useAutoPrecision = false; // if autoPrecision doesn't equal "off" (i.e. is "on", null, undefined) then auto precision should be used.
    ...
    if (useAutoPrecision === true) {
      if (tickStep == 0 && minValue == maxValue) {
        ...
        if (scaleFactorDiff <= 0) // Value is same or larger than the scale factor, ensure 4 significant digits.
          ...
          decimalPlaces = Math.max(scaleFactorDiff + 3, 0);else // Value is smaller, ensure enough decimals to show 1 significant digit
  ...
  LinearScaleAxisValueFormatter.prototype._getScaleFactor = function (scaleName) {
    ...
    if (!scaleName) scaleName = LinearScaleAxisValueFormatter.SCALE_AUTO;
    var scaleFactor,
        scale = this._scales[scaleName];
  ...
  LinearScaleAxisValueFormatter.prototype.format = function (value, converter) {
    ...
    if (typeof parsed === 'number') {
      ...
      var defaultConverter = this._context.getNumberConverter(scaleConverterOptions); // Formatting for scale
      ...
      var formattedScaledNumber = Number(formattedScaleParts[1]) / scale * parsed; // Formatting for scaled number
  ...
  LinearScaleAxisValueFormatter.prototype._formatFraction = function (value) {
    var formatted = value.toString(); // Don't format scientific notation (e.g. '1e-7')

    if (formatted.indexOf('e') != -1) return formatted;
    var decimalSep = '.'; // TODO: probably need some translation here?
  ...
  LinearScaleAxisValueFormatter.prototype._getPowerOfTen = function (value) {
    ...
    var power = 0; // Check for degenerate and zero values
