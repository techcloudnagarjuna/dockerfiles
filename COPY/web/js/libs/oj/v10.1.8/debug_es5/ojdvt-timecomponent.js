 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojtimeaxis-toolkit'], function (exports, dvt, ojtimeaxisToolkit) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2022, Oracle and/or its affiliates.
  ...
  var TimeComponent = function TimeComponent(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(TimeComponent, dvt.BaseComponent);
  ...
  TimeComponent.prototype.SetOptions = function (options) {
    ...
    this.Options = this.Defaults.calcOptions(options); // If viewportStart/End is invalid within start and end,
  ...
  TimeComponent.prototype.prepareViewportLength = function () {
    this.setRelativeStartPos(0); // Check the initial viewportStart/End.
  ...
  TimeComponent.prototype.HandleMouseWheel = function (event) {
    ...
    if (this.hasValidOptions()) {
      ...
      if (wheelEvent.wheelDeltaX != null) event.wheelDeltaX = wheelEvent.wheelDeltaX / TimeComponent.WHEEL_UNITS_PER_LINE; // number of lines scrolled per mouse wheel click
      else if (wheelEvent.deltaX != null) {
        if (wheelEvent.deltaMode == wheelEvent.DOM_DELTA_LINE) event.wheelDeltaX = -wheelEvent.deltaX;else if (wheelEvent.deltaMode == wheelEvent.DOM_DELTA_PIXEL) event.wheelDeltaX = -wheelEvent.deltaX / TimeComponent.SCROLL_LINE_HEIGHT; // number of lines scrolled per mouse wheel click
      }
  ...
  TimeComponent.prototype.handleZoom = function (zoomIn) {
    if (!zoomIn) this.zoomBy(TimeComponent.ZOOM_BY_VALUE);else this.zoomBy(1 / TimeComponent.ZOOM_BY_VALUE);
  };
  ...
  var TimeComponentEventManager = function TimeComponentEventManager(comp) {
  ...
  dvt.Obj.createSubclass(TimeComponentEventManager, dvt.EventManager); // TODO: shouldn't this be in the toolkit?
  ...
  TimeComponentEventManager.prototype.addListeners = function (displayable) {
    ...
    if (!dvt.Agent.isTouchDevice()) {
      if (dvt.Agent.browser === 'firefox') displayable.addEvtListener(TimeComponentEventManager.GECKO_MOUSEWHEEL, this.OnMouseWheel, false, this);else displayable.addEvtListener(dvt.MouseEvent.MOUSEWHEEL, this.OnMouseWheel, false, this);
    }
  };
  /**
   * @override
  ...
  TimeComponentEventManager.prototype.RemoveListeners = function (displayable) {
    ...
    if (!dvt.Agent.isTouchDevice()) {
      if (dvt.Agent.browser === 'firefox') displayable.removeEvtListener(TimeComponentEventManager.GECKO_MOUSEWHEEL, this.OnMouseWheel, false, this);else displayable.removeEvtListener(dvt.MouseEvent.MOUSEWHEEL, this.OnMouseWheel, false, this);
    }
  };
  /**
   * @override
  ...
  TimeComponentEventManager.prototype._onMouseDragStart = function (event) {
    if (event.button != dvt.MouseEvent.RIGHT_CLICK_BUTTON) {
      ...
      var dragHandler = this._getDragHandler(relPos); // TODO: Consider encapsulate panning logic into a dragHandler, so that only the else block-like logic is needed.
  ...
  TimeComponentEventManager.prototype.OnMouseMove = function (event) {
    TimeComponentEventManager.superclass.OnMouseMove.call(this, event); // Update the cursor
  ...
  TimeComponentEventManager.prototype._onTouchDragStart = function (event) {
    ...
    if (touches.length == 1) {
      ...
      var dragHandler = this._getDragHandler(); // only pan if inside chart/graphical area
  ...
  TimeComponentEventManager.prototype.ProcessMarqueeEvent = function (event) {// Subclasses should implement
  };
  /**
   * Cancels marquee.
  ...
  var TimeComponentKeyboardHandler = function TimeComponentKeyboardHandler(manager) {
    this.Init(manager);
  };

  dvt.Obj.createSubclass(TimeComponentKeyboardHandler, dvt.KeyboardHandler);
  /**
   * @override
   */
