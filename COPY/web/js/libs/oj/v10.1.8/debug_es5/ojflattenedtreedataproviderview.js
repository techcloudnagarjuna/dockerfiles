 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojset', 'ojs/ojeventtarget', 'ojs/ojkeyset', 'ojs/ojobservable', 'ojs/ojmap'], function (oj, ojSet, ojeventtarget, KeySet, ojobservable, ojMap) {
  'use strict';
  ...
  var FlattenedTreeDataProviderView = /*#__PURE__*/function () {
    function FlattenedTreeDataProviderView(dataProvider, options) {
      ...
      this.DataProviderRefreshEventDetail = /*#__PURE__*/function () {
        function _class14(disregardAfterKey) {
      ...
      this.DataProviderOperationEventDetail = /*#__PURE__*/function () {
        function _class15(_parent, keys, metadata, data, indexes) {
      ...
      this.DataProviderAddOperationEventDetail = /*#__PURE__*/function () {
        function _class16(_parent, keys, afterKeys, addBeforeKeys, metadata, data, indexes) {
    ...
    _createClass(FlattenedTreeDataProviderView, [{
      key: "_handleUnderlyingMutation",
      value: function _handleUnderlyingMutation(mutationEventDetail) {
        var self = this;
      ...
      key: "_handleUnderlyingRefresh",
      value: function _handleUnderlyingRefresh() {
        this._clearCache();
      ...
      value: function fetchFirst(params) {
        ...
        var newIterator = new self.AsyncIterable(self, new self.AsyncIterator(self, function () {
          return function () {
            ...
            var updatedParams = new self.FetchByOffsetParameters(self, currentOffset, self._fetchSize, sortCriteria, filterCriterion, fetchAttributes);
      ...
      value: function _checkCacheByOffset(params, expandKey) {
        if (params[this._SIZE] === -1 && this._done === true) {
          return true;
        } else if (expandKey != null) {
          if (this._getLocalDescendentCount(expandKey) >= params[this._SIZE] && params[this._SIZE] !== -1) {
            return true;
          }
        } else if (this._cache.length >= params[this._OFFSET] + params[this._SIZE] && params[this._SIZE] !== -1) {
          return true;
        }
      ...
      value: function _fetchChildrenByOffsetFromAncestors(params, dataprovider, parentKey, finalParams, expandKey) {
        var self = this;
        ...
        var handleFetchFromAncestors = function handleFetchFromAncestors(lastParentKey, result) {
          ...
          if (self._checkCacheByOffset(finalParams, expandKey) || result[self._DONE] && lastParentKey === null || lastEntry === null) {
          ...
          var childrenPromise = self._fetchChildrenByOffsetFromDataProvider(newParams, childDataProvider, lastEntryParentKey, finalParams, expandKey);

          return childrenPromise.then(handleFetchFromAncestors.bind(self, lastEntryParentKey));
        };

        return self._fetchChildrenByOffsetFromDataProvider(params, dataprovider, parentKey, finalParams, expandKey).then(handleFetchFromAncestors.bind(self, parentKey)).then(self._getFetchByOffsetResultsFromCache.bind(self, finalParams));
      }
    }, {
      key: "_fetchChildrenByOffsetFromDataProvider",
      value: function _fetchChildrenByOffsetFromDataProvider(params, dataprovider, parentKey, finalParams, expandKey) {
        var self = this;
        ...
        var handleNextItemInResults = function handleNextItemInResults(result) {
          ...
          if (self._isExpanded(updatedItem.metadata.key)) {
            ...
            if (childDataProvider != null) {
              ...
              var childrenPromise = self._fetchChildrenByOffsetFromDataProvider(newParams, childDataProvider, updatedItem.metadata.key, finalParams, expandKey);
      ...
      value: function setExpanded(keySet) {
        ...
        var completionPromise = new Promise(function (resolve) {
          expandPromise.then(function (expandReturnObject) {
            var operationAddEventDetail = expandReturnObject.operationAddEventDetail;
            var fetchedCountMap = expandReturnObject.fetchedCountMap;
            var disregardAfterKey = null;
            var previousCacheIndex = null;

            if (self._fetchSize !== -1) {
              fetchedCountMap.forEach(function (value, key) {
                var fetchedCount = value;

                if (fetchedCount >= self._fetchSize) {
                  var currentCacheIndex = self._getItemIndexByKey(key);
            ...
            if (previousCacheIndex !== null) {
              var removedItems = self._cache.splice(previousCacheIndex + 1, self._cache.length);

              removedItems.forEach(function () {
            ...
            if (disregardAfterKey !== null) {
              var refreshEvent = new oj.DataProviderRefreshEvent(new self.DataProviderRefreshEventDetail(disregardAfterKey));
      ...
      value: function _expand(keys) {
        ...
        keys.forEach(function (key) {
          var params = new self.FetchByOffsetParameters(self, 0, self._fetchSize, self._currentSortCriteria, self._currentFilterCriteria, null);
