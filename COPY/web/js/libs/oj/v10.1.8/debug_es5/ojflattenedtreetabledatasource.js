...
define(['ojs/ojcore', 'jquery', 'ojs/ojdatasource-common'], function (oj, $) {
  ...
  oj.FlattenedTreeTableDataSource = function (data, options) {
    ...
    this._startIndex = 0;
    this._nodeSetList = [];
    this._hasMore = true; // override the fetchSize with -1 if not already specified.
    ...
    var self = this; // override the insert/removeRows function
    ...
    this._data.insertRows = function (insertAtIndex, insertAtKey, nodeSet) {
      ...
      for (var i = 0; i < nodeSet.getCount(); i++) {
        ...
        self._nodeSetList[rowIdx].startIndex = insertAtIndex; // update the startIndex of the shifted rows
    ...
    this._data.removeRows = function (rowKeys) {
      ...
      for (var i = rowKeys.length - 1; i >= 0; i--) {
        var rowIdx = rowKeys[i].index; // just create a dummy row for deletion

        rowArray.push('');
        keyArray.push(rowKeys[i].key);
        indexArray.push(rowIdx);

        self._nodeSetList.splice(rowIdx, 1); // update the startIndex of the shifted rows
    ...
    if (_options != null && (_options.startFetch === 'enabled' || _options.startFetch == null) || _options == null) {
  ...
  oj.FlattenedTreeTableDataSource.prototype.get = function (id, options) {
    ...
    if (row != null) {
      result = {
        data: this._wrapWritableValue(row),
        key: id,
        index: rowIdx
      };
    }
  ...
  oj.FlattenedTreeTableDataSource.prototype._fetchInternal = function (_options) {
    ...
    this._startIndex = options.startIndex == null ? this._startIndex : options.startIndex; // if pageSize is not specified then fetch everything
    ...
    return new Promise(function (resolve, reject) {
      self._data.fetchRows(rangeOption, {
        success: function success(nodeSet) {
          self._handleFetchRowsSuccess(nodeSet); // eslint-disable-next-line no-param-reassign


          options.refresh = true;

          var _endIndex = oj.FlattenedTreeTableDataSource._getEndIndex(self._rows, self._startIndex, self._pageSize);
        ...
        error: function error(_error) {
          self._endFetch(options, null, _error);
  ...
  oj.FlattenedTreeTableDataSource._getRowArray = function (nodeSet, rowSet, rows, startIndex) {
    for (var i = 0; i < nodeSet.getCount(); i++) {
      var row = nodeSet.getData(nodeSet.getStart() + i); // eslint-disable-next-line no-param-reassign

      rows.data[startIndex + i] = row; // eslint-disable-next-line no-param-reassign

      rows.keys[startIndex + i] = nodeSet.getMetadata(nodeSet.getStart() + i).key; // eslint-disable-next-line no-param-reassign
