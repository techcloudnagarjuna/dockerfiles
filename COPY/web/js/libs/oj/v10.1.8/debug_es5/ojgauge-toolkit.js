 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojdvt-axis'], function (exports, dvt, ojdvtAxis) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2022, Oracle and/or its affiliates.
  ...
  var DvtGaugeUtils = {
     ...
     */
    hasLabel: function hasLabel(options) {
  ...
  DvtGaugeRenderer.renderEmptyText = function (gauge, container, availSpace) {
    ...
    gauge.renderEmptyText(container, emptyTextStr, new dvt.Rectangle(availSpace.x, availSpace.y, availSpace.w, availSpace.h), gauge.getEventManager(), metricLabelStyle);
  };
  /**
  ...
  DvtGaugeRenderer._formatMetricLabelValue = function (value, gauge, converter, scaling, autoPrecision, isPercent) {
    ...
    if (converter && converter['format']) output = formatter.format(value, converter);else if (isPercent) {
  ...
  DvtGaugeStyleUtils.getBorderColor = function (gauge) {
    ...
    var thresholdIndex = DvtGaugeDataUtils.getValueThresholdIndex(gauge);
    var threshold = DvtGaugeDataUtils.getThreshold(gauge, thresholdIndex);
    if (threshold && threshold['borderColor'] && (!(gauge.getGaugeType() === 'statusMeter') || gauge.getGaugeType() === 'statusMeter' && options['thresholdDisplay'] == 'onIndicator')) return threshold['borderColor'];
  ...
  DvtGaugeStyleUtils.getPlotAreaBorderColor = function (gauge) {
    ...
    if (gauge.getGaugeType() === 'statusMeter' && options['orientation'] != 'circular' && borderColor == null) {
      return '#D6DFE6';
    }
  ...
  DvtGauge.prototype.PostRender = function (options, container) {
    ...
    if (!this._bStaticRendering && !this.Options['readOnly']) {
      ...
      if (dvt.Agent.isTouchDevice()) {
        ...
        if (value != tooltip) tooltip = value + dvt.AriaUtils.ARIA_LABEL_DESC_DELIMITER + tooltip;
      }
    ...
    if (!this._bStaticRendering) {
      ...
      if (this.IsParentRoot()) {
        ...
        if (!this.IsInteractive()) {
          ...
          this.getCtx().setAriaLabel(dvt.ResourceUtils.format(translations.labelAndValue, [translations.labelDataVisualization, dvt.Displayable.generateAriaLabel(dvt.AriaUtils.processAriaLabel(this.GetComponentDescription()), tooltip ? [tooltip] : null)]));
        } else {
          this.getCtx().setAriaRole('application');
          this.getCtx().setAriaLabel(dvt.ResourceUtils.format(translations.labelAndValue, [translations.labelDataVisualization, dvt.AriaUtils.processAriaLabel(this.GetComponentDescription())]));
        }
  ...
  var DialGauge = function DialGauge() {};

  dvt.Obj.createSubclass(DialGauge, DvtGauge);
  /**
  ...
  DialGauge.prototype.Init = function (context, callback, callbackObj) {
    DialGauge.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  DvtLedGaugeRenderer._renderShape = function (gauge, container, bounds) {
    ...
    if (rotation && (type == 'arrow' || type == 'triangle' || shape instanceof dvt.Path && type != 'human' && type != "star")) shape = DvtLedGaugeRenderer._rotate(gauge, container, shape, bounds); // Add the shape to the container
  ...
  var LedGauge = function LedGauge() {};

  dvt.Obj.createSubclass(LedGauge, DvtGauge);
  /**
  ...
  LedGauge.prototype.Init = function (context, callback, callbackObj, bStaticRendering) {
    LedGauge.superclass.Init.call(this, context, callback, callbackObj, bStaticRendering); // Create the defaults object
  ...
  LedGauge.prototype.SetOptions = function (options) {
    ...
    LedGauge.superclass.SetOptions.call(this, this.Defaults.calcOptions(options)); // animationOnDisplay="auto" will do "zoom"
  ...
  DvtRatingGaugeEventManager.prototype.OnMouseMove = function (event) {
    ...
    if (this._gauge.IsInteractive() && !this.IsMouseEditing && this._gauge.getOptions()['value'] !== this._gauge.GetValueAt(coords.x, coords.y)) {
      this.IsMouseEditing = true;
    }

    if (!isDisabled) DvtRatingGaugeEventManager.superclass.OnMouseMove.call(this, event);
  };
  /**
   * @override
   */
  ...
  DvtRatingGaugeRenderer._createShape = function (context, x, y, width, height, stateOptions, gaugeOptions) {
    ...
    if (stateOptions['source']) {
      ...
      if (gaugeOptions['disabled'] || dvt.Agent.isHighContrast()) {
        var className = "oj-rating-gauge-".concat(stateOptions.state === 'unselected' ? 'unselected' : 'selected').concat(gaugeOptions['disabled'] ? '-disabled' : '');

        if (gaugeOptions['disabled']) {
          stateOptions.svgClassName = className;
          stateOptions.svgStyle = null;
        } else {
          stateOptions.svgClassName = "".concat(stateOptions.svgClassName || '', " ").concat(className).trim();
        }
  ...
  var RatingGauge = function RatingGauge() {};

  dvt.Obj.createSubclass(RatingGauge, DvtGauge);
  /**
  ...
  RatingGauge.prototype.Init = function (context, callback, callbackObj) {
    RatingGauge.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  var DvtStatusMeterGaugeUtils = {
     ...
     * @private
     */
    calcPointOnArc: function calcPointOnArc(bounds, radius, angle) {
     ...
     */
    createCircularPathCmd: function createCircularPathCmd(bounds, startAngle, angleExtent, innerRadius, outerRadius) {
      var cmd;
      var p1, p2, p3, p4;
      ...
      if (angleExtent < dvt.Math.TWO_PI) {
        ...
        p4 = DvtStatusMeterGaugeUtils.calcPointOnArc(bounds, innerRadius, startAngle); // Create the command and feed it into the path
        ...
        p4 = DvtStatusMeterGaugeUtils.calcPointOnArc(bounds, innerRadius, startAngle + angleExtent / 2); // Create the command and return it
  ...
  DvtStatusMeterGaugeRenderer._drawCircularArc = function (gauge, container, bounds, startAngle, angleExtent, innerRadius, outerRadius, color, isPlotArea, plotAreaBorderColor) {
    ...
    if (isPlotArea) {
      shape = new dvt.Path(context, DvtStatusMeterGaugeUtils.createCircularPathCmd(bounds, startAngle, angleExtent, innerRadius, outerRadius));
    } else {
  ...
  var StatusMeterGauge = function StatusMeterGauge() {};

  dvt.Obj.createSubclass(StatusMeterGauge, DvtGauge);
  /**
  ...
  StatusMeterGauge.prototype.Init = function (context, callback, callbackObj) {
    StatusMeterGauge.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
