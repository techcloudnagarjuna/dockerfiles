...
define(['exports', 'ojs/ojcore', 'knockout', 'ojs/ojmodel'], function (exports, oj, ko, Model) {
  ...
  var KnockoutUtils = function KnockoutUtils() {};
  ...
  KnockoutUtils.map = function (m, callback, array) {
    ...
    if (m instanceof Model.Collection) {
      ...
      koObject = array ? ko.observableArray(prealloc) : prealloc; // Need access to original wrapped collection
      ...
      var updateCollection = function updateCollection(changes) {
        try {
          if (!koObject[KnockoutUtils.underUpdateProp]) {
            koObject[KnockoutUtils.collUpdatingProp] = true; // Process change information
            ...
            if (m.comparator) {
              koObject[KnockoutUtils.underUpdateProp] = true; // These could have been resorted--need to alter the observable array
      ...
      if (array && koObject.subscribe) {
        ...
        koObject[KnockoutUtils.subscriptionProp] = koObject.subscribe(updateCollection, null, 'arrayChange');
      }

      var updateObservableArrayRemove = function updateObservableArrayRemove(model, collection, options) {
        try {
      ...
      var updateObservableArrayAdd = function updateObservableArrayAdd(model, collection, options) {
        try {
          if (koObject[KnockoutUtils.collUpdatingProp]) {
            return;
          }
            ...
            if (_index !== undefined && _index > -1) {
              ...
              if (options.fillIn) {
                ...
                for (var ii = currLen; ii < _index; ii++) {
                  koObject.splice(ii, 0, KnockoutUtils.map(collection._atInternal(ii, null, true, false), callback));
      ...
      var updateObservableArrayReset = function updateObservableArrayReset(collection) {
        try {
          if (koObject[KnockoutUtils.collUpdatingProp]) {
            return;
          }
          ...
          if (collection instanceof Model.Collection) {
            koObject[KnockoutUtils.underUpdateProp] = true; // koObject.splice(0, koObject().length);
            ...
            if (ko.isObservable(koObject)) {
              ...
              if (koObject[KnockoutUtils.updatingCollectionFunc]) {
                koObject.subscribe(koObject[KnockoutUtils.updatingCollectionFunc], null, 'arrayChange');
              }
            } else {
              koObject = [];
            }
      ...
      var updateObservableArraySort = function updateObservableArraySort(collection) {
        try {
          if (koObject[KnockoutUtils.collUpdatingProp]) {
            return;
          }
          ...
          if (collection instanceof Model.Collection) {
            koObject[KnockoutUtils.underUpdateProp] = true; // Redo the knockout observable array
      ...
      m.OnInternal(Model.Events.EventType.ADD, updateObservableArrayAdd, undefined, undefined, true);
      m.OnInternal(Model.Events.EventType.REMOVE, updateObservableArrayRemove, undefined, undefined, true);
      m.OnInternal(Model.Events.EventType.RESET, updateObservableArrayReset, undefined, undefined, true);
      m.OnInternal(Model.Events.EventType.SORT, updateObservableArraySort, undefined, undefined, true);
    } else {
      if (m === undefined) {
        return undefined;
      }
      ...
      for (var j = 0; j < props.length; j++) {
        ...
        updateModel._prop = prop; // Subscribe to any changes pushed by Knockout into the observable
      ...
      var updateObservable = function updateObservable(model) {
        try {
          koObject[KnockoutUtils.underUpdateProp] = true;
      ...
      m.OnInternal(Model.Events.EventType.CHANGE, updateObservable, undefined, undefined, true); // Need access to original wrapped model

      KnockoutUtils._storeOriginalObject(koObject, m); // Activate user callback if specified, to allow user computed observables, etc.
  ...
  KnockoutUtils._callSort = function (a, b, comparator, collection, caller) {
    return Model.Collection.SortFunc(KnockoutUtils._getModel(a), KnockoutUtils._getModel(b), comparator, collection, caller);
