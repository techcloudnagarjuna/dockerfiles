 ...
 * @ignore
 */
define(['ojs/ojcore', 'jquery', 'ojs/ojcontext', 'ojs/ojmessaging', 'ojs/ojknockout', 'knockout'], function (oj, $, Context, Message, ojknockout, ko) {
  'use strict';
  ...
  *     }
  ...
  var InvalidComponentTracker = function InvalidComponentTracker() {
    this.Init();
  };

  oj._registerLegacyNamespaceProp('InvalidComponentTracker', InvalidComponentTracker); // Subclass from oj.Object


  oj.Object.createSubclass(InvalidComponentTracker, oj.Object, 'oj.InvalidComponentTracker'); // DOCLETS for public properties
  ...
  InvalidComponentTracker.prototype.Init = function () {
    InvalidComponentTracker.superclass.Init.call(this); // INTERNAL PROPERTIES
    ...
    this._tracked = []; // tracks invalid components showing messages. indices correspond to this_tracked.
    // e.g., [true, false, false, false, true]

    this._invalid = []; // tracks invalid components hiding messages. indices correspond to this_tracked.
    // e.g., [false, true, false, false, false]

    this._invalidHidden = []; // PUBLIC PROPERTIES
    // Whether there is at least one component
    // (tracked by this object) that is invalid and is currently showing messages.

    this.invalidShown = false; // Whether there is at least one component that is invalid with deferred messages,
  ...
  InvalidComponentTracker.prototype.focusOnFirstInvalid = function () {
    ...
    var self = this;
    var updateCounter = this._updateCounter; // we don't always have a element associated, so this is on the page level context
    ...
    setTimeout(function () {
      ...
      firstInvalid = updateCounter === self._updateCounter ? firstInvalid || self._getFirstInvalidComponent() : self._getFirstInvalidComponent();
  ...
  InvalidComponentTracker.prototype._getFirstInvalidComponent = function () {
    ...
    var len = this._invalid.length; // locate first invalid component and set focus on it
    ...
    invalidComponents.sort(function (a, b) {
      ...
      var elementB = b.element; // If elementA precedes elementB in dom order, return -1
  ...
  InvalidComponentTracker.prototype._remove = function (component) {
    var trackedIndex = -1;
    var mutated = false; // locate the index in tracked, for the component that was updated
    ...
    if (trackedIndex >= 0) {
      this._tracked.splice(trackedIndex, 1); // stop tracking them in the internal arrays
  ...
  InvalidComponentTracker.prototype._update = function (component, element, option, value) {
    ...
    var mutated = true;
    var result;
    var trackedIndex = -1; // locate the index in tracked, for the component that was updated.
    ...
    switch (option) {
      ...
      case InvalidComponentTracker._OPTION_MESSAGES_HIDDEN:
        ...
        if (value) {
          ...
          if (trackedIndex < 0) {
            trackedIndex = this._tracked.push({
              component: component,
              element: element
            }) - 1; // adds the trackedIndex/result to this._invalid and this._invalidHidden boolean arrays,
          ...
          if (!compValid) {
            if (InvalidComponentTracker._hasInvalidMessages(value)) {
              ...
              if (option === InvalidComponentTracker._OPTION_MESSAGES_SHOWN) {
                ...
                // disabled.
                var isDisabled = component.call(component, 'option', InvalidComponentTracker._OPTION_DISABLED);
                var isReadOnly = component.call(component, 'option', InvalidComponentTracker._OPTION_READONLY);
          ...
          this._updateInvalidProperties(); // update properties
      ...
      case InvalidComponentTracker._OPTION_READONLY:
        ...
        if (value) {
          mutated = this._updateInvalidTracker(InvalidComponentTracker._OPTION_MESSAGES_SHOWN, trackedIndex || 0, false);
          mutated = this._updateInvalidTracker(InvalidComponentTracker._OPTION_MESSAGES_HIDDEN, trackedIndex || 0, false) || mutated;
  ...
  InvalidComponentTracker.prototype._initializeInvalidTrackers = function (trackedIndex, result) {
    if (this._invalid[trackedIndex] === undefined) {
      this._updateInvalidTracker(InvalidComponentTracker._OPTION_MESSAGES_SHOWN, trackedIndex, result);
    }

    if (this._invalidHidden[trackedIndex] === undefined) {
      this._updateInvalidTracker(InvalidComponentTracker._OPTION_MESSAGES_HIDDEN, trackedIndex, result);
    }
  };
  ...
  var ValueBinding = function ValueBinding() {};
  ...
  *     }
  ...
  ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER = 'invalidComponentTracker'; // A listener is added for this event to listen to changes to the 'messagesHidden' or
  // 'messagesShown' options. The listener updates the InvalidComponentTracker.

  ValueBinding._EVENT_OPTIONCHANGE = 'ojoptionchange'; // Options we are interested in listening to changes for.

  ValueBinding._OPTION_MESSAGES_SHOWN = 'messagesShown';
  ValueBinding._OPTION_MESSAGES_HIDDEN = 'messagesHidden'; // options that are managed primarily to detect changes for tracker to be notified.

  ValueBinding._OPTION_DISABLED = 'disabled';
  ValueBinding._OPTION_READONLY = 'readOnly'; // callback called when managed attribute is being updated
  ...
  ValueBinding._afterCreate = function (property, element, component, valueAccessor) {
    ...
    if (property === ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER) {
      ...
      if (isICTOptionSet) {
        ...
        ValueBinding._registerInvalidComponentTrackerWriteback(property, optionsSet, element, component);
      }
    }
  ...
  ValueBinding._beforeDestroy = function (property, element, component, valueAccessor) {
    ...
    if (property === ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER) {
      ...
      if (ictObs && ko.isWriteableObservable(ictObs)) {
        icTracker = ictObs.peek(); // remove component from tracker
  ...
  ValueBinding._updateInvalidComponentTracker = function (event) {
    ...
    if (option === ValueBinding._OPTION_MESSAGES_SHOWN || option === ValueBinding._OPTION_MESSAGES_HIDDEN) {
      if (ictObs && ko.isWriteableObservable(ictObs)) {
  ...
  ValueBinding._registerInvalidComponentTrackerWriteback = function (property, options, element, component) {
    var ictObs = options[property];
    var icTracker;
    var jElem = $(element); // Create new intsance of InvalidComponentTracker if the observable is not set.

    if (ko.isObservable(ictObs)) {
      icTracker = ictObs.peek(); // push new instance of oj.InvalidComponentTracker onto observable if none present.
      ...
      throw new Error('Binding attribute ' + ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER + ' should be bound to a ko observable.');
    ...
    if (ko.isWriteableObservable(ictObs)) {
      ...
      var messagesShown = component.call(component, 'option', ValueBinding._OPTION_MESSAGES_SHOWN); // EditableValue's messagesHidden option is an Array of messages currently hidden on component

      var messagesHidden = component.call(component, 'option', ValueBinding._OPTION_MESSAGES_HIDDEN);

      icTracker._update.call(icTracker, component, element, ValueBinding._OPTION_MESSAGES_SHOWN, messagesShown);

      icTracker._update.call(icTracker, component, element, ValueBinding._OPTION_MESSAGES_HIDDEN, messagesHidden);
    ...
    jElem.on(ValueBinding._EVENT_OPTIONCHANGE, eventData, ValueBinding._updateInvalidComponentTracker);
  };
  /**
  ...
  oj.ComponentBinding.getDefaultInstance().setupManagedAttributes({
    for: 'editableValue',
    attributes: [ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER, ValueBinding._OPTION_DISABLED, ValueBinding._OPTION_READONLY],
