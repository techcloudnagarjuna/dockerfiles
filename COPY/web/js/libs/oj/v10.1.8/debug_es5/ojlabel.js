 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojjquery-hammer', 'ojs/ojcomponentcore', 'ojs/ojpopup', 'ojs/ojcore-base', 'jquery', 'hammerjs', 'ojs/ojlogger', 'ojs/ojcontext', 'ojs/ojdomutils'], function (ojcore, ojjqueryHammer, ojcomponentcore, ojpopup, oj, $, hammerjs, ojlogger, Context, DomUtils) {
  'use strict';
    ...
    oj.__registerWidget('oj.ojLabel', $.oj.baseComponent, {
      ...
      */
      ...
      _attachHelpDefToIconAnchor: function _attachHelpDefToIconAnchor() {
        ...
        this._openPopupForHelpDefCallbackListener = function (event) {
          if ($helpDefPopupDiv == null) {
            // create popup's div
            $helpDefPopupDiv = self._createHelpDefPopupDiv(); // Once we have created the popup, we can now set an event
            // listener on it.

            $helpDefPopupDiv.on('mouseleave' + self._helpDefPopupNamespace, self._mouseleaveClosePopupForHelpDefListener);
            $helpDefPopupDiv.on('mouseenter' + self._helpDefPopupNamespace, self._clearTimeoutOnPopupMouseEnterListener);
            position = {
              my: 'start bottom',
              at: 'end top',
        ...
        this._mouseleaveClosePopupForHelpDefListener = function (e) {
          ...
          if (self._helpDefPopupDivId != null) {
            var targetIsHelpIcon = e.currentTarget === $helpIcon[0];
            var targetIsPopupDiv = e.currentTarget === $helpDefPopupDiv[0];
      ...
      _addShowHelpDefinitionEventHandlers: function _addShowHelpDefinitionEventHandlers($helpIcon) {
        ...
        $helpIcon.on('focusin' + this._helpDefPopupNamespace + ' mouseenter' + this._helpDefPopupNamespace, this._openPopupForHelpDefCallbackListener); // close the popup when you leave the help icon

        $helpIcon.on('mouseleave' + this._helpDefPopupNamespace, this._mouseleaveClosePopupForHelpDefListener);
        ...
        if (this._bTouchSupported) {
          ...
          if (this.options.help.source) {
            hammerOptions = {
              recognizers: [[hammerjs.Press, {
                time: DomUtils.PRESS_HOLD_THRESHOLD
              }]]
            };
            ...
            hammerOptions = {
              recognizers: [[hammerjs.Tap], [hammerjs.Press, {
                time: DomUtils.PRESS_HOLD_THRESHOLD
              }]]
            };
       ...
       * @private
       */
      _resolveHelpDefClosePopupTimoutBC: function _resolveHelpDefClosePopupTimoutBC() {
        this.closePopupTimer = null; // this busy context was set during the setTimeout popup close call
      ...
      _handleOpenHelpDefPopup: function _handleOpenHelpDefPopup(event, helpDefPopupDiv, helpIcon) {
        ...
        if (this._bTouchSupported) {
          ...
          if (event.type === 'press' || event.type === 'tap' || !DomUtils.recentTouchStart() && (event.type === 'focusin' || event.type === 'mouseenter')) {
      ...
      _removeHelpDefIconEventListeners: function _removeHelpDefIconEventListeners(helpIcon) {
        ...
        if (this._helpDefPopupDivId != null) {
          var $helpDefPopupDiv = $(document.getElementById(this._helpDefPopupDivId));
