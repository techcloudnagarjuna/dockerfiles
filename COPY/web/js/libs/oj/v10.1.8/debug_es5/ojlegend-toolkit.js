...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var DvtLegendUtils = {
     ...
     * @return {String}
     */
    getItemCategory: function getItemCategory(item, legend) {
      var category = null;
     ...
     */
    isCategoryHidden: function isCategoryHidden(category, legend) {
     ...
     */
    isSectionCollapsed: function isSectionCollapsed(section, legend) {
  ...
  DvtLegendObjPeer.prototype.Init = function (legend, displayables, item, tooltip, datatip, drillable) {
    ...
    this._hoverBorderRadius = parseInt(legend.getOptions()['_hoverBorderRadius']); // Apply the cursor for drilling if specified
  ...
  DvtLegendObjPeer.prototype.showKeyboardFocusEffect = function () {
    ...
    if (this._displayables[0]) {
      if (this._displayables[0] instanceof dvt.IconButton) this._displayables[0].showKeyboardFocusEffect();else this._displayables[0].setSolidStroke(dvt.Agent.getFocusColor());
    }
  };
  /**
   * @override
  ...
  DvtLegendObjPeer.prototype.hideKeyboardFocusEffect = function () {
    ...
    if (this._displayables[0]) {
      if (this._displayables[0] instanceof dvt.IconButton) this._displayables[0].hideKeyboardFocusEffect();else this._displayables[0].setStroke(null);
    }
  };
  /**
   * @override
  ...
  DvtLegendObjPeer.prototype.showHoverEffect = function () {
    if (this._displayables[0]) {
      if (this._displayables[0] instanceof dvt.Rect) this._displayables[0].setClassName("oj-legend-hover");
  ...
  DvtLegendEventManager.prototype.OnMouseOver = function (event) {
    ...
    var obj = this.GetLogicalObject(event.target);
    if (!obj) return; // Accessibility Support
  ...
  DvtLegendEventManager.prototype.ProcessRolloverEvent = function (event, obj, bOver) {
    ...
    if (options['hoverBehavior'] === 'none' && (options['hideAndShowBehavior'] === 'none' || options['hideAndShowBehavior'] === 'off')) return; // Do not fire rollover event for section collapse/expand button
    ...
    if (options['hoverBehavior'] !== 'none') {
      ...
      options['highlightedCategories'] = bOver && categories ? categories.slice() : null; // Fire the event to the rollover handler, who will fire to the component callback.
  ...
  DvtLegendRenderer._createButton = function (context, legend, item, resources, prefix, x, y, tooltip, id, callback, callbackObj) {
    ...
    var button = new dvt.IconButton(context, 'borderless', {
      style: iconStyle,
      size: DvtLegendRenderer._BUTTON_SIZE
    }, null, id, callback, callbackObj);
  ...
  DvtLegendRenderer._renderVerticalSection = function (legend, container, section, availSpace, rowHeight, id, minimizeNumRows) {
    ...
    if (!hasItems && !hasSections || DvtLegendUtils.isSectionCollapsed(section, legend)) return sectionDim; // Title+button should always be on its own row
  ...
  DvtLegendRenderer._createLegendSymbol = function (legend, x, y, rowHeight, item, i) {
    ...
    if (symbolType == 'line') {
      ...
      if (!DvtLegendUtils.isCategoryHidden(DvtLegendUtils.getItemCategory(item, legend), legend)) symbol.addChild(DvtLegendRenderer._createMarker(legend, cx, cy, symbolWidth * DvtLegendRenderer._LINE_MARKER_SIZE_FACTOR, symbolHeight * DvtLegendRenderer._LINE_MARKER_SIZE_FACTOR, item));
  ...
  dvt.Obj.createSubclass(Legend, dvt.BaseComponent);
  /**
  ...
  Legend.prototype.__registerObject = function (peer) {
    if (peer.getDisplayables()[0] instanceof dvt.IconButton) this._navigablePeers.push(peer); // peer is navigable if associated with button
    else {
