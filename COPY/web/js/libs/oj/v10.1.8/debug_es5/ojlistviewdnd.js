 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore', 'jquery', 'ojs/ojlogger', 'ojdnd', 'ojs/ojlistview'], function (exports, oj, $, Logger, ojdnd, ojlistview) {
  'use strict';
  ...
  var ListViewDndContext = function ListViewDndContext(listview) {
    this.listview = listview;
  };

  oj._registerLegacyNamespaceProp('ListViewDndContext', ListViewDndContext); // Subclass from oj.Object
  ...
  ListViewDndContext.prototype._getSelectedItems = function (excludeContextMenuItem) {
    ...
    if (this.listview._isSelectionEnabled()) {
      ...
      for (var i = 0; i < selection.length; i++) {
        var elem = this.listview.FindElementByKey(selection[i]); // make sure item is focusable also
  ...
  ListViewDndContext.prototype.setSelectionDraggable = function () {
    var elems = []; // clear previous draggable selections

    this._unsetSelectionDraggable(); // make current selection draggable
    ...
    for (var i = 0; i < selection.length; i++) {
      ...
      if (elem != null && !this.listview.SkipFocus($(elem))) {
        elems.push(elem);
        $(elem).addClass('oj-draggable'); // for touch draggable needs to be set prior to touch interaction
      ...
      if (callback && typeof callback === 'function') {
        try {
          if (this.listview.ojContext._IsCustomElement()) {
            ...
            callback(event.originalEvent, ui); // preventDefault is called on the original event
  ...
  ListViewDndContext.prototype._calculateOffset = function (elem) {
    ...
    var offset = parseFloat(style.marginLeft) + parseFloat(style.marginRight) + parseFloat(style.paddingLeft) + parseFloat(style.paddingRight) + parseFloat(style.borderLeftWidth) + parseFloat(style.borderRightWidth);
    return offset;
  };

  ListViewDndContext.prototype._calculateAffordanceOffset = function (item, affordance) {
    if (this.m_cachedOffset == null) {
      this.m_cachedOffset = Math.max(0, affordance.offsetLeft - item.offsetLeft) + affordance.offsetWidth / 2;
  ...
  ListViewDndContext.prototype.SetDragItemImage = function (nativeEvent, items) {
    ...
    if (items.length > 1) {
      ...
      var i;
      var minTop = Number.MAX_VALUE; // need to figure out which is the top item
  ...
  ListViewDndContext.prototype._handleDragStart = function (event) {
    ...
    if (options != null || this.IsItemReOrdering()) {
      ...
      if (items.length > 0) {
        ...
        if (ui) {
          var ret = this._invokeDndCallback('drag', 'dragStart', event, ui); // dragStart is optional
  ...
  ListViewDndContext.prototype._handleDragEnd = function (event) {
    ...
    this._cleanupDropTarget(); // clean up drag image


    this._destroyDragImage(); // remove style classes from selected items that were draggable


    this._unsetSelectionDraggable(); // invoke callback


    this._invokeDndCallback('drag', 'dragEnd', event); // save dragItems for potential drop within same listview (reordering)


    this.m_itemsDragged = this.m_dragItems; // reset drag variables (except this.m_dragItems)

    this.m_dragImage = null;
    this.m_currentDragItem = null;
    this.m_dragItems = null; // focus should now be at document body, restore focus
  ...
  ListViewDndContext.prototype._createDropTarget = function (item) {
    ...
    if (type === 'line') {
      ...
      var top = elem.offsetTop + elem.offsetHeight - this._getDropIndicatorHeight(); // if it's the last item and gridlines are visible, need to make an adjustment
  ...
  ListViewDndContext.prototype._handleDragEnter = function (event) {
    ...
    if (item != null && item.length > 0) {
      var returnValue = this._invokeDropCallback('dragEnter', event, {
        item: item.get(0)
      });
  ...
  ListViewDndContext.prototype._setAccInfo = function (item, position) {
    ...
    var msg = this.listview.ojContext.getTranslatedString(key, {
      item: label
    });
  ...
  ListViewDndContext.prototype._checkFirstItemDropTarget = function (item, dropTarget, target) {
    if (target.classList.contains('oj-listview-first-drop-target')) {
      ...
      this.m_dropPosition = 'before'; // eslint-disable-next-line no-param-reassign
    ...
    if (this._getDropIndicator() === 'line') {
      if (item.previousElementSibling === null) {
        ...
        if (this.m_firstDropTarget == null) {
          ...
          this.m_dropPosition = 'after'; // eslint-disable-next-line no-param-reassign
  ...
  ListViewDndContext.prototype._isDropOnDragItem = function (event, item) {
    ...
    return this._matchDragDataType(event) && this.m_dragItems != null && this.m_dragItems.length >= 0 && this.m_dragItems[0].key === item.get(0).key;
  };
  /**
  ...
  ListViewDndContext.prototype._handleDragOver = function (event) {
    var item;
    var dropTarget;
    var returnValue; // do any neccessary group item

    this._adjustGroupItemStyle(); // checks whether this is initially dragging over the drag item


    if (this.m_dragItems != null && $(this.m_dragItems[0]).css('display') !== 'none') {
      // take out the current drag item and create drop target
      item = $(this.m_dragItems[0]); // this will check for matching data types

      returnValue = this._invokeDropCallback('dragOver', event, {
        item: item.get(0)
      ...
      if (returnValue === -1 && this.IsItemReOrdering() && this.isDndInProgress() || returnValue === false || event.isDefaultPrevented()) {
      ...
      if (item != null && item.length > 0) {
        ...
        if (this._isDropOnDragItem(event, item)) {
          ...
          returnValue = this._invokeDndCallback('drop', 'dragOver', event, {
            item: item.get(0)
          });
        ...
        if (returnValue === -1 && this.IsItemReOrdering() && this.isDndInProgress() || returnValue === false || event.isDefaultPrevented()) {
          ...
          if (item.hasClass(this.listview.getItemStyleClass())) {
            this._cleanupGroupItem(); // if a drop target has not been created for this item
            ...
            this._cleanupDropTarget(); // for group item
          ...
          item.addClass('oj-invalid-drop'); // remove any drop target placeholder
  ...
  ListViewDndContext.prototype._isDndEventInElement = function (event, element) {
    ...
    return domEvent.clientX >= rect.left && domEvent.clientX < rect.right && domEvent.clientY >= rect.top && domEvent.clientY < rect.bottom;
  };
  /**
  ...
  ListViewDndContext.prototype._handleDragLeave = function (event) {
    ...
    if (item != null && item.length > 0) {
      ...
      returnValue = this._invokeDropCallback('dragLeave', event, {
        item: item.get(0)
  ...
  ListViewDndContext.prototype._isEmptyItem = function (item) {
    return item.hasClass(this.listview.getEmptyTextStyleClass()) || item.hasClass('oj-listview-no-data-item');
  };
  /**
   * Drop event handler
   * @param {Event} event  jQuery event object
  ...
  ListViewDndContext.prototype._handleDrop = function (event) {
    ...
    var source = event.originalEvent.dataTransfer.getData(this.GetDragSourceType()); // invoke callback
    ...
    this._destroyDragImage(); // fire drop event


    var returnValue = this._invokeDropCallback('drop', event, ui); // fire reorder event
    ...
    if (ui.reorder) {
      ...
      this.listview.Trigger('reorder', event, this.CreateReorderPayload(ui.items, ui.position, ui.item)); // the drop should be complete regardless the value of callback
  ...
  ListViewDndContext.prototype.prepareContextMenu = function (contextMenu) {
    var self = this; // only add context menu if item reordering is enabled
  ...
  ListViewDndContext.prototype._getDndContextMenuItemSelector = function () {
    var self = this;
    var query = '';
    var commands = ['cut', 'copy', 'paste', 'paste-before', 'paste-after', 'pasteBefore', 'pasteAfter'];
  ...
  ListViewDndContext.prototype._getCommands = function (contextMenu, callback) {
    ...
    listItems.each(function () {
      ...
      if (anchor.length === 0) {
        ...
        command = $(this).attr('data-oj-command'); // mapping for paste
  ...
  ListViewDndContext.prototype._buildContextMenuLabel = function (command, useOjOption) {
    ...
    var textNode = document.createTextNode(this.listview.ojContext.getTranslatedString(key)); // for custom elements, no <a> tag is required
  ...
  ListViewDndContext.prototype._handleCut = function (event) {
    ...
    var items = this.GetCutItems(event); // focus should be moved back to listview after context menu is closed

    this.listview.ojContext.element.focus();
    $(items).addClass(this.GetCutStyleClass());
    this.m_clipboard = items; // fire cut event

    this.listview.Trigger('cut', event, {
      items: items
    });
  };
  /**
  ...
  ListViewDndContext.prototype._handleCopy = function (event) {
    ...
    this.listview.Trigger('copy', event, {
      items: items
    });
  };
  /**
  ...
  ListViewDndContext.prototype._handlePaste = function (event, item, position) {
    // fire paste event
    this.listview.Trigger('paste', event, {
      item: item.get(0)
    }); // restore cut item style

    $(this.m_clipboard).removeClass(this.GetCutStyleClass()); // fire reorder event

    this.listview.Trigger('reorder', event, this.CreateReorderPayload(this.m_clipboard, position,
    /** @type {Element} */
    item.get(0))); // clear the clipboard
  ...
  ListViewDndContext.prototype._handleContextMenuSelect = function (event, ui) {
    ...
    switch (item.attr('data-oj-command')) {
      ...
      case ListViewDndContext.PASTE_COMMAND:
        this._handlePaste(event, this.m_contextMenuItem, 'inside'); // cleanup
      ...
      case ListViewDndContext.PASTE_BEFORE_COMMAND:
        this._handlePaste(event, this.m_contextMenuItem, 'before'); // cleanup
      ...
      case ListViewDndContext.PASTE_AFTER_COMMAND:
        this._handlePaste(event, this.m_contextMenuItem, 'after'); // cleanup
  ...
  ListViewDndContext.prototype._handleContextMenuBeforeOpen = function (event, ui) {
    var menuContainer = $(event.target); // disable all menu items first, needs to be done even if there's no default menu items since
    ...
    if (item.children().first().hasClass(this.listview.getGroupItemStyleClass())) {
      ...
      this._appendToMenuContainer(menuContainer, 'copy'); // check if cut action was performed
  ...
  ListViewDndContext.prototype.HandleKeyDown = function (event) {
    if (event.ctrlKey || event.metaKey) {
      var keyCode = event.keyCode; // quickly short circuit it if it's not one of the supported keys

      if (keyCode === ListViewDndContext.X_KEY || keyCode === ListViewDndContext.C_KEY || ListViewDndContext.V_KEY) {
