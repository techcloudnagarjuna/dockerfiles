 ...
 * @ignore
 */
define(['require', 'exports', 'ojs/ojcore', 'ojs/ojknockout', 'ojs/ojbutton', 'ojs/ojjquery-hammer', 'ojs/ojcore-base', 'jquery', 'knockout', 'ojs/ojanimation', 'ojs/ojdomutils', 'ojs/ojcomponentcore', 'ojs/ojcomposite', 'ojs/ojcontext', 'hammerjs', 'ojs/ojlogger', 'ojs/ojthemeutils', 'ojs/ojtranslation'], function (require, exports, ojcore, ojknockout, ojbutton, ojjqueryHammer, oj, $, ko, AnimationUtils, DomUtils, Components, Composite, Context, Hammer, Logger, ThemeUtils, Translations) {
  'use strict';
  ...
  function MessageViewModel(context) {
    ...
    this.handleKeydown = this._handleKeydown.bind(this); // Store a reference to the slotCounts which will be later used to determine if message detail is present

    this._slotCounts = context.slotCounts; // Store a reference to the properties for any later use

    this._properties = context.properties;

    this._createObservables(); // Update the severity class on the root element. Since this is the initial
  ...
  MessageViewModel.prototype._propertyChanged = function (detail) {
    ...
    if (subPropertyChanged(detail, 'message', 'severity')) {
      ...
      var previousSeverity = this.computedSeverity();
      this.computedSeverity(this._computeSeverity());
      this.computedCategory(this._computeCategory());
      this.computedIconClass(this._computeIconClass());
      this.computedMessageContainerSelectors(this._computeMessageContainerSelectors()); // Update the severity style classes on the root element.
      // This method relies on the observables, so call this method after the observables
      // are updated and represent the current severity.
  ...
  MessageViewModel.prototype._registerSwipeHandler = function () {
    if (DomUtils.isTouchSupported() && this._computeCloseAffordance() === 'defaults') {
      ...
      var swipeDirections = DomUtils.getReadingDirection() === 'rtl' ? 'swipeleft swipeup' : 'swiperight swipeup';
  ...
  MessageViewModel.prototype._getConverterPromise = function (timestamp) {
    var dateTimeConverterLoadPromise = new Promise(function (resolve, reject) {
      require(['ojs/ojconverter-datetime'], function (m) {
  ...
  MessageViewModel.prototype._computeCategory = function () {
    ...
    var translatedCategory = translations && translations.categories ? translations.categories[severity] : undefined;
  ...
  MessageViewModel.prototype._computeMessageContainerSelectors = function () {
    var messageContainerSelectors = ['oj-message-container']; // Add no icon selector if no icons are specified, add relevent selectors
  ...
  MessageViewModel.prototype._updateSeverityStyleClass = function (previousSeverity) {
    if (previousSeverity != null) {
      ...
      var previousStyleClass = MessageViewModel._getStyleClassForSeverity(previousSeverity);

      this._composite.classList.remove(previousStyleClass);
    } // Add the class for current severity


    var styleClass = MessageViewModel._getStyleClassForSeverity(this.computedSeverity());
