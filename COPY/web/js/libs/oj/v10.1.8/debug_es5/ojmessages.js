 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'knockout', 'ojs/ojanimation', 'ojs/ojcontext', 'ojs/ojthemeutils', 'ojs/ojtranslation', 'ojs/ojcomposite', 'ojs/ojcomponentcore', 'ojs/ojlogger', 'ojs/ojdomutils', 'ojs/ojknockout', 'ojs/ojpopupcore', 'ojs/ojmessage', 'ojs/ojdataprovider', 'ojs/ojcustomelement-utils'], function (oj, $, ko, AnimationUtils, Context, ThemeUtils, Translations, Composite, Components, Logger, DomUtils, ojknockout, ojpopupcore, ojmessage, ojdataprovider, ojcustomelementUtils) {
  'use strict';
  ...
  **/
  ...
  function MessagesViewModel(context) {
    ...
    this._updateLandmark(); //  When slideIn/out animation is used, like the defaults in notification messages, the
  ...
  MessagesViewModel.prototype._bindingsApplied = function () {
    ...
    messagesContainerDiv.addEventListener('ojFocus', this._navigationEventListener.bind(this), false); // If 'messages' property is specified, we will not have inlined oj-message children.
    ...
    if (inlinedMessageChildren.length !== 0) {
      ...
      for (var i = 0; i < inlinedMessageChildren.length; i++) {
        this._animateMessageAction(inlinedMessageChildren[i].firstChild, 'open', this._resolveInlinedMessagesOpenBusyState(inlinedMessageChildren[i].getProperty('message'), inlinedMessageChildren.length));
      }
    }
  };

  MessagesViewModel.prototype._resolveInlinedMessagesOpenBusyState = function (message, numInlinedChildren) {
    this._updateLiveRegionAndContainer(message);

    this._numInlinedChildrenAnimated = this._numInlinedChildrenAnimated ? this._numInlinedChildrenAnimated + 1 : 1;

    if (this._numInlinedChildrenAnimated === numInlinedChildren) {
      this._numInlinedChildrenAnimated = 0; // Now that we are done open animating all inlined children, resolve the busy context
  ...
  MessagesViewModel.prototype._disconnected = function () {
    MessagesViewModel.NAVIGATION_TRACKER.remove(this._messagesContainerId); // detaching an open message overlay results in implicit dismissal
  ...
  MessagesViewModel.prototype._updateLiveRegionAndContainer = function (message) {
    var translations = Translations.getComponentTranslations('oj-ojMessage').categories; // oj.Message has 'fatal' severity which is no different from 'error', oj-message does not support
  ...
  MessagesViewModel.prototype._getTranslationsDefault = function (key, options) {
    ...
    var keySegments = key.split('.'); // key is a dot separated qualifier, break it so we can use in [] notation to access sub-props
  ...
  MessagesViewModel.prototype._handleClose = function (event) {
    ...
    var nextFocusElement; // If the event was triggered from the close icon, the originalEvent will be populated
    ...
    $(closeMessageElement).remove(); // if we do not have any "default" slot children, then the message being closed is the last one,
  ...
  MessagesViewModel.prototype._animateMessageAction = function (messageElement, action, endCallback) {
    ...
    var options = this._getThemedAnimateOptions(display, action); // oj-messages doesn't publish animateStart/animateEnd so use the simpler syntax for now and allow
    // the event to bubble.
  ...
  MessagesViewModel.prototype._getThemedAnimateOptions = function (display, action) {
    ...
    if (themedDefaults && themedDefaults[display] && themedDefaults[display].animation && themedDefaults[display].animation[action]) {
  ...
  MessagesViewModel.prototype._computeContainerSelectors = function () {
    ...
    var composite = $(this._composite); // something about the layout changed, take this opp. to update classes on the root element
  ...
  MessagesViewModel.prototype._getDefaultSlotMessageElements = function () {
    ...
    var messageElements = []; // we just need to deal with "default" slot
    ...
    for (var i = 0; body && i < body.length; i++) {
      ...
      if (body[i].nodeName !== 'OJ-MESSAGE') {
        ...
        if (body[i].nodeName !== 'OJ-BIND-IF') {
          Logger.error(["JET oj-messages id='", toSelector(this._composite), "': can contain only oj-message children in its default slot. ", "Found a child element id='", toSelector(body[i]), "'."].join(''));
        }
      } else {
  ...
  MessagesViewModel.prototype._openOverlay = function () {
    ...
    this._overlayEventsCallback = MessagesViewModel._overlayEventsListener.bind(this, composite); // snap on tab key handler
  ...
  MessagesViewModel.prototype._closeOverlay = function () {
    ...
    var psOptions = {};
    psOptions[oj.PopupService.OPTION.POPUP] = composite;
    oj.PopupService.getInstance().close(psOptions); // remove tab key handler
  ...
  MessagesViewModel.prototype._isOverlayOpen = function () {
    ...
    return status === oj.ZOrderUtils.STATUS.OPENING || status === oj.ZOrderUtils.STATUS.OPEN || status === oj.ZOrderUtils.STATUS.CLOSING;
  };
  ...
  MessagesViewModel.prototype._refresh = function () {
    if (this._isOverlayOpen()) {
      var messagesBoundingRect = this._composite.getBoundingClientRect(); // We will reposition only if messages are fully in viewport. Otherwise overflowing messages
      //  cannot be scrolled with page scroll since our popup repositions for scroll events.


      if (messagesBoundingRect.height < document.documentElement.clientHeight && messagesBoundingRect.width < document.documentElement.clientWidth) {
  ...
  MessagesViewModel.NAVIGATION_TRACKER = {
     ...
     * @param {string} id
     */
    togglePreviousFocus: function togglePreviousFocus(id) {
     ...
     */
    _addPriorFocusCache: function _addPriorFocusCache(id, target) {
    ...
    _start: function _start(id) {
      ...
      $(messagesContainerDiv).data('oj_messages_nmtl', messageContainerCallback); // return if already listening
     ...
     * @return {number}
     */
    _indexOfFocusWithin: function _indexOfFocusWithin(target) {
     ...
     * @param {Event} event
     */
    _documentListener: function _documentListener(event) {
      if (event.defaultPrevented) {
        return;
      ...
      if (event.type === 'keydown' && (event.keyCode === 117 || event.key === 'F6') && messagesContainerIds.length > 0) {
        ...
        s = messagesContainerIds.length - 1; // target the most recently disclosed

        for (var i = s; i > -1; i--) {
          var messagesContainerDiv = document.getElementById(messagesContainerIds[i]); // if the container doesn't exist or is not visible continue to the next most recently used

          if (messagesContainerDiv && $(messagesContainerDiv).is(':visible') && oj.ZOrderUtils.isAboveTopModalLayer(messagesContainerDiv)) {
            ...
            event.preventDefault(); // captrue what had focus prior to pressing F6

            this._addPriorFocusCache(messagesContainerIds[i], event.target);

            element.focus(); // Use a custom message to signal navigation to the messages container. The event doesn't
            // bubble. The event is used to announce navigation instructions.

            var customEvent = new CustomEvent('ojFocus', {
              bubbles: false,
              cancelable: true
            });
     ...
     * @param {Event} event
     */
    _messageContainerListener: function _messageContainerListener(id, event) {
      ...
      if (event.type === 'focus' || event.type === 'click') {
        ...
        if (prevActiveElement && messagesContainerDiv && !DomUtils.isAncestorOrSelf(messagesContainerDiv, prevActiveElement)) {
  ...
  LiveRegion.prototype.destroy = function () {
    var liveRegion = $(document.getElementById(LiveRegion._LIVE_REGION_ID)); // remove all the messages per messages container
