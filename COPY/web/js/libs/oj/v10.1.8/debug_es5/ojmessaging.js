...
define(['ojs/ojcore-base', 'jquery'], function (oj, $) {
  ...
  var Message = function Message(summary, detail, severity) {
  ...
  Message.getSeverityLevel = function (severity) {
    ...
    if (_severity) {
      ...
      } else if (typeof _severity === 'number' && (_severity < Message.SEVERITY_LEVEL.CONFIRMATION || _severity > Message.SEVERITY_LEVEL.FATAL)) {
        _severity = Message.SEVERITY_LEVEL.ERROR;
      }
    }
  ...
  Message._LEVEL_TO_TYPE = ['none', // this can never be set
  Message.SEVERITY_TYPE.CONFIRMATION, Message.SEVERITY_TYPE.INFO, Message.SEVERITY_TYPE.WARNING, Message.SEVERITY_TYPE.ERROR, Message.SEVERITY_TYPE.FATAL];
  /**
   * @license
   * Copyright (c) 2014, 2022, Oracle and/or its affiliates.
   * The Universal Permissive License (UPL), Version 1.0
  ...
  var ComponentMessage = function ComponentMessage(summary, detail, severity, options) {
  ...
  ComponentMessage.prototype.canDisplay = function () {
    return !(this._options && this._options.display ? this._options.display === ComponentMessage.DISPLAY.HIDDEN : false);
  };
  /**
   *
