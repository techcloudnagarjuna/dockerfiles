 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'hammerjs', 'ojs/ojcontext', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojlogger', 'ojs/ojdomutils', 'ojs/ojfocusutils', 'ojs/ojjquery-hammer', 'ojs/ojpopupcore'], function (exports, oj, $, Hammer, Context, ThemeUtils, Components, Logger, DomUtils, FocusUtils, ojjqueryHammer, ojpopupcore) {
  'use strict';
  ...
  *       <td>$(".selector").on("ojopen", function(event, offcanvas) {});</td>
  ...
  OffcanvasUtils._getDisplayMode = function (offcanvas) {
    ...
    if (displayMode !== OffcanvasUtils.DISPLAY_MODE_OVERLAY && displayMode !== OffcanvasUtils.DISPLAY_MODE_PUSH && displayMode !== OffcanvasUtils.DISPLAY_MODE_REFLOW) {
  ...
  OffcanvasUtils._getAnimateWrapper = function (offcanvas) {
    ...
    if (OffcanvasUtils._noInnerWrapper(offcanvas) || offcanvas[OffcanvasUtils.DISPLAY_MODE_KEY] === OffcanvasUtils.DISPLAY_MODE_OVERLAY) {
      return drawer;
    }
  ...
  OffcanvasUtils._toggleClass = function (offcanvas, wrapper, isOpen) {
    ...
    var wrapperClass = displayMode === OffcanvasUtils.DISPLAY_MODE_OVERLAY ? OffcanvasUtils.TRANSITION_SELECTOR + ' oj-offcanvas-overlay' : OffcanvasUtils.TRANSITION_SELECTOR; // toggle offcanvas and inner wrapper classes
  ...
  OffcanvasUtils._registerCloseHandler = function (_offcanvas) {
    ...
    if (OffcanvasUtils._isAutoDismiss(offcanvas)) {
      var drawer = OffcanvasUtils._getDrawer(offcanvas); // save dismisHandler
  ...
  OffcanvasUtils._registerSwipeHandler = function (_offcanvas) {
    if (DomUtils.isTouchSupported()) {
      ...
      if (edge === OffcanvasUtils.EDGE_START && !OffcanvasUtils._isRTL() || edge === OffcanvasUtils.EDGE_END && OffcanvasUtils._isRTL()) {
        options = {
      ...
      } else if (edge === OffcanvasUtils.EDGE_START && OffcanvasUtils._isRTL() || edge === OffcanvasUtils.EDGE_END && !OffcanvasUtils._isRTL()) {
        options = {
          recognizers: [[Hammer.Swipe, {
  ...
  OffcanvasUtils._noInnerWrapper = function (offcanvas) {
    return offcanvas[OffcanvasUtils.CONTENT_KEY] || OffcanvasUtils._isFixed(OffcanvasUtils._getDrawer(offcanvas)) || OffcanvasUtils._isReflow(offcanvas);
  };
  /**
   * @memberof OffcanvasUtils
   * @private
  ...
  OffcanvasUtils._afterCloseHandler = function (offcanvas) {
    var wrapper = OffcanvasUtils._getAnimateWrapper(offcanvas); // bail if pan to reveal is in progress
    ...
    var isReflow = OffcanvasUtils._isReflow(offcanvas); // validate offcanvas
    ...
    if (isReflow) {
      drawer.removeClass(OffcanvasUtils.OPEN_SELECTOR + ' ' + OffcanvasUtils.REFLOW_TRANSITION_SELECTOR);
    } else {
  ...
  OffcanvasUtils.setupResponsive = function (offcanvas) {
    ...
    if (mqs !== null) {
      ...
      OffcanvasUtils._setVisible(selector, query.matches, edge); // keep the listener in the offcanvas jquery data
  ...
  OffcanvasUtils._openPush = function (offcanvas, resolve, reject, edge) {
    ...
    drawer.addClass(OffcanvasUtils.OPEN_SELECTOR); // set translationX or Y
    ...
    window.setTimeout(function () {
      ...
      if (edge === OffcanvasUtils.EDGE_START || edge === OffcanvasUtils.EDGE_END) {
        ...
        OffcanvasUtils._setTransform(drawer, OffcanvasUtils._getTranslationY2(size, edge === OffcanvasUtils.EDGE_TOP));
      ...
      window.setTimeout(function () {
        ...
        OffcanvasUtils._setAnimateClass(offcanvas, drawer, $main, 'translate3d(0, 0, 0)', translation);

        OffcanvasUtils._toggleOuterWrapper(offcanvas, drawer, false); // add transition end listener
    ...
    OffcanvasUtils._applyModality(offcanvas, drawer); //  - opening offcanvas automatically scrolls to the top
    //  - perf: fif jank: nav drawer and list view items
  ...
  OffcanvasUtils._openOverlay = function (offcanvas, resolve, reject, edge) {
    var drawer = OffcanvasUtils._getDrawer(offcanvas); // Before animation, remove display:none and add transition class
    ...
    OffcanvasUtils._onTransitionEnd(drawer, function () {
      // After animation, remove transition class
      drawer.removeClass(OffcanvasUtils.TRANSITION_SELECTOR); //  - opening offcanvas automatically scrolls to the top
      //  - perf: fif jank: nav drawer and list view items
      // Moving the focus before animation works fine with the "start" and "top" drawers, but not
      // with the "end" and "bottom" drawers. (There may be a browser bug causing problems)
  ...
  OffcanvasUtils._closePush = function (offcanvas, resolve, reject, drawer, animation) {
    var $main = $(offcanvas[OffcanvasUtils.CONTENT_KEY]); // since drawer and main are animated seperately,
    // only resolve true when both transitions are ended
    ...
    OffcanvasUtils._toggleOuterWrapper(offcanvas, drawer, false); // dim glassPane
    ...
    if (animation) {
      ...
      $main.addClass(OffcanvasUtils.TRANSITION_SELECTOR);
      drawer.addClass(OffcanvasUtils.TRANSITION_SELECTOR); // add transition end listener
  ...
  OffcanvasUtils._openOldDrawer = function (offcanvas, resolve, reject, edge, displayMode) {
    ...
    if (edge === OffcanvasUtils.EDGE_START || edge === OffcanvasUtils.EDGE_END) {
      ...
      size = size === undefined ? drawer.outerWidth(true) + 'px' : size; // don't set transform for OffcanvasUtils.DISPLAY_MODE_OVERLAY
      ...
      size = size === undefined ? drawer.outerHeight(true) + 'px' : size; // don't set transform for OffcanvasUtils.DISPLAY_MODE_OVERLAY
    ...
    OffcanvasUtils._onTransitionEnd(wrapper, function () {
      // After animation, remove transition class
      wrapper.removeClass(OffcanvasUtils.TRANSITION_SELECTOR); //  - opening offcanvas automatically scrolls to the top
      //  - perf: fif jank: nav drawer and list view items
      // Moving the focus before animation works fine with the "start" and "top" drawers, but not
      // with the "end" and "bottom" drawers. (There may be a browser bug causing problems)
  ...
  OffcanvasUtils._closeOldDrawer = function (offcanvas, resolve, reject, drawer, animation) {
    ...
    if (animation) {
      // Before animation, add transition class
      wrapper.addClass(OffcanvasUtils.TRANSITION_SELECTOR); // add transition end listener
  ...
  OffcanvasUtils.open = function (offcanvas) {
    ...
    var promise = new Promise(function (resolve, reject) {
      ...
      var edge = OffcanvasUtils._saveEdge(offcanvas); // fire before open event
      ...
      var isReflow = OffcanvasUtils._isReflow(offcanvas); // only support horizontal offcanvas for reflow
      ...
      $.data(drawer[0], OffcanvasUtils._DATA_OFFCANVAS_KEY, myOffcanvas); // throw an error if CONTENT_KEY is specified and the html markup contains an inner wrapper.
      ...
      if (offcanvas[OffcanvasUtils.CONTENT_KEY]) {
        ...
        resolveBusyState = busyContext.addBusyState({
          description: "The offcanvas selector ='" + offcanvas[OffcanvasUtils.SELECTOR_KEY] + "' doing the open animation."
        });

        if (isReflow) {//        OffcanvasUtils._openReflow(myOffcanvas, resolve, reject, edge);
    ...
    if (!veto) {
      ...
      if (nOffcanvas) {
        nOffcanvas[OffcanvasUtils.OPEN_PROMISE_KEY] = promise; // notify subtree
  ...
  OffcanvasUtils.close = function (offcanvas) {
    return OffcanvasUtils._close(offcanvas[OffcanvasUtils.SELECTOR_KEY], offcanvas[OffcanvasUtils.ANIMATE_KEY] === undefined);
  };
  /**
   * @memberof OffcanvasUtils
   * @private
  ...
  OffcanvasUtils._close = function (selector, animation) {
    ...
    var offcanvas = $.data(drawer[0], OffcanvasUtils._DATA_OFFCANVAS_KEY); // if we are in the middle of closing, then return the previous saved promise
    ...
    var promise = new Promise(function (resolve, reject) {
      if (OffcanvasUtils._isOpen(drawer)) {
        ...
        if (animation) {
          var busyContext = Context.getContext(drawer[0]).getBusyContext();
          resolveBusyState = busyContext.addBusyState({
            description: "The offcanvas selector ='" + offcanvas[OffcanvasUtils.SELECTOR_KEY] + "' doing the close animation."
          });
        }
    ...
    if (!veto) {
      ...
      if (offcanvas) {
        offcanvas[OffcanvasUtils.CLOSE_PROMISE_KEY] = promise; // notify subtree
  ...
  OffcanvasUtils._applyModality = function (_offcanvas, drawer) {
    ...
    if (OffcanvasUtils._isModal(offcanvas)) {
      ...
      offcanvas[OffcanvasUtils.GLASS_PANE_KEY] = OffcanvasUtils._addGlassPane(drawer); // bring the drawer <div> to the front
      // to keep this order:  mainContent, glassPane, drawer

      OffcanvasUtils._swapOrder(offcanvas, drawer); //  - acc: talkback reports on elements on background page whilst navdrawer open
  ...
  OffcanvasUtils._removeModality = function (offcanvas) {
    if (OffcanvasUtils._isModal(offcanvas)) {
      offcanvas[OffcanvasUtils.GLASS_PANE_KEY].remove(); // restore the order

      OffcanvasUtils._restoreOrder(offcanvas); //  - acc: talkback reports on elements on background page whilst navdrawer open
  ...
  OffcanvasUtils.setupPanToReveal = function (_offcanvas) {
    ...
    outerWrapper = OffcanvasUtils._getOuterWrapper(drawer); // use hammer for swipe
    ...
    $(outerWrapper).ojHammer(mOptions).on('panstart', function (event) {
      ...
      if (!evt.isDefaultPrevented()) {
        ...
        OffcanvasUtils._toggleClass(offcanvas, wrapper, true); // stop touch event from bubbling to prevent for example pull to refresh from happening
      ...
      if (direction === 'start' && delta > 0 && !isRTL || direction === 'start' && delta < 0 && isRTL || direction === 'end' && delta < 0 && !isRTL || direction === 'end' && delta > 0 && isRTL) {
      ...
      drawer.css('width', delta); // don't do css transition animation while panning
      ...
      _listener = function listener() {
        ...
        wrapper.removeClass(OffcanvasUtils.TRANSITION_SELECTOR); // remove handler
  ...
  OffcanvasUtils._animateWrapperAndDrawer = function (wrapper, drawer, edge, size, offcanvas) {
    ...
    wrapper.removeClass(OffcanvasUtils.TRANSITION_SELECTOR); // ideal ms per frame
    ...
    _func = function func() {
      ...
      if (current === final) {
        window.cancelAnimationFrame(reqId);
        wrapper.addClass(OffcanvasUtils.TRANSITION_SELECTOR); // fire after completely open
  ...
  OffcanvasUtils.tearDownPanToReveal = function (offcanvas) {
    ...
    var outerWrapper = OffcanvasUtils._getOuterWrapper(drawer); // remove all listeners


    $(outerWrapper).off('panstart panmove panend');
  };

  var close = OffcanvasUtils.close;
  var open = OffcanvasUtils.open;
  var setupResponsive = OffcanvasUtils.setupResponsive;
  var setupPanToReveal = OffcanvasUtils.setupPanToReveal;
  var tearDownResponsive = OffcanvasUtils.tearDownResponsive;
  var tearDownPanToReveal = OffcanvasUtils.tearDownPanToReveal;
  var toggle = OffcanvasUtils.toggle;
  var _getOuterWrapper = OffcanvasUtils._getOuterWrapper;
