 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojdatasource-common', 'ojs/ojpagingmodel'], function (oj, jquery, ojdatasourceCommon, ojpagingmodel) {
  'use strict';
  ...
  var PagingTableDataSource = function PagingTableDataSource(dataSource, options) {
    // Initialize
    ...
    this.Init(); // PagingTableDataSource doesn't need its own copy of sortCriteria and can
  ...
  oj._registerLegacyNamespaceProp('PagingTableDataSource', PagingTableDataSource); // Subclass from oj.DataSource
  ...
  PagingTableDataSource.prototype.setPage = function (value, options) {
    ...
    options.pageSize = this._pageSize; // eslint-disable-next-line no-param-reassign
  ...
  PagingTableDataSource.prototype.on = function (eventType, eventHandler) {
    ...
    if (eventType === oj.TableDataSource.EventType.SYNC) {
    ...
    } else if (eventType === oj.TableDataSource.EventType.ADD || eventType === oj.TableDataSource.EventType.REMOVE || eventType === oj.TableDataSource.EventType.CHANGE) {
      ev = function ev(event) {
      ...
      dataSource.on(eventType, ev);
    } else if (eventType === oj.TableDataSource.EventType.REFRESH || eventType === oj.TableDataSource.EventType.RESET) {
      ev = function ev(event) {
        self._startIndex = 0;
        eventHandler(event);
      };
      ...
      dataSource.on(eventType, ev);
    } else if (eventType === oj.PagingModel.EventType.PAGE || eventType === oj.PagingModel.EventType.BEFOREPAGE || eventType === oj.PagingModel.EventType.PAGECOUNT) {
  ...
  PagingTableDataSource.prototype.off = function (eventType, eventHandler) {
    if (eventType === oj.PagingModel.EventType.PAGE || eventType === oj.PagingModel.EventType.PAGECOUNT) {
    ...
    if (this._dataSourceWrappedEventHandlers != null) {
      ...
      for (i = 0; i < dataSourceWrappedEventHandlersCount; i++) {
        if (this._dataSourceWrappedEventHandlers[i].eventType === eventType && this._dataSourceWrappedEventHandlers[i].eventHandler === eventHandler) {
