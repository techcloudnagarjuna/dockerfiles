 ...
 * @ignore
 */
define(['touchr', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojpulltorefresh', 'ojs/ojlogger'], function (touchr, oj, $, Context, ojpulltorefresh, ojlogger) {
  'use strict';
    ...
    */
    ...
    oj.__registerWidget('oj.ojRefresher', $.oj.baseComponent, {
      ...
      _ReleaseResources: function _ReleaseResources() {
        ...
        ojpulltorefresh.tearDownPullToRefresh(this.scrollerElement);
      },
       ...
       * @private
       */
      _checkObserver: function _checkObserver() {
       ...
       * @private
       */
      _setupRefresh: function _setupRefresh() {
        var self = this;
        ...
        if (this.scrollerElement) {
          ...
          ojlogger.error('Issue with the target selected: Target missing or not found');
        }
      },
       ...
       * @private
       */
      _setupPullToRefresh: function _setupPullToRefresh() {
        var threshold = this.options.threshold === 0 ? null : this.options.threshold;
        ojpulltorefresh.setupPullToRefresh(this.scrollerElement, this.options.refreshContent, {
          threshold: threshold,
          primaryText: this.options.text,
          refresherElement: this.element[0]
        });
      },
       ...
       * @private
       */
      _setupScrollerElement: function _setupScrollerElement() {
       ...
       * @private
       */
      _setupObserver: function _setupObserver(self) {
        if (self.scrollerElement) {
          ...
          self.observer = new MutationObserver(function (mutations) {
            mutations.forEach(function (mutation) {
              if (mutation.removedNodes.length > 0 && mutation.removedNodes[0] === self.scrollerElement) {
                self.observer.disconnect(); // eslint-disable-next-line no-param-reassign

                self.observer = null;
                ojpulltorefresh.tearDownPullToRefresh(self.scrollerElement);
              }
            });
          ...
          if (self.parentNode) {
            self.observer.observe(self.parentNode, {
              childList: true
            });
          } else {
            self.observer.observe(document, {
              childList: true
            });
          }
        }
