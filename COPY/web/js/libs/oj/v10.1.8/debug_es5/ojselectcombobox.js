...
define(['ojs/ojeditablevalue', 'ojs/ojoptgroup', 'ojs/ojoption', 'ojs/ojhighlighttext', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojset', 'ojs/ojtimerutils', 'ojs/ojcontext', 'ojs/ojlistdataproviderview', 'ojs/ojtreedataproviderview', 'ojs/ojtranslation', 'ojs/ojlogger', 'ojs/ojcustomelement-utils', 'ojs/ojcomponentcore', 'ojs/ojthemeutils'], function (ojeditablevalue, ojoptgroup, ojoption, ojhighlighttext, oj$1, $, DomUtils, ojSet, TimerUtils, Context, ListDataProviderView, TreeDataProviderView, Translation, Logger, ojcustomelementUtils, Components, ThemeUtils) {
  ...
  var _AbstractOjChoice = _ComboUtils.clazz(Object, {
    ...
    _init: function _init(_opts) {
      ...
      this.isOjOption = this.ojContext._IsCustomElement() && !opts.options && opts.element.find('oj-option').length > 0; // JET-39086 - raw-value is not getting updated until space in android devices
      // In android device we need to update rawValue even for composition events
      // Get and store agent info

      this._isAndroidDevice = oj$1.AgentUtils.getAgentInfo().os === oj$1.AgentUtils.OS.ANDROID; // 'opts.element' is initialized in _setup() method in component files
    ...
    _selectItemFromArray: function _selectItemFromArray(values, options) {
      ...
      var optionsKeys = this.ojContext.options.optionsKeys || {};
      ...
      for (var i = 0; i < optionsArray.length; i++) {
        ...
        var value = _ComboUtils.lookupOptionKeys(item, optionsKeys, 'value');

        if (values.includes(value)) {
          foundItems.set(value, item); // prematurely break the loop if all values are found
  ...
  var _AbstractMultiChoice = _ComboUtils.clazz(_AbstractOjChoice, {
    ...
    _InvokeSelection: function _InvokeSelection(items, options) {
      ...
      var valueOptions = [];
      var valueOptionsData = [];
      var valueOptionsMetadata = []; // Iterate through the items and generate value and valueOptions
  ...
  var _OjMultiCombobox = _ComboUtils.clazz(_AbstractMultiChoice, {
    ...
    _setPlaceholder: function _setPlaceholder() {
      ...
      var isValid = this.ojContext.isValid();

      var isValidValueForPlaceholder = _ComboUtils.isValueForPlaceholder(true, this.getVal()) && _ComboUtils.isValueOptionsForPlaceholder(true, this.getValOpts());

      var isCurrentValueForPlaceholder = _ComboUtils.isValueForPlaceholder(true, this.currentValue) && _ComboUtils.isValueOptionsForPlaceholder(true, this.currentItem);
      ...
      if (isValid && isValidValueForPlaceholder || !isValid && isCurrentValueForPlaceholder) {
        ...
        _OjMultiCombobox.superclass._setPlaceholder.apply(this, arguments); // After setting the placeholder, resize the input field to make sure it has proper
        // width
