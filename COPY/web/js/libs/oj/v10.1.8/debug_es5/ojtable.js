...
define(['touchr', 'ojdnd', 'ojs/ojeditablevalue', 'ojs/ojinputnumber', 'ojs/ojmenu', 'ojs/ojpopup', 'ojs/ojbutton', 'ojs/ojdatasource-common', 'ojs/ojdataprovideradapter', 'ojs/ojlistdataproviderview', 'ojs/ojselector', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojlogger', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojtranslation', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojdatacollection-common', 'ojs/ojanimation', 'ojs/ojdomscroller', 'ojs/ojcustomelement-utils', 'ojs/ojkeyset', 'ojs/ojvalidator-regexp'], function (touchr, ojdnd, ojeditablevalue, ojinputnumber, ojmenu, ojpopup, ojbutton, ojdatasourceCommon, ojdataprovideradapter, ListDataProviderView, ojselector, oj, $, DomUtils, Logger, Context, ojconfig, ojtranslation, ojthemeutils, ojcomponentcore, DataCollectionUtils, ojanimation, DomScroller, ojcustomelementUtils, ojkeyset, RegExpValidator) {
  ...
  Table.prototype._draw = function () {
    this._setFinalTask(function () {
      ...
      if (this._resetAriaLabel && (this._accRowIndex != null && this._accColumnIndex != null || this._accHeaderIndex != null || this._accFooterIndex != null)) {
      ...
      if (headerRenderer) {
        ...
        if (headerSlotTemplate) {
          ...
          if (templateEngine != null) {
            ...
            var headerContent = templateEngine.execute(componentElement, headerSlotTemplate, slotContext, this.options.as, headerColumn);
        ...
        if (footerRenderer) {
          ...
          if (footerSlotTemplate) {
            ...
            if (templateEngine != null) {
              ...
              var footerContent = templateEngine.execute(componentElement, footerSlotTemplate, slotContext, this.options.as, footerCell);
    ...
    if (rows.length === 0 && tableBodyRows.length === 0) {
      ...
      if (this._animateOnFetch && this._IsCustomElement()) {
        ...
        for (i = 0; i < rowsCount; i++) {
          ...
          addedTableBodyRows.push(this._addSingleTableBodyRow(rows[i].rowIdx, rows[i].row, tableBodyDocFrag, startIndex));
        }
  ...
  Table.prototype._refreshTableBodyRow = function (rowIdx, row, tableBodyRow, docFrag, docFragStartIdx) {
    ...
    if (rowRenderer != null || this._isDefaultRowTemplateSlotValid() && this._getSlotTemplate('rowTemplate') !== null) {
      if (rowRenderer != null) {
        ...
        if (templateEngine != null) {
          var slotContext = this._getRowSlotTemplateContextObject(context); // tableBodyRow may be replaced, so we need to pass in the <tbody> element itself to ensure
          // busy context parent traversal continues to function correctly


          var nodes = templateEngine.execute(componentElement, rowSlotTemplate, slotContext, this.options.as, tableBody);
  ...
  Table.prototype._showNoDataMessage = function () {
    if (!this._noDataMessageShown) {
      ...
      if (this._isDefaultTemplateSlotValid('noData') && noDataTemplate !== null) {
        ...
        if (templateEngine != null) {
          var nodes = templateEngine.execute(this._getRootElement(), noDataTemplate, {}, null, tableBody);
  ...
  Table.prototype._createContextInfo = function () {
    ...
    contextInfo.classList.add(Table.CSS_CLASSES._HIDDEN_CONTENT_ACC_CLASS); // row context info
    ...
    contextInfo.appendChild(rowContext); // column context info
  ...
  Table.prototype._updateAccStatusInfo = function (rowIndex, columnIndex, headerIndex, footerIndex) {
    ...
    if (headerIndex != null) {
      ...
      this._columnContextInfo.textContent = this.getTranslatedString('accessibleColumnHeaderContext', {
        index: headerIndex + 1
      });
      ...
      this._columnContextInfo.textContent = this.getTranslatedString('accessibleColumnFooterContext', {
        index: footerIndex + 1
      });
      label += this._columnContextInfo.id + ' ';
      var tableFooterCell;
  ...
  Table.prototype._executeTableBodyRowsAdd = function (eventDetail) {
    ...
    if (eventDetailKeys && eventDetailKeys.size > 0) {
      this._queueTask(function () {
        ...
        layoutManager.notifyTableUpdate(Table._UPDATE._ROWS_ADDED); // see if we should batch add
        ...
        if (!batchAdd) {
          ...
          for (i = 0; i < rowsCount; i++) {
            ...
            tableBodyDocFrag = document.createDocumentFragment();
            addedTableBodyRows.push(this._addSingleTableBodyRow(rowIndex, rows[i].row, tableBodyDocFrag, rowIndex));
            layoutManager.handleAfterRowsProcessed(tableBodyDocFrag);
  ...
  Table.prototype._executeTableBodyRowsChange = function (eventDetail, startIndex) {
    this._queueTask(function () {
      ...
      for (var i = 0; i < rowsCount; i++) {
        var row = rows[i]; // if existing (stale) row contains focus, reset focus to overall table element

        var staleRow = this._getTableBodyRow(row.rowIdx);

        if (staleRow != null && staleRow !== document.activeElement && staleRow.contains(document.activeElement)) {
          resetFocus = true;
        }
    ...
    if (!cellRenderer && !cellSlotTemplate) {
      ...
      if (cellRenderer) {
        ...
        if (templateEngine != null) {
          ...
          var cellContent = templateEngine.execute(componentElement, cellSlotTemplate, slotContext, this.options.as, tableBody);
