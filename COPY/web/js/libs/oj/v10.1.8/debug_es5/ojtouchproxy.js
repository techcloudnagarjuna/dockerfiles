 ...
 * @ignore
 */
define(['jquery', 'ojs/ojcore-base'], function ($, oj) {
  'use strict';
  ...
  var _TouchProxy = function _TouchProxy(elem) {
    this._init(elem);
  };
  ...
  _TouchProxy.prototype._init = function (elem) {
    ...
    this._touchMoved = false; // add touchListeners
    ...
    this._elem[0].addEventListener('touchstart', this._touchStartHandler, {
      passive: true
    });

    this._elem[0].addEventListener('touchmove', this._touchMoveHandler, {
      passive: false
    });
  };
  ...
  _TouchProxy.prototype._touchHandler = function (event, simulatedType) {
    ...
    var simulatedEvent = document.createEvent('MouseEvent'); // Initialize the simulated mouse event using the touch event's coordinates
    // initMouseEvent(type, canBubble, cancelable, view, clickCount,
    //                screenX, screenY, clientX, clientY, ctrlKey,
    //                altKey, shiftKey, metaKey, button, relatedTarget);

    simulatedEvent.initMouseEvent(simulatedType, true, true, window, 1, touch.screenX, touch.screenY, touch.clientX, touch.clientY, false, false, false, false, 0
    /* left*/
    , null);
  ...
  _TouchProxy.prototype._touchStart = function (event) {
    ...
    this._touchHandled = true; // Track movement to determine if interaction was a click

    this._touchMoved = false; // touchstart is registered with addEventListener but
    // downstream code expects jQuery event
    // eslint-disable-next-line no-param-reassign

    event = $.Event(event); // Simulate the mouseover, mousemove and mousedown events
  ...
  _TouchProxy.prototype._touchMove = function (event) {
    ...
    this._touchMoved = true; // touchmove is registered with addEventListener but
    // downstream code expects jQuery event
    // eslint-disable-next-line no-param-reassign

    event = $.Event(event); // Simulate the mousemove event
  ...
  _TouchProxy.prototype._touchEnd = function (event) {
    ...
    this._touchHandler(event, 'mouseout'); // If the touch interaction did not move, it should trigger a click
