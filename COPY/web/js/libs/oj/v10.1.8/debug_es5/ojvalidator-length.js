 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojtranslation', 'ojs/ojvalidator', 'ojs/ojvalidation-error'], function (oj, Translations, Validator, ojvalidationError) {
  'use strict';
  ...
  var LengthValidator = function LengthValidator(options) {
    this.Init(options);
  };
  /**
  ...
  LengthValidator.prototype.Init = function (options) {
    ...
    this._max = options.max !== undefined ? parseInt(options.max, 10) : null; // check that the min/max make sense, otherwise throw an error
    ...
    if (this._min !== null && this._min < 0) {
      throw new Error("length validator's min option cannot be less than 0. min option is " + options.min);
    }

    if (this._max !== null && this._max < 1) {
      throw new Error("length validator's max option cannot be less than 1. max option is " + options.max);
    }
  ...
  LengthValidator.prototype.getHint = function () {
    ...
    if (min !== null && max !== null) {
      if (min !== max) {
        ...
        hint = hintInRange ? translations.applyParameters(hintInRange, params) : translations.getTranslatedString('oj-validator.length.hint.inRange', params);
      } else {
        params = {
          length: min
        };
        hint = hintExact ? translations.applyParameters(hintExact, params) : translations.getTranslatedString('oj-validator.length.hint.exact', params);
      }
    } else if (min !== null) {
      params = {
        min: min
      };
      hint = hintMinimum ? translations.applyParameters(hintMinimum, params) : translations.getTranslatedString('oj-validator.length.hint.min', params);
    } else if (max !== null) {
      params = {
        max: max
      };
      hint = hintMaximum ? translations.applyParameters(hintMaximum, params) : translations.getTranslatedString('oj-validator.length.hint.max', params);
    }
  ...
  LengthValidator.prototype.validate = function (value) {
    ...
    var length = this._getLength(string); // If only min is set and length is at least min, or
    ...
    if (length < this._min) {
      ...
      summary = messageSummaryTooShort ? translations.applyParameters(messageSummaryTooShort, params) : translations.getTranslatedString('oj-validator.length.messageSummary.tooShort');
      detail = messageTooShort ? translations.applyParameters(messageTooShort, params) : translations.getTranslatedString('oj-validator.length.messageDetail.tooShort', params);
    } else {
      // too long
      params = {
        value: value,
      ...
      summary = messageSummaryTooLong ? translations.applyParameters(messageSummaryTooLong, params) : translations.getTranslatedString('oj-validator.length.messageSummary.tooLong');
      detail = messageTooLong ? translations.applyParameters(messageTooLong, params) : translations.getTranslatedString('oj-validator.length.messageDetail.tooLong', params);
    }
  ...
  LengthValidator.prototype._getLength = function (text) {
    ...
    switch (countBy) {
      case 'codepoint':
        ...
        oj.Assert.assert(surrogateLength % 2 === 0, 'the number of surrogate chars must be an even number.');
