 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojtranslation', 'ojs/ojvalidator', 'ojs/ojvalidation-error', 'ojs/ojconverterutils'], function (oj, Translations, Validator, ojvalidationError, ConverterUtils) {
  'use strict';
  ...
  NumberRangeValidator.prototype.validate = function (value) {
    ...
    var generateValidationError = function generateValidationError(minStr, maxStr) {
      ...
      if (max !== null && min !== null && min === max) {
        ...
        detail = messageDetailExact ? translations.applyParameters(messageDetailExact, params) : translations.getTranslatedString('oj-validator.range.number.messageDetail.exact', params); // if number is greater than max, the summary may say "The number is too high"

        if (numberValue > max) {
          summary = messageSummaryRangeOverflow || translations.getTranslatedString('oj-validator.range.number.messageSummary.rangeOverflow');
        } else if (numberValue < min) {
          // if number is less than min, the summary may say "The number is too low"
          summary = messageSummaryRangeOverflow ? messageSummaryRangeUnderflow : translations.getTranslatedString('oj-validator.range.number.messageSummary.rangeUnderflow');
        }
      } else if (max !== null && numberValue > max) {
        ...
        summary = messageSummaryRangeOverflow || translations.getTranslatedString('oj-validator.range.number.messageSummary.rangeOverflow');
        detail = messageDetailRangeOverflow ? translations.applyParameters(messageDetailRangeOverflow, params) : translations.getTranslatedString('oj-validator.range.number.messageDetail.rangeOverflow', params);
      } else {
        ...
        summary = messageSummaryRangeUnderflow || translations.getTranslatedString('oj-validator.range.number.messageSummary.rangeUnderflow');
        detail = messageDetailRangeUnderflow ? translations.applyParameters(messageDetailRangeUnderflow, params) : translations.getTranslatedString('oj-validator.range.number.messageDetail.rangeUnderflow', params);
      }
  ...
  NumberRangeValidator.prototype.getHint = function () {
    ...
    var generateHintText = function generateHintText(minStr, maxStr) {
      var hint = null; // if both min and max are specified, the hint may say something like "Enter a value
      // between {min} and {max}".
      ...
      if (min !== null && max !== null) {
        if (min !== max) {
          ...
          hint = hintInRange ? translations.applyParameters(hintInRange, {
            min: minStr,
            max: maxStr
          ...
          // use that string, else use the default.
          hint = hintExact ? translations.applyParameters(hintExact, {
            num: minStr
        ...
        // use that string, else use the default.
        hint = hintMinimum ? translations.applyParameters(hintMinimum, {
          min: minStr
        ...
        // use that string, else use the default.
        hint = hintMaximum ? translations.applyParameters(hintMaximum, {
          max: maxStr
