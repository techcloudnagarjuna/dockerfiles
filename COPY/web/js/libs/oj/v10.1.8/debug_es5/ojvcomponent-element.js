 ...
 * @ignore
 */
define(['require', 'exports', 'ojs/ojvcomponent', 'ojs/ojvdom', 'ojs/ojcustomelement', 'ojs/ojcore-base', 'ojs/ojdefaultsutils', 'ojs/ojcontext', 'ojs/ojcustomelement-utils'], function (require, exports, ojvcomponent, ojvdom, ojcustomelement, oj, ojdefaultsutils, Context, ojcustomelementUtils) {
  'use strict';
  ...
  */
  ...
  var ElementVComponent = /*#__PURE__*/function (_ojvcomponent$VCompon) {
    _inherits(ElementVComponent, _ojvcomponent$VCompon);

    var _super = _createSuper(ElementVComponent);

    function ElementVComponent() {
      _classCallCheck(this, ElementVComponent);

      return _super.apply(this, arguments);
    }

    return _createClass(ElementVComponent);
  }(ojvcomponent.VComponent);

  var flattenChildren = ojvcomponent.flattenChildren;

  var VComponentState = /*#__PURE__*/function (_ojcustomelementUtils) {
    _inherits(VComponentState, _ojcustomelementUtils);

    var _super2 = _createSuper(VComponentState);

    function VComponentState() {
      _classCallCheck(this, VComponentState);

      return _super2.apply(this, arguments);
    }

    _createClass(VComponentState, [{
      ...
      value: function GetPreCreatedPromise() {
        ...
        var preCreatePromise = _get(_getPrototypeOf(VComponentState.prototype), "GetPreCreatedPromise", this).call(this);

        if (!VComponentState._cachedTemplateEngine && this._hasDirectTemplateChildren()) {
          return preCreatePromise.then(function () {
            return _this._getTemplateEnginePromise();
          });
        }
      ...
      value: function GetDescriptiveTransferAttributeValue(attrName) {
        ...
        var props = bridge._getVComponentProps();

        return props[attrName];
      }
    }, {
      key: "_getTemplateEnginePromise",
      value: function _getTemplateEnginePromise() {
      ...
      key: "_hasDirectTemplateChildren",
      value: function _hasDirectTemplateChildren() {
        var childNodeList = this.Element.childNodes;
  ...
  oj.CollectionUtils.copyInto(VirtualElementBridge.proto, {
    ...
    AttributeChangedCallback: function AttributeChangedCallback(attr, oldValue, newValue) {
      ...
      if (oldValue !== newValue) {
        ...
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this); // If we haven't already called HandleAttributeChanged in superclass, check to see
    ...
    CreateComponent: function CreateComponent(element) {
      ...
      if (!vcomp) {
        var elementInfo = this._initSlotMapAndRootProps(element);

        var vprops = this._getVComponentProps();

        var descriptor = ojcustomelementUtils.CustomElementUtils.getElementDescriptor(element.tagName);
        vcomp = new descriptor._CONSTRUCTOR(vprops);
        vcomp._uniqueId = elementInfo.uniqueId; // eslint-disable-next-line no-param-reassign
    ...
    InitializeElement: function InitializeElement(element) {
      if (!element._vcomp) {
        ...
        var writebackProps = this._EXTENSION._WRITEBACK_PROPS;
    ...
    initializeBridge: function initializeBridge(element, descriptor) {
      ...
      this._PROPS = {}; // Stores internal property change events that are waiting to be fired.

      this._EVENT_QUEUE = []; // Has getters/setters and calls to set properties on this._PROPS
    ...
    InitializeActionCallbacks: function InitializeActionCallbacks(element, eventsMeta) {
      var _this2 = this;

      Object.keys(eventsMeta).forEach(function (event) {
        var eventMeta = eventsMeta[event];
        var eventProp = ojcustomelementUtils.AttributeUtils.eventTypeToEventListenerProperty(event);

        _this2._PROPS[eventProp] = function (detailObj) {
          ...
          var eventPromise = _this2._queueFireEventsTask(customEvent, element);

          if (cancelable) {
            return eventPromise.then(function () {
    ...
    InitializeWritebackCallbacks: function InitializeWritebackCallbacks(writebackProps) {
      var _this3 = this;

      writebackProps.forEach(function (prop) {
        // e.g. value -> onValueChanged
        var callbackProp = ojcustomelementUtils.AttributeUtils.propertyNameToChangedCallback(prop);

        _this3._PROPS[callbackProp] = function (value) {
          // The inner set will trigger a call to _updateProperty
          // to mutate the property and queue the property changed event
          _this3._PROPS_PROXY[prop] = value;
        };
      });
    },
    HandlePropertyChanged: function HandlePropertyChanged(element, property, value, previousValue, isOuter, subprop) {
     ...
     * @private
     */
    _updateProperty: function _updateProperty(element, property, value, meta, isOuter) {
      ...
      if (!this.SaveEarlyPropertySet(element, property, value)) {
        ...
        if (!oj.BaseCustomElementBridge.__CompareOptionValues(property, meta, value, previousValue)) {
          ...
          this.State.dirtyProps.add(property); // Queue a property change event to fire

          this._queuePropertyChangedEvent(element, property, validatedValue, previousValue, isOuter, null); // This will get called before connected callback so short circuit render for that case
    ...
    _queuePropertyChangedEvent: function _queuePropertyChangedEvent(element, property, value, previousValue, isOuter, subprop) {
      ...
      if (!this._SKIP_PROP_CHANGE_EVENT && (!isOuter || this.State.isComplete)) {
        ...
        var customEvent = this._getPropertyChangedEvent(property, value, previousValue, updatedFrom, subprop);
     ...
     * @private
     */
    _queueFireEventsTask: function _queueFireEventsTask(customEvent, element) {
      var _this4 = this;
      ...
      if (!this._eventsQueued) {
        ...
        var busyCallback = busyContext.addBusyState({
          description: ojcustomelementUtils.CustomElementUtils.getElementInfo(element) + ' event queued.'
        });
        this._eventsQueued = new Promise(function (resolve) {
          window.queueMicrotask(function () {
    ...
    _getPropertyChangedEvent: function _getPropertyChangedEvent(property, value, previousValue, updatedFrom, subprop) {
      ...
      detail.updatedFrom = updatedFrom; // The bridge sets the ready to fire flag after the component has been instantiated.
      ...
      return new CustomEvent(property + 'Changed', {
        detail: detail
      });
    },
    ...
    _getVComponentProps: function _getVComponentProps() {
      ...
      Object.assign(propsCopy, this._PROPS, this._VCOMP_CONTROLLED_PROPS); // We need to remove readOnly properties so they're not available to VComponents in this.props

      var readOnlyProps = this._EXTENSION._READ_ONLY_PROPS;

      if (readOnlyProps) {
        readOnlyProps.forEach(function (prop) {
          return delete propsCopy[prop];
        });
      }
    ...
    _getCallbacks: function _getCallbacks(element) {
      ...
      return {
        getPropsForRender: function getPropsForRender() {
          return _this5._getVComponentProps();
        },
        patch: function patch(props) {
          ...
          var controlledPropsCopy = Object.assign({}, _this5._LIVE_CONTROLLED_PROPS);

          element._vcomp.patchContent(props, controlledPropsCopy, _this5._content); // Store unslotted nodes


          VirtualElementBridge._storeUnslottedNodes(element, _this5._slotVNodes);
        },
        convertChildrenToSlotProps: function convertChildrenToSlotProps(children) {
          ...
          if (!_this5._slotProps) {
            // Save the slot nodes so we can store unslotted nodes after render
            _this5._slotVNodes = children; // Generate the map of slot property names to vnode arrays

            var slotMap = _generateSlotMap(children, _this5.METADATA, _this5._EXTENSION); // Convert to map of slot property names as the right types
            // 1) VNode[] for default children property
            // 2) () => VNode[] for named non template slots
            // 3) (context) => VNode[] for template slots


            _this5._slotProps = _generateSlotPropsMap(element, slotMap, _this5.METADATA.slots, _this5._EXTENSION._DYNAMIC_SLOT);
          }

          return _this5._slotProps;
        }
      };
    },
    ...
    _unmountAndReset: function _unmountAndReset(element) {
      ...
      element._vcomp.notifyUnmounted(); // When we unmount, we will reset certain flags
      // so we can recreate from scratch the next time
      // we reconnected to the DOM


      this._bCreateCalled = false; // Store slot content so we're ready to redistribute on connect

      VirtualElementBridge._storeAllSlotContent(element, this._slotVNodes); // Null out the vcomp instance
      // eslint-disable-next-line no-param-reassign


      element._vcomp = null;
    },
    _initSlotMapAndRootProps: function _initSlotMapAndRootProps(element) {
      ...
      if (!this._slotMap) {
        ...
        this._slotMap = ojcustomelementUtils.CustomElementUtils.getSlotMap(element); // Initialize controlled root properties now that we know bindings have resolved and
        // data bound global attributes should be resolved.

        this._initializeControlledProps(element); // Define the _vcomp property on the element to be set by the CreateComponent call
  ...
  VirtualElementBridge.register = function (tagName, constr) {
    ...
    var registration = {
      ...
      bridgeProto: VirtualElementBridge.proto,
  ...
  function _generateSlotMap(children, metadata, ext) {
    var slots = metadata.slots || {}; // dynamicSlot is an object of form { prop: [propName], isTemplate: [1 for true and 0 for false]}

    var dynamicSlot = ext._DYNAMIC_SLOT; // TODO do we throw an error or log a warning if we encounter a slot item
    // that should go into a dynamic prop but there's no dynamic prop, basically
    // mean the slot content was marked in correctly?

    var dynamicSlotProp = dynamicSlot ? dynamicSlot.prop : null;
    var slotMap = {};
  ...
  function _generateSlotPropsMap(element, slotMap, slots, dynamicSlot) {
    ...
    Object.keys(slotMap).forEach(function (slot) {
      var slotContent = slotMap[slot]; // The template slot case only supports render functions
      ...
      if (slot === VirtualElementBridge._DEFAULT_SLOT_PROP) {
        ...
        Object.keys(slotContent).forEach(function (dynSlot) {
          propsMap[slot][dynSlot] = dynamicSlot && dynamicSlot.isTemplate ? getSlotRenderer(element, slotContent[dynSlot]) : slotContent[dynSlot];
        });
      }
    });
    return propsMap;
  ...
  function getSlotRenderer(element, vnodes) {
    var domNode = _unwrapNode(vnodes[0]);

    if (domNode.nodeName !== 'TEMPLATE') {
      throw new Error("Template node expected for template slot, but found ".concat(domNode.nodeName, " instead."));
    }

    return function (context) {
      var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(element);
      var cachedTemplateEngine = bridge.State.getTemplateEngine(); // Execute the template engine and wrap the resulting DOM nodes

      if (!cachedTemplateEngine) {
        throw new Error('Unexpected call to render a template slot');
      }

      var domNodes = cachedTemplateEngine.execute(element, domNode, context);
  ...
  function customElement(tagName) {
    return function (constructor) {
      ...
      constructor.prototype.mount = function (props, content, uncontrolledRootProps) {
        ...
        var mountNode = this._ref = ojvdom.mountCustomElement(this._vnode, uncontrolledRootProps);
      ...
      function _isValidRootProp(prop, metadata) {
        ...
        return allowedProps[prop] || verifiedRootPropMap[prop] || ojcustomelementUtils.AttributeUtils.isEventListenerProperty(prop);
      }
