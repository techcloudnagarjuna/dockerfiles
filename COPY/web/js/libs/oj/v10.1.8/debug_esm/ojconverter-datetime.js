 ...
 * @ignore
 */
const OraDateTimeConverter = (function () {
  var instance;
  ...
  function _getLocaleDecimalSeparator(localeElements, locale) {
    var numberingSystemKey = OraI18nUtils.getNumberingSystemKey(
  ...
  function _getTimezoneName(localeElements, value, options, len, isTimeOnly) {
    ...
    if (zoneNameEntry0 === undefined) {
      ...
      if (offset !== 0) {
        offset = OraI18nUtils.getTimeStringFromOffset(_UTC,
          offset, true, true);
      } else {
        offset = _UTC;
      }
  ...
  function _formatImpl(localeElements, options, isoStrInfo, locale) {
    ...
    function _getPaddedPart(_ret, _value, idx, len) {
      ...
      _ret.push(len > 1 ? OraI18nUtils.padZeros(val, len) : val);
    }
  ...
  function _formatRelativeImplicit(now, relativeDate, localeElements, field) {
    ...
    var mainNodeKey = OraI18nUtils.getLocaleElementsMainNodeKey(
      localeElements);
    var mainNode = OraI18nUtils.getLocaleElementsMainNode(localeElements);
  ...
  function _arrayIndexOfDay(daysObj, item) {
    ...
    for (var i = 0; i < daysKeys.length; i++) {
      var d = daysKeys[i];
      if (OraI18nUtils.trim(
        OraI18nUtils.toUpper(daysObj[d])) ===
          OraI18nUtils.trim(item)) {
        return days[d];
      }
    }
    return -1;
  ...
  function _arrayIndexOfMonth(monthsObj, item) {
    ...
    for (var i = 0; i < monthsKeys.length; i++) {
      var m = monthsKeys[i];
      if (OraI18nUtils.trim(
        OraI18nUtils.toUpper(monthsObj[m])) ===
          OraI18nUtils.trim(item)) {
        return i;
      }
    }
  ...
  function _parseLenienthms(result, timepart, format, localeElements, dtype) {
    ...
    switch (match[0]) {
      case 'h':
        ...
        idx = (OraI18nUtils.toUpper(timepart)).indexOf(
          OraI18nUtils.toUpper(calPM));
        if (idx !== -1 && hour < 12) {
          hour += 12;
        }
        break;
  ...
  function _parseLenientyMEd(value, format, options, localeElements, isDateTime) {
    ...
    month -= 1;
    var daysInMonth = OraI18nUtils._getDaysInMonth(year, month);
  ...
  function _parseLenientyMMMEd(value, format, options, localeElements, isDateTime) {
    ...
    for (i = 0; i < months.length; i++) {
      ...
      for (j = 0; j < monthKeys.length; j++) {
        mName = OraI18nUtils.toUpper(months[i][monthKeys[j]]);
    ...
    for (var j = 0, jl = groups.length; j < jl; j++) {
      ...
      if (matchGroup) {
        ...
        switch (currentGroup.token) {
          ...
          case 'dayOfMonth':
            ...
            if (date > 31 && lenientParse === 'full') {
              try {
                ...
                var MonthDays = OraI18nUtils._getDaysInMonth(year, month);
          ...
          case 'ampm':
            pmHour =
              (OraI18nUtils.toUpper(matchGroup)).indexOf(
                OraI18nUtils.toUpper(calPM)) !== -1;
            break;
          case 'tzhm':
            // Time zone hours minutes: -0800
  ...
  function _availableTimeZonesImpl(localeElements) {
    ...
    function getLocalizedName(id, offset, _metaZones, _cities) {
      ...
      if (offset !== 0) {
        locName =
          OraI18nUtils.getTimeStringFromOffset(_UTC, offset, true, true);
        locName = '(' + locName + ')';
      }
      if (locZone === undefined) {
        locZone = '';
  ...
  function _init() {
    return {
      ...
      format: function (value, localeElements, options, locale) {
        ...
        var isoStrInfo = OraI18nUtils.getISOStrFormatInfo(val);
...
IntlDateTimeConverter.prototype._getHintValue = function () {
  ...
  try {
    // example date
    value =
      this.format(IntlConverterUtils.dateToLocalIso(
...
IntlDateTimeConverter.prototype.parse = function (value) {
  ...
  try {
    ...
    if (parsed) {
      ...
      // field.
      if (result.warning) {
        warn('The value ' + value + ' was leniently parsed to represent a date ' +
...
IntlDateTimeConverter.prototype.compareISODates = function (isoStr, isoStr2) {
  ...
  return this._getWrapped().compareISODates(isoStr, isoStr2, __getBundle());
};
...
IntlDateTimeConverter.prototype.getAvailableTimeZones = function () {
  return this._getWrapped().getAvailableTimeZones(__getBundle());
};
...
IntlDateTimeConverter.prototype._getTimePositioning = function (param) {
  ...
  return this._getWrapped().getTimePositioning(__getBundle(),
                                               resolvedOption,
                                               getLocale());
};

export { DateTimeConverter, IntlDateTimeConverter };
