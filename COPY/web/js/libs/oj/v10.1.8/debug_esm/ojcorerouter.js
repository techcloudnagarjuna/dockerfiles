 ...
 */
  ...
  function CoreRouter(routes, options, parentRouter) {
    ...
    // _offset is the distance from the root router
    this._offset = 0;
    ...
    if (!parentRouter) {
      ...
      // _offset is the relative offset to the root router
  ...
  CoreRouter.prototype._prepublish = function (state) {
    ...
    var activeSync = Promise.all(allPre)
    .then(function () {
      return state;
    });
  ...
  CoreRouter.prototype.createChildRouter = function (routes, options = {}) {
    ...
    var cs = this._activeState;
    if (!cs) {
      throw Error('Router(' + this._name + ') has no current state. ' +
        'Call sync() on the router first.');
    }
    ...
    var childRouter = new CoreRouter(routes, options, this);
