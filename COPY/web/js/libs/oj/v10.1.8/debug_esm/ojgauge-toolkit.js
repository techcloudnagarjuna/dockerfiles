 ...
 * @ignore
 */
import { Obj, BaseComponentDefaults, CSSStyle, JsonUtils, Container, Matrix, Rectangle, ResourceUtils, OutputText, TextUtils, MultilineText, Agent, Image, ImageLoader, Point, Automation, EventManager, TouchEvent, KeyboardEvent, KeyboardHandler, EventFactory, BaseComponent, Rect, AriaUtils, SelectionEffectUtils, CustomDatatipPeer, SimpleObjPeer, BlackBoxAnimationHandler, CustomAnimation, Animator, ParallelPlayable, Displayable, Easing, Math as Math$1, ColorUtils, Cache, Polygon, Circle, Path, DisplayableUtils, PathUtils, PolygonUtils, LinearGradientFill, Use, ImageMarker, ClipPath, Line, Stroke, Shadow } from 'ojs/ojdvt-toolkit';
...
Obj.createSubclass(DvtGaugeDefaults, BaseComponentDefaults);
...
DvtGaugeDefaults.prototype.Init = function(defaultsMap) {
  ...
  var ret = {
    'alta': JsonUtils.merge(defaultsMap['alta'], DvtGaugeDefaults.SKIN_ALTA)
  };
...
Obj.createSubclass(DvtDialGaugeDefaults, DvtGaugeDefaults);
...
DvtDialGaugeDefaults.SKIN_ALTA = {
  ...
  'metricLabel': {'style': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA)},
  'tickLabel': {
    'scaling': 'auto',
    'style': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA)
  }
...
Obj.createSubclass(DvtDialGaugeIndicator, Container);
...
Obj.createSubclass(DvtGaugeDataUtils, Obj);
...
Obj.createSubclass(DvtGaugeRenderer, Obj);
...
DvtGaugeRenderer.renderEmptyText = function(gauge, container, availSpace) {
  ...
  if (gauge.getGaugeType() === 'statusMeter') {
...
DvtGaugeRenderer.renderLabel = function(gauge, container, bounds, color, valign) {
  ...
  // Create and position the metricLabel
  if (DvtGaugeUtils.hasLabel(options)) {
...
Obj.createSubclass(DvtDialGaugeRenderer, Obj);


/**
 * Renders the gauge in the specified area.
 * @param {DialGauge} gauge The gauge being rendered.
 * @param {dvt.Container} container The container to render into.
...
DvtDialGaugeRenderer.updateIndicatorAndLabel = function(gauge, container, width, height)
{
  ...
  DvtDialGaugeRenderer._renderLabel(gauge, container, new Rectangle(0, 0, width, height));
};
...
DvtDialGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  var translateContainer = new Container(gauge.getCtx());
  ...
  if (images && images.length > 0) {
    ...
    for (i = 0; i < images.length; i++) {
      ...
      if (isSvg || (width >= widthRes && height >= heightRes) || i == images.length - 1) {
        var shape = new Image(gauge.getCtx(), source, 0, 0, width, height);
        ...
        if (isSvg && (Agent.browser === 'safari' || Agent.engine === 'blink')) {
          var imageDims = ImageLoader.loadImage(source, shape.__setDimensions.bind(shape));
          if (imageDims)
...
DvtDialGaugeRenderer._createIndicator = function(gauge, bounds) {
  ...
  // Calculate the required resolution needed.  This check isn't ideal, but it's close enough for now.
  var heightRes = Agent.isTouchDevice() ? 2 * indicatorLength : indicatorLength;
...
DvtDialGaugeRenderer._getIndicatorAnchorPoint = function(gauge, bounds, indicatorBounds) {
  ...
  if (!isNaN(anchorX) && !isNaN(anchorY))
    return new Point(anchorX, anchorY); // already adjusted for image size
  else // default to center of the bottom edge
    return new Point(indicatorBounds.x + indicatorBounds.w / 2, indicatorBounds.y + indicatorBounds.h);
};
...
DvtDialGaugeRenderer._renderLabel = function(gauge, container, bounds) {
  ...
  if (options['metricLabel']['rendered'] == 'on') {
    ...
    // Create the label and align
    var label = new OutputText(gauge.getCtx(), labelString, cx, cy);
    ...
    // Truncate if needed, null is returned if the label doesn't fit
    if (TextUtils.fitText(label, labelWidth, labelHeight, container)) {
      gauge.__label = label;
    }
  }
};
...
DvtDialGaugeRenderer._renderTickLabels = function(gauge, container, bounds) {
  ...
  if (options['background']['radius'] && options['background']['majorTickCount']) {
    ...
    var fontSize = 12;
    var labelBounds = new Rectangle(cx, cy, bounds.w, bounds.h);
    ...
    if (!style) {
      var label = new OutputText(gauge.getCtx(), '', cx, cy);
    ...
    for (var x = 0; x < majorTickCount; x++)
    {
      ...
      // Truncate if needed
      TextUtils.fitText(label, labelBounds.w + .5, labelBounds.h + .5, container);
    }
  }
};
...
Obj.createSubclass(DvtGaugeAutomation, Automation);
...
Obj.createSubclass(DvtGaugeEventManager, EventManager);
 ...
 * @param {event} event
 */
DvtGaugeEventManager.prototype.StopMouseEditing = function (event) {
  this.IsMouseEditing = false;
  var coords = this.GetRelativePosition(event.pageX, event.pageY);
  this._gauge.__processValueChangeEnd(coords.x, coords.y);
};
...
Obj.createSubclass(DvtGaugeKeyboardHandler, KeyboardHandler);
...
Obj.createSubclass(DvtGaugeStyleUtils, Obj);
...
DvtGaugeStyleUtils.getColor = function(gauge) {
  ...
  var thresholdIndex = DvtGaugeDataUtils.getValueThresholdIndex(gauge);
  var threshold = DvtGaugeDataUtils.getThreshold(gauge, thresholdIndex);
  if (threshold && (!(gauge.getGaugeType() === 'statusMeter') ||
      ((gauge.getGaugeType() === 'statusMeter') && options['thresholdDisplay'] == 'onIndicator'))) {
...
DvtGaugeStyleUtils.getBorderColor = function(gauge) {
  ...
  if (threshold && threshold['borderColor'] &&
      (!(gauge.getGaugeType() === 'statusMeter') ||
      (gauge.getGaugeType() === 'statusMeter') && (options['thresholdDisplay'] == 'onIndicator')))
...
DvtGaugeStyleUtils.getPlotAreaColor = function(gauge) {
  ...
  var thresholdIndex = DvtGaugeDataUtils.getValueThresholdIndex(gauge);
  var threshold = DvtGaugeDataUtils.getThreshold(gauge, thresholdIndex);
  if (threshold && (!(gauge.getGaugeType() === 'statusMeter') || ((gauge.getGaugeType() === 'statusMeter') && options['thresholdDisplay'] != 'onIndicator'))) {
    return DvtGaugeStyleUtils.getThresholdColor(gauge, threshold, thresholdIndex);
  }
...
DvtGaugeStyleUtils.getPlotAreaBorderColor = function(gauge) {
  ...
  if ((gauge.getGaugeType() === 'statusMeter') && options['orientation'] != 'circular' &&
      borderColor == null) {
    return '#D6DFE6';
  }
...
Obj.createSubclass(DvtGauge, BaseComponent);
...
/**
 * Returns the gauge type as a string.
 * @return {string}
 */
DvtGauge.prototype.getGaugeType = function() {
  return null;
};
...
DvtGauge.prototype.CreateAnimationOnDataChange = function(oldObjs, newObjs, animationType, animationDuration) {
  ...
  for (var i = 0; i < oldObjs.length; i++) {
    ...
    animation.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, newObj, newObj.getAnimationParams, newObj.setAnimationParams, endState);
 ...
 * @extends {DvtGauge}
 */
const DialGauge = function() {};

Obj.createSubclass(DialGauge, DvtGauge);


/**
 * Returns a new instance of DialGauge.
 * @param {dvt.Context} context The rendering context.
 * @param {string} callback The function that should be called to dispatch component events.
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 * @return {DialGauge}
 */
DialGauge.newInstance = function(context, callback, callbackObj) {
 ...
 * @override
 */
DialGauge.prototype.Init = function(context, callback, callbackObj) {
  DialGauge.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @override
 */
DialGauge.prototype.SetOptions = function(options) {
  ...
  DialGauge.superclass.SetOptions.call(this, this.Defaults.calcOptions(options));
};

/**
 * @override
 */
DialGauge.prototype.Render = function(container, width, height)
{
 ...
 * @override
 */
DialGauge.prototype.getGaugeType = function() {
  return 'dial';
};
 ...
 * override
 */
DialGauge.prototype.renderUpdate = function() {
 ...
 * @override
 */
DialGauge.prototype.CreateAnimationOnDisplay = function(objs, animationType, animationDuration) {
  ...
  for (var i = 0; i < objs.length; i++) {
    ...
    animation.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, obj, obj.getAnimationParams, obj.setAnimationParams, endState);
 ...
 * @override
 */
DialGauge.prototype.GetValueAt = function(x, y) {
  var angleRads = Math.atan2(y - this.__anchorPt.y, x - this.__anchorPt.x);
  var angle = Math$1.radsToDegrees(angleRads);
...
Obj.createSubclass(DvtLedGaugeDefaults, DvtGaugeDefaults);
...
Obj.createSubclass(DvtLedGaugeRenderer, Obj);
 ...
 * @extends {DvtGauge}
 */
const LedGauge = function() {};

Obj.createSubclass(LedGauge, DvtGauge);


/**
 * Returns a new instance of LedGauge.
 * @param {dvt.Context} context The rendering context.
 * @param {string=} callback The function that should be called to dispatch component events.
 * @param {object=} callbackObj The optional object instance on which the callback function is defined.
 ...
 * @return {LedGauge}
 */
LedGauge.newInstance = function(context, callback, callbackObj, bStaticRendering) {
  var gauge = new LedGauge();
 ...
 * @override
 */
LedGauge.prototype.Init = function(context, callback, callbackObj, bStaticRendering) {
  LedGauge.superclass.Init.call(this, context, callback, callbackObj, bStaticRendering);
 ...
 * @override
 */
LedGauge.prototype.SetOptions = function(options) {
  // NOTE: This extra clone should be removed once we stop supporting the deprecated attrs
  options = JsonUtils.clone(options);
  ...
  // Combine the user options with the defaults and store
  LedGauge.superclass.SetOptions.call(this, this.Defaults.calcOptions(options));
 ...
 * @override
 */
LedGauge.prototype.Render = function(container, width, height)
{
...
Obj.createSubclass(DvtRatingGaugeDefaults, DvtGaugeDefaults);
...
Obj.createSubclass(DvtRatingGaugeEventManager, DvtGaugeEventManager);
 ...
 * @override
 */
DvtRatingGaugeEventManager.prototype.OnContextMenu = function (event) {
  ...
  DvtRatingGaugeEventManager.superclass.OnContextMenu.call(this, event);
};
...
Obj.createSubclass(DvtRatingGaugePeer, SimpleObjPeer);
...
Obj.createSubclass(DvtRatingGaugeRenderer, Obj);
...
DvtRatingGaugeRenderer._createShape = function(context, x, y, width, height, stateOptions, gaugeOptions) {
  ...
  if (stateOptions['source']) {
    ...
    shape = LedGauge.newInstance(context, null, null, true);
    if (x != 0 && y != 0)
 ...
 * @extends {DvtGauge}
 */
const RatingGauge = function() {};

Obj.createSubclass(RatingGauge, DvtGauge);


/**
 * Returns a new instance of RatingGauge.
 * @param {dvt.Context} context The rendering context.
 * @param {string} callback The function that should be called to dispatch component events.
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 * @return {RatingGauge}
 */
RatingGauge.newInstance = function(context, callback, callbackObj) {
  var gauge = new RatingGauge();
  gauge.Init(context, callback, callbackObj);
  return gauge;
};
 ...
 * @override
 */
RatingGauge.prototype.Init = function(context, callback, callbackObj) {
  RatingGauge.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @override
 */
RatingGauge.prototype.SetOptions = function(options) {
  // NOTE: This extra clone should be removed once we stop supporting the deprecated attrs
  options = JsonUtils.clone(options);
  ...
  if (options['size'] != 'fit') {
    ...
    svg.style.height = options._height;
    svg.style.width = options._width;
  } else {
    this.getCtx().addSizingSvg();
  }

  // Combine the user options with the defaults and store
  RatingGauge.superclass.SetOptions.call(this, this.Defaults.calcOptions(options));
};

/**
 * @override
 */
RatingGauge.prototype.Render = function(container, width, height)
{
  ...
  if (selectedSource && preserveAspectRatio && size === 'fit') {
    ...
    ImageLoader.loadImage(this.Options['selectedState']['source'], onLoad.bind(this));
    return false;
  } else {
 ...
 * @override
 */
RatingGauge.prototype.getGaugeType = function() {
  return 'rating';
};
 ...
 * @override
 */
RatingGauge.prototype.__getLogicalObject = function() {
  var customTooltip = this.Options['tooltip'];
  var tooltipFunc = customTooltip ? customTooltip['renderer'] : null;
  var color = DvtGaugeStyleUtils.getColor(this);
  if (tooltipFunc) {
 ...
 * @override
 */
RatingGauge.prototype.GetValueAt = function(x, y) {
 ...
 * @param {number} y The y coordinate of the value change.
 */
RatingGauge.prototype.__processHoverEnd = function(x, y) {
 ...
 * @override
 */
RatingGauge.prototype.__processValueChangeStart = function(x, y) {
  this.__processValueChangeMove(x, y);
};
 ...
 * @override
 */
RatingGauge.prototype.__processValueChangeMove = function(x, y) {
 ...
 * @override
 */
RatingGauge.prototype.__processValueChangeEnd = function(x, y) {
 ...
 */
RatingGauge.prototype.__updateClipRects = function(value, proc, container) {
  if (!DvtGaugeDataUtils.hasData(this))
    return;
 ...
 * @return {dvt.Displayable}
 */
RatingGauge.prototype.__getRatingGaugeItem = function(index) {
...
Obj.createSubclass(DvtStatusMeterGaugeDefaults, DvtGaugeDefaults);
...
const DvtStatusMeterGaugeUtils = {
   ...
   * @private
   */
  calcPointOnArc: function (bounds, radius, angle) {
   ...
   */
  createCircularPathCmd: function (bounds, startAngle, angleExtent, innerRadius, outerRadius) {
    var cmd;
    var p1, p2, p3, p4;
...
Obj.createSubclass(DvtStatusMeterGaugeCircularIndicator, Path);
...
Obj.createSubclass(DvtStatusMeterGaugeIndicator, Path);
...
Obj.createSubclass(DvtStatusMeterGaugeRenderer, Obj);
...
DvtStatusMeterGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  if (options['metricLabel']['rendered'] == 'on' && !DvtStatusMeterGaugeRenderer._hasMetricLabelOutsidePlotArea(options) && !DvtGaugeUtils.hasLabel(options)) {
  ...
  // Render the label
  else if (DvtGaugeUtils.hasLabel(options)) {
...
DvtStatusMeterGaugeRenderer._renderPlotAreaVisualEffects = function(gauge, container, shape, color, gradientAngle) {
  ...
  // Gradient
  arColors = [ColorUtils.adjustHSL(color, 0, - .04, - .05), ColorUtils.adjustHSL(color, 0, - .09, .04)];
    gradient = new LinearGradientFill(gradientAngle, arColors, [1, 1], [0, 1]);
  shape.setFill(gradient);
...
DvtStatusMeterGaugeRenderer._renderMetricLabelOutsidePlotArea = function(gauge, container, bounds) {
  ...
  if (isVert && options['metricLabel']['rendered'] == 'on') {
    ...
    var maxMetricLabel = new OutputText(gauge.getCtx(), maxValue);
    ...
    var size = metricLabelStyle.getStyle('font-size') || TextUtils.getOptimalFontSize(maxMetricLabel.getCtx(), maxMetricLabel.getTextString(), maxMetricLabel.getCSSStyle(), computedMetricLabelBounds);
  ...
  // Allocate space for the horizontal metricLabel
    ...
    if (options['min'] < 0 && options['plotArea']['rendered'] != 'on' && !(options['plotArea']['rendered'] == 'auto' && (options['thresholdDisplay'] == 'onIndicator'))) {
      ...
      var minMetricLabel = new OutputText(gauge.getCtx(), minValue);
...
DvtStatusMeterGaugeRenderer._hasMetricLabelOutsidePlotArea = function(options) {
  var metricLabelPosition = options['metricLabel']['position'];
  return metricLabelPosition == 'auto' || metricLabelPosition == 'outsidePlotArea' || ((metricLabelPosition == 'withLabel' || metricLabelPosition == 'withTitle') && !DvtGaugeUtils.hasLabel(options));
};
 ...
 * @extends {DvtGauge}
 */
const StatusMeterGauge = function() {};

Obj.createSubclass(StatusMeterGauge, DvtGauge);


/**
 * Returns a new instance of StatusMeterGauge.
 * @param {dvt.Context} context The rendering context.
 * @param {string} callback The function that should be called to dispatch component events.
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 * @return {StatusMeterGauge}
 */
StatusMeterGauge.newInstance = function(context, callback, callbackObj) {
  var gauge = new StatusMeterGauge();
  gauge.Init(context, callback, callbackObj);
  return gauge;
};
 ...
 * @override
 */
StatusMeterGauge.prototype.Init = function(context, callback, callbackObj) {
  StatusMeterGauge.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @override
 */
StatusMeterGauge.prototype.SetOptions = function(options) {
  // NOTE: This extra clone should be removed once we stop supporting the deprecated attrs
  options = JsonUtils.clone(options);
  ...
  // Combine the user options with the defaults and store
  StatusMeterGauge.superclass.SetOptions.call(this, this.Defaults.calcOptions(options));
};
 ...
 * @override
 */
StatusMeterGauge.prototype.Render = function(container, width, height)
{
 ...
 * @override
 */
StatusMeterGauge.prototype.getGaugeType = function() {
  return 'statusMeter';
};
 ...
 * @override
 */
StatusMeterGauge.prototype.CreateAnimationOnDisplay = function(objs, animationType, animationDuration) {
  var animatedObjs = [];
  for (var i = 0; i < objs.length; i++) {
    var obj = objs[i];
    var endState = obj.getAnimationParams();
 ...
 * @override
 */
StatusMeterGauge.prototype.GetValueAt = function(x, y) {
...
export { DialGauge, LedGauge, RatingGauge, StatusMeterGauge };
