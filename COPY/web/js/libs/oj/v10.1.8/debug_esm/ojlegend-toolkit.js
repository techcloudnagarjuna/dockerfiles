...
const DvtLegendUtils = {
   ...
   * @return {String}
   */
  getItemCategory: function (item, legend) {
    var category = null;
   ...
   */
  isCategoryHidden: function (category, legend) {
   ...
   */
  isSectionCollapsed: function (section, legend) {
...
Obj.createSubclass(DvtLegendObjPeer, Obj);
...
DvtLegendObjPeer.prototype.showKeyboardFocusEffect = function() {
  ...
  if (this._displayables[0]) {
    if (this._displayables[0] instanceof IconButton)
      ...
      this._displayables[0].setSolidStroke(Agent.getFocusColor());
  }
};
 ...
 * @override
 */
DvtLegendObjPeer.prototype.showHoverEffect = function() {
  if (this._displayables[0]) {
 ...
 * @override
 */
DvtLegendObjPeer.prototype.hideHoverEffect = function() {
  if (this._displayables[0]) {
    if (this._displayables[0] instanceof Rect){
...
Obj.createSubclass(DvtLegendAutomation, Automation);
...
Obj.createSubclass(DvtLegendDefaults, BaseComponentDefaults);
...
DvtLegendDefaults.SKIN_ALTA = {
  ...
  'borderColor': null,
  'textStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_11 + 'color: #333333;'),
  'titleStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_11 + 'color: #737373;'),
  '_sectionTitleStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_11 + 'color: #737373;'),
  'titleHalign': 'start',
  'hiddenCategories': [],
...
DvtLegendDefaults.getGapSize = function(legend, defaultSize) {
  ...
  var scalingFactor = Math.min(TextUtils.getTextStringHeight(legend.getCtx(), legend.getOptions()['textStyle']) / 14, 1);
...
Obj.createSubclass(DvtLegendEventManager, EventManager);
...
DvtLegendEventManager.prototype.processHideShowEvent = function(obj) {
  ...
  for (var i = 0; i < displayables.length; i++) {
    ...
    if (displayable instanceof SimpleMarker) // setHollow is a toggle
      displayable.setHollow(obj.getColor());
    else if (displayable instanceof Rect)
      obj.updateAriaLabel();
  }
...
DvtLegendEventManager.prototype.ProcessRolloverEvent = function (event, obj, bOver) {
  ...
  if (obj.getDisplayables && obj.getDisplayables()[0] instanceof IconButton)
    return;
...
DvtLegendEventManager.prototype.toggleSectionCollapse = function(event, sectionIdArray) {
  ...
  if (isExpand != null) {
    var event = new EventFactory.newExpandCollapseEvent(isExpand ? 'expand' : 'collapse',
      section.id,
      section,
...
DvtLegendEventManager.prototype.GetDragDataContexts = function(bSanitize) {
  ...
  if (obj instanceof DvtLegendObjPeer) {
    ...
    if (bSanitize) {
      dataContext = JsonUtils.clone(dataContext, null, {component: true, componentElement: true});
...
Obj.createSubclass(DvtLegendKeyboardHandler, KeyboardHandler);
...
DvtLegendKeyboardHandler.prototype.processKeyDown = function(event) {
  ...
  var isButton = currentNavigable && currentNavigable.getDisplayables()[0] instanceof IconButton;
  ...
  if (currentNavigable == null && keyCode == KeyboardEvent.TAB) {
    ...
    else if (isButton && (keyCode == KeyboardEvent.LEFT_ARROW || keyCode == KeyboardEvent.RIGHT_ARROW)) {
      this._eventManager.onCollapseButtonClick(event, currentNavigable.getDisplayables()[0]);
      EventManager.consumeEvent(event);
    }
    else
...
Obj.createSubclass(DvtLegendRenderer, Obj);
...
DvtLegendRenderer.render = function(legend, availSpace) {
  ...
  var totalDim = DvtLegendRenderer._renderContents(legend, contentContainer, new Rectangle(availSpace.x, availSpace.y, availSpace.w, availSpace.h));
  ...
  for (var i = 0; i < titles.length; i++)
    LayoutUtils.align(totalDim, titles[i].halign, titles[i].text, titles[i].text.getDimensions().w);
...
DvtLegendRenderer._renderBackground = function(legend, availSpace) {
  ...
  if (backgroundColor || borderColor || Object.keys(legendDrop).length > 0 || Object.keys(legendDrag).length > 0) {
    var rect = new Rect(legend.getCtx(), availSpace.x, availSpace.y, availSpace.w, availSpace.h);
...
DvtLegendRenderer._renderTitle = function(legend, container, titleStr, availSpace, section, isAligned, id, button) {
  ...
  if (section) {
    ...
    titleStyle = section['titleStyle'] ?  defaultStyle.merge(new CSSStyle(section['titleStyle'])): defaultStyle;
  }
  ...
  if (TextUtils.fitText(title, availSpace.w, Infinity, container)) {
    ...
    if (!options['isLayout']) {
      ...
      legend.getEventManager().associate(title, new SimpleObjPeer(title.getUntruncatedTextString(), null, null, params));
...
DvtLegendRenderer._createButton = function(context, legend, item, resources, prefix, x, y, tooltip, id, callback, callbackObj) {
  ...
  var button = new IconButton(context, 'borderless', {style: iconStyle, size: DvtLegendRenderer._BUTTON_SIZE}, null, id, callback, callbackObj);
...
DvtLegendRenderer._renderVerticalSection = function(legend, container, section, availSpace, rowHeight, id, minimizeNumRows) {
  ...
  if (isCollapsible) {
    ...
    buttonDim = new Rectangle(buttonX, sectionSpace.y, DvtLegendRenderer._BUTTON_SIZE, DvtLegendRenderer._BUTTON_SIZE);
  ...
  var titleDim = title ? title.getDimensions() : new Rectangle(isRTL ? sectionSpace.x + sectionSpace.w : sectionSpace.x, sectionSpace.y, 0, 0);
  ...
  var contentDim = new Rectangle(isRTL ? sectionSpace.x + sectionSpace.w - contentWidth : sectionSpace.x, sectionSpace.y, contentWidth, contentHeight);
...
DvtLegendRenderer._renderHorizontalSection = function(legend, container, section, availSpace, rowHeight) {
  ...
  var titleDim = title ? title.getDimensions() : new Rectangle(isRTL ? availSpace.x + availSpace.w : availSpace.x, availSpace.y, 0, 0);
  if (!hasItems)
    return titleDim;
  ...
  for (i = 0; i < numItems; i++) {
    ...
    textWidth = Math.ceil(TextUtils.getTextStringWidth(legend.getCtx(), item['text'], options['textStyle']));
  ...
  var sectionDim = new Rectangle(isRTL ? availSpace.x + availSpace.w - totalWidth : availSpace.x, availSpace.y, totalWidth, Math.max(rowHeight, titleDim.h));
...
DvtLegendRenderer._calcColumns = function(legend, availSpace, rowHeight, items, minimizeNumRows) {
  ...
  var textWidth = TextUtils.getMaxTextStringWidth(legend.getCtx(), itemTexts, options['textStyle']);
...
DvtLegendRenderer._createImage = function(legend, x, y, symbolWidth, symbolHeight, rowHeight, item) {
  ...
  return new ImageMarker(context, imageX, imageY, symbolWidth, symbolHeight, null, item['source']);
};
...
DvtLegendRenderer._createMarker = function(legend, cx, cy, symbolWidth, symbolHeight, item) {
  ...
  if (pattern && (pattern != 'none')) {
    ...
    legendMarker = new SimpleMarker(context, shape, 0, 0, symbolWidth, symbolHeight, null, null, true);
    legendMarker.setFill(new PatternFill(pattern, color, '#FFFFFF'));
    legendMarker.setTranslate(cx, cy);
  }
  else {
    legendMarker = new SimpleMarker(context, shape, cx, cy, symbolWidth, symbolHeight, null, null, true);
...
Obj.createSubclass(Legend, BaseComponent);
 ...
 * @return {Legend}
 */
Legend.newInstance = function(context, callback, callbackObj) {
 ...
 */
Legend.getDefaults = function(skin)
{
 ...
 * @protected
 */
Legend.prototype.Init = function(context, callback, callbackObj) {
  Legend.superclass.Init.call(this, context, callback, callbackObj);
 ...
 */
Legend.prototype.getPreferredSize = function(options, maxWidth, maxHeight) {
 ...
 * @override
 */
Legend.prototype.render = function(options, width, height)
{
 ...
 * @override
 */
Legend.prototype.highlight = function(categories) {
 ...
 */
Legend.prototype.processEvent = function(event, source) {
  var type = event['type'];
 ...
 */
Legend.prototype.__registerObject = function(peer) {
 ...
 * @return {array}
 */
Legend.prototype.__getObjects = function() {
  return this._peers;
};
 ...
 * @return {array}
 */
Legend.prototype.__getKeyboardObjects = function() {
 ...
 * @param {Object} bounds
 */
Legend.prototype.__setBounds = function(bounds) {
 ...
 */
Legend.prototype.__getBounds = function() {
  return this._bounds;
};
 ...
 */
Legend.prototype.__registerTitle = function(title) {
 ...
 */
Legend.prototype.__getTitles = function() {
  return this._titles;
};
 ...
 */
Legend.prototype.getAutomation = function() {
 ...
 */
Legend.prototype.getKeyboardFocus = function() {
 ...
 */
Legend.prototype.setKeyboardFocus = function(navigable, isShowingFocusEffect) {
  if (this.EventManager == null)
    return;
 ...
 * @override
 */
Legend.prototype.getDimensions = function(targetCoordinateSpace) {
 ...
 */
Legend.prototype.getContentDimensions = function(targetCoordinateSpace) {
  if (!targetCoordinateSpace || targetCoordinateSpace === this)
    return this._contentDimensions;
  else { // Calculate the bounds relative to the target space
 ...
 * @private
 */
Legend.prototype._transferVisibilityProperties = function(sections) {
  if (!sections || sections.length <= 0)
    return;
 ...
 * @override
 */
Legend.prototype.UpdateAriaAttributes = function() {
 ...
 * @return {boolean}
 */
Legend.prototype.isNavigable = function() {
 ...
 */
Legend.getItemCount = function(legend) {
  ...
  for (var i = 0; i < sections.length; i++) {
    var section = sections[i];
    itemsCount += Legend.getSectionItemsCount(section);
  }
 ...
 */
Legend.getSectionItemsCount = function(section) {
  var itemsCount = 0;
  if (section.items) {
