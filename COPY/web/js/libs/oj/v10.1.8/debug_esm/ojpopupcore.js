...
PositionUtils.isWithinViewport = function (element) {
  ...
  function isPositioned(_element) {
    return ['fixed', 'absolute', 'relative'].indexOf(_element.css('position')) > -1 &&
           (Math.abs(getCSSLengthAsInt(_element.css('top'))) > 0 ||
            Math.abs(getCSSLengthAsInt(_element.css('bottom'))) > 0 ||
            Math.abs(getCSSLengthAsInt(_element.css('left'))) > 0 ||
            Math.abs(getCSSLengthAsInt(_element.css('right'))) > 0);
  }
...
class VPopup extends VComponent {
    constructor(props) {
        super(props);
        this._setRootRef = this._setRootRef.bind(this);
    }
    render() {
        return (h("div", { style: { display: 'none' }, ref: this._setRootRef }, this.props.children));
    }
    mounted() {
