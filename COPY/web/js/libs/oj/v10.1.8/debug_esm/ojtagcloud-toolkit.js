...
Obj.createSubclass(DvtTagCloudKeyboardHandler, KeyboardHandler);
 ...
 */
var DvtTagCloudObjPeer$1 = function(tagCloud, displayable, data) {
  this.Init(tagCloud, displayable, data);
};

Obj.createSubclass(DvtTagCloudObjPeer$1, Obj);
...
DvtTagCloudObjPeer$1.prototype.Init = function(tagCloud, displayable, data) {
  ...
  if (data['url']) {
    ...
    this._linkCallback = ToolkitUtils.getLinkCallback('_blank', data['url']);
  } else {
...
DvtTagCloudObjPeer$1.prototype.getDatatipColor = function() {
  return this._displayable.getItemStyle().getStyle(CSSStyle.COLOR);
};
...
Obj.createSubclass(DvtTagCloudAutomation, Automation);
...
Obj.createSubclass(DvtTagCloudDefaults, BaseComponentDefaults);
...
DvtTagCloudDefaults.VERSION_1 = {
  ...
  'layout': 'rectangular',
  'selectionMode': 'none',
  '_statusMessageStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA + 'color: #333333;'),
  'styleDefaults': {
    'animationDuration' : 500,
    'hoverBehaviorDelay' : 200,
    '_style' : new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA + 'color: #333333;')
  },
  'touchResponse' : 'auto'
};
...
Obj.createSubclass(DvtTagCloudEventManager, EventManager);
...
Obj.createSubclass(DvtTagCloudStyleUtils, Obj);
...
Obj.createSubclass(DvtTagCloudItem, BackgroundOutputText);
...
DvtTagCloudItem.prototype.animateUpdate = function(handler, oldObj) {
  var anim = new CustomAnimation(this.getCtx(), this, DvtTagCloudStyleUtils.getAnimationDuration(this._tagCloud));
  var style = this.getCSSStyle();
  var oldStyle = oldObj.getCSSStyle();
  var bSizeChange = false;

  // Color change
  var startColor = oldStyle.getStyle(CSSStyle.COLOR);
  var endColor = style.getStyle(CSSStyle.COLOR);
  if (startColor != endColor) {
    var tag = this;
    var getColorStyle = function() {
      return tag.getCSSStyle().getStyle(CSSStyle.COLOR);
    };
    var setColorStyle = function(value) {
      tag.setCSSStyle(tag.getCSSStyle().setStyle(CSSStyle.COLOR, value));
    };
    this.setCSSStyle(style.setStyle(CSSStyle.COLOR, startColor));
    anim.getAnimator().addProp(Animator.TYPE_COLOR, this, getColorStyle, setColorStyle, endColor);
  }

  // Size change
  var startSize = parseFloat(oldStyle.getStyle(CSSStyle.FONT_SIZE));
  var endSize = parseFloat(style.getStyle(CSSStyle.FONT_SIZE));
  if (startSize != endSize) {
    bSizeChange = true;
    var tag = this;
    var getFontSize = function() {
      return parseFloat(tag.getCSSStyle().getStyle(CSSStyle.FONT_SIZE));
    };
    this.setFontSize(startSize);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this, getFontSize, this.setFontSize, endSize);
  }
  ...
  if (endX != startX || (bSizeChange && endAlign != startAlign)) {
    ...
    this.setX(startX);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this, this.getX, this.setX, endX);
  }
  ...
  if (endY != startY) {
    this.setY(startY);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this, this.getY, this.setY, endY);
  }
...
DvtTagCloudItem.prototype.animateDelete = function(handler) {
  handler.add(new AnimFadeOut(this.getCtx(), this, DvtTagCloudStyleUtils.getAnimationDuration(this._tagCloud)), DvtTagCloudItem.ANIMATION_DELETE_PRIORITY);
};
...
DvtTagCloudItem.prototype.animateInsert = function(handler) {
  this.setAlpha(0);
  handler.add(new AnimFadeIn(this.getCtx(), this, DvtTagCloudStyleUtils.getAnimationDuration(this._tagCloud)), DvtTagCloudItem.ANIMATION_INSERT_PRIORITY);
};
...
DvtTagCloudItem.prototype._createFeedbackStyles = function(style) {
  ...
  var color = this._normalStyle.getStyle(CSSStyle.COLOR);
  this._normalStyle.setStyle(CSSStyle.BACKGROUND_COLOR, null);
  ...
  this._hoverStyle.setStyle(CSSStyle.BACKGROUND_COLOR, hoverColor);
  this._hoverStyle.setStyle(CSSStyle.COLOR, ColorUtils.getContrastingTextColor(hoverColor));

  this._selectedStyle = this._normalStyle.clone();
  this._selectedStyle.setStyle(CSSStyle.BACKGROUND_COLOR, color);
  this._selectedStyle.setStyle(CSSStyle.COLOR, ColorUtils.getContrastingTextColor(color));
  ...
  this._hoverSelectedStyle.setStyle(CSSStyle.BACKGROUND_COLOR, hoverSelectedColor);
  this._hoverSelectedStyle.setStyle(CSSStyle.COLOR, ColorUtils.getContrastingTextColor(hoverSelectedColor));
};
...
DvtTagCloudItem._adjustX = function(tag, x, hAlign) {
  ...
  if (hAlign == OutputText.H_ALIGN_LEFT)
    return x + dims.w;
...
DvtTagCloudItem._lightenColor = function(color, opacity) {
  ...
  return ColorUtils.makeRGB(Math.floor(lighterR), Math.floor(lighterG), Math.floor(lighterB));
};
...
Obj.createSubclass(DvtTagCloudLayoutUtils, Obj);
...
DvtTagCloudLayoutUtils.cloudLayout = function(availSpace, items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    maxFontSize = Math.max(maxFontSize, parseFloat(tag.getCSSStyle().getStyle(CSSStyle.FONT_SIZE)));
    ...
    while (!placed) {
      ...
      var rect = new Rectangle(x, y, dims.w, dims.h);
      placed = true;
  ...
  for (var j = 0; j < items.length; j++) {
    ...
    var fontSize = parseFloat(tag.getCSSStyle().getStyle(CSSStyle.FONT_SIZE));
...
DvtTagCloudLayoutUtils.rectangleLayout = function(availSpace, items, isBidi) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    maxFontSize = Math.max(maxFontSize, parseFloat(tag.getCSSStyle().getStyle(CSSStyle.FONT_SIZE)));
    arDims.push(new Dimension(dims.w, dims.h));
  }
...
Obj.createSubclass(DvtTagCloudRenderer, Obj);
...
DvtTagCloudRenderer._renderEmptyText = function(tagCloud, container, availSpace) {
  ...
  tagCloud.renderEmptyText(container, emptyTextStr,
...
DvtTagCloudRenderer._renderBackground = function(tagCloud, container, availSpace) {
  ...
  var rect = new Rect(tagCloud.getCtx(), availSpace.x, availSpace.y, availSpace.w, availSpace.h);
...
DvtTagCloudRenderer._renderItems = function(tagCloud, container, availSpace) {
  ...
  if (defaultStyle && !(defaultStyle instanceof Object))
    defaultStyle = CSSStyle.cssStringToObject(defaultStyle);
  ...
  for (var i = 0; i < dataItems.length; i++) {
    ...
    itemStyle = JsonUtils.merge(itemStyle, defaultStyle);
    if (itemStyle) {
      ...
      for (var index = 0; index < textProperties.length; index++) {
        ...
        var attribute = CSSStyle.cssStringToObjectProperty(textProperties[index]);
    ...
    if (color)
      style.setStyle(CSSStyle.COLOR, color);
    style.setStyle(CSSStyle.FONT_SIZE, fontSizeFunction.call(null, data['value']).toString());
    ...
    if (peer.isSelectable() || data['url'])
      item.setCursor(SelectionEffectUtils.getSelectingCursor());
  ...
  if (items.length > 0) {
    if (options['layout'] === 'cloud')
      ...
      DvtTagCloudLayoutUtils.rectangleLayout(availSpace, items, Agent.isRightToLeft(tagCloud.getCtx()));
  }
  return items;
};
...
DvtTagCloudRenderer._renderLegend = function(tagCloud, container, availSpace) {
  ...
  if (legendData && legendData['sections']) {
    var legendOptions = JsonUtils.clone(legendData);
    ...
    var legend = Legend.newInstance(tagCloud.getCtx(), tagCloud.processEvent, tagCloud);
    ...
    LayoutUtils.position(availSpace, 'bottom', legend, preferredSize.w, preferredSize.h, 0);
 ...
 */
const TagCloud = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(TagCloud, BaseComponent);
...
TagCloud.prototype.render = function(options, width, height) {
  ...
  this._container = new Container(this.getCtx());
  this.addChild(this._container);
  DvtTagCloudRenderer.render(this, this._container, new Rectangle(0, 0, this.Width, this.Height));
  ...
  if (!this._oldContainer) {
    if (this.Options['animationOnDisplay'] !== 'none') {
      this.Animation = BlackBoxAnimationHandler.getInAnimation(this.getCtx(), BlackBoxAnimationHandler.ALPHA_FADE, this._container, bounds, animationDuration);
    }
  }
    ...
    this._deleteContainer = new Container(this.getCtx());
    this.addChild(this._deleteContainer);
    var ah = new DataAnimationHandler(this.getCtx(), this._deleteContainer);
...
TagCloud.prototype.highlight = function(categories) {
  ...
  CategoryRolloverHandler.highlight(categories, this.getObjects(), this.Options['highlightMatch'] === 'any');
