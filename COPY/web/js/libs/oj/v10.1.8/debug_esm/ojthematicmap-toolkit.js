 ...
 * @ignore
 */
import { Obj, Rectangle, Point, PathUtils, Container, Shape, Path, Image, Stroke, OutputText, Polyline, CSSStyle, Matrix, EventFactory, DataAnimationHandler, BlackBoxAnimationHandler, Rect, ParallelPlayable, DisplayableUtils, SolidFill, BaseComponent, Agent, KeyboardEvent, Displayable, SelectionEffectUtils, MouseEvent, KeyboardHandler, SimpleMarker, CustomAnimation, Animator, AnimFadeOut, AnimFadeIn, Automation, BaseComponentDefaults, JsonUtils, EventManager, SelectionHandler, ArrayUtils, ImageMarker, ImageLoader, ColorUtils, LayoutUtils, CategoryRolloverHandler, ToolkitUtils } from 'ojs/ojdvt-toolkit';
import { PanZoomCanvasEventManager, PanZoomCanvasKeyboardHandler, PanZoomComponent } from 'ojs/ojdvt-panzoomcanvas';
...
if (null) {
  if (null['DvtBaseMapManager']) {
    DvtBaseMapManager = null['DvtBaseMapManager'];
  } else {
    DvtBaseMapManager = {};
    null['DvtBaseMapManager'] = DvtBaseMapManager;
...
Obj.createSubclass(DvtBaseMapManager, Obj);
/** @const */
...
DvtBaseMapManager.getBaseMapDim = function(baseMapName, layerName) {
  ...
  if (layer) {
    ...
    if (dimAr)
      return new Rectangle(dimAr[0], dimAr[1], dimAr[2], dimAr[3]);
  }
  return null;
};
  ...
  if (zoomFactor > 0) {
    ...
    if (paths) {
      for (var path in paths) {
        ...
        if (isNaN(pathAr))
          pathAr = PathUtils.createPathArray(paths[path]);
...
Obj.createSubclass(DvtMapArea, Container);
...
DvtMapArea.prototype.HandleZoomEvent = function(pzcMatrix) {
  if (!this._bSupportsVectorEffects && this._shape && this._areaStrokeWidth) {
    ...
    var zoomStroke = new Stroke(copyStroke.getColor(), copyStroke.getAlpha(),
...
Obj.createSubclass(DvtMapLabel, OutputText);
    ...
    if (state == -1) {
      ...
      if (this._leaderLines) {
        ...
        if (state > 0) {
          ...
          // reset label color
          var style = this.getCSSStyle();
          style.setStyle(CSSStyle.COLOR, this._labelColor);
          this.setCSSStyle(style);
        }
      }
    }
  ...
  if (this._currentState != -1) {
    ...
    if (this._leaderLine) {
      ...
      if (!this._bSupportsVectorEffects) {
        ...
        var stroke = new Stroke(copyStroke.getColor(), copyStroke.getAlpha(), (1 / zoom), copyStroke.isFixedWidth());
...
Obj.createSubclass(DvtMapLayer, Obj);
...
DvtMapLayer.prototype.updateDataLayer = function(dataLayer, pzcMatrix, topLayerName) {
  ...
  if (this._oldDataLayer) {
    ...
    if (anim == 'auto') { // data change animation
    ...
    else if (BlackBoxAnimationHandler.isSupported(anim)) { // black box animation
      ...
      for (var i = 0; i < oldNonScaledContainers.length; i++) {
        var rect = new Rect(this._tmap.getCtx(), 0, 0, this._tmap.getWidth(), this._tmap.getHeight());
        rect.setFill(null);
      ...
      var anim1 = BlackBoxAnimationHandler.getCombinedAnimation(this._tmap.getCtx(), anim,
          oldNonScaledContainers,
          newNonScaledContainers, bounds1, animDur);

      var bounds2 = new Rectangle(0, 0, this._tmap.getWidth() / this.PzcMatrix.getA(), this._tmap.getHeight() / this.PzcMatrix.getA());
      var oldScaledContainers = this._oldDataLayer.getScaledContainers();
      for (var i = 0; i < oldScaledContainers.length; i++) {
        var rect = new Rect(this._tmap.getCtx(), 0, 0, this._tmap.getWidth() / this.PzcMatrix.getA(), this._tmap.getHeight() / this.PzcMatrix.getA());
        rect.setFill(null);
      ...
      var anim2 = BlackBoxAnimationHandler.getCombinedAnimation(this._tmap.getCtx(), anim,
          oldScaledContainers,
...
Obj.createSubclass(DvtMapAreaLayer, DvtMapLayer);
...
DvtMapAreaLayer.prototype.getLayerDim = function() {
  if (!this._layerDim) {
    ...
    if (this._isolatedArea) {
      ...
      if (!basemap)
        this._layerDim = DisplayableUtils.getDimensionsForced(this._tmap.getCtx(), this.getAreaShape(this._isolatedArea));
      else
        this._layerDim = PathUtils.getDimensions(PathUtils.createPathArray(DvtBaseMapManager.getPathForArea(this._tmap.getMapName(), this.LayerName, this._isolatedArea)));
    } else {
        ...
        if (this._labelDisplay != 'off' && this.AreaLabels) {
          ...
          if (labelText) {
            if (this._labelInfo && this._labelInfo[area])
              ...
              var areaDim = DisplayableUtils.getDimensionsForced(this._tmap.getCtx(), areaShape);
...
DvtMapAreaLayer.prototype.__showDropSiteFeedback = function(obj) {
  ...
  if (obj) {
    ...
    if (this._dropSiteFeedback) {
      this._dropSiteFeedback.setFill(new SolidFill(this._dropSiteCSSStyle.getStyle(CSSStyle.BACKGROUND_COLOR)));
      var strokeWidth = this._dropSiteCSSStyle.getStyle(CSSStyle.BORDER_WIDTH) ?
          this._dropSiteCSSStyle.getStyle(CSSStyle.BORDER_WIDTH).substring(0, this._dropSiteCSSStyle.getStyle(CSSStyle.BORDER_WIDTH).indexOf('px')) : 1;
      if (!this._tmap.supportsVectorEffects())
        strokeWidth /= this.PzcMatrix.getA();
      var stroke = new Stroke(this._dropSiteCSSStyle.getStyle(CSSStyle.BORDER_COLOR), 1, strokeWidth, this._tmap.supportsVectorEffects());
...
Obj.createSubclass(DvtCustomDataItem, Container);
...
DvtCustomDataItem.prototype.Init = function(context, dataItem, styles) {
  ...
  var hos = new Stroke('rgb(235, 236, 237)', 1, 2);
...
DvtCustomDataItem.prototype.updateRootElement = function(rootContent, shouldRemoveOldContent) {
  ...
  if (rootContent instanceof BaseComponent)
...
const DvtThematicMapUtils = {
   ...
   * @return {string}
   */
  calcCurve: function (dataLayer, origStartPt, origEndPt, startPt, endPt) {
    var x1 = startPt.x;
    var y1 = startPt.y;
    var x2 = endPt.x;
    var y2 = endPt.y;
   ...
   */
  getFirstNavigableLink: function (marker, event, listOfLinks) {
    ...
    for (var i = 0; i < listOfLinks.length; i++) {
      ...
      if ((direction == KeyboardEvent.OPEN_ANGLED_BRACKET && linkPt.x <= markerPt.x) ||
...
Obj.createSubclass(DvtMapObjPeer, Obj);
...
DvtMapObjPeer.prototype.setSelectable = function(bSelectable) {
  ...
  if (this.Displayable.setSelectable) {
    ...
    if (label && bSelectable)
      label.setCursor(SelectionEffectUtils.getSelectingCursor());
  }
};
...
DvtMapObjPeer.prototype.positionLabel = function() {
  if (this._label) {
    ...
    var markerType = this.Displayable instanceof SimpleMarker ? this.Displayable.getType() : 'image';
...
DvtMapObjPeer.prototype.animateUpdate = function(handler, oldObj) {
  var anim = new CustomAnimation(this._view.getCtx(), this.Displayable, this.getDataLayer().getAnimationDuration());
    ...
    if (endFill instanceof SolidFill && !endFill.equals(startFill)) {
      ...
      if (oldObj.getLabel() && this._label) {
        ...
        if (!endLabelFill.equals(this._label.getFill()))
          anim.getAnimator().addProp(Animator.TYPE_FILL, this._label, this._label.getFill, this._label.setFill, endLabelFill);
      }
      anim.getAnimator().addProp(Animator.TYPE_FILL, this.Displayable, this.Displayable.getFill, this.Displayable.setFill, endFill);
    }
  }
  ...
  if (this.Displayable.getCenterDimensions) {
    ...
    if (startRect.x != endRect.x || startRect.y != endRect.y || startRect.w != endRect.w || startRect.h != endRect.h) {
      ...
      anim.getAnimator().addProp(Animator.TYPE_RECTANGLE, this.Displayable, this.Displayable.getCenterDimensions, this.Displayable.setCenterDimensions, endRect);
    }
  }
  ...
  if (startRotation != endRotation) {
    ...
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this.Displayable, this.Displayable.getRotation, this.Displayable.setRotation, endRotation);
  }
  ...
  if (startCenter && endCenter) {
    if (startCenter.x != endCenter.x || startCenter.y != endCenter.y || startRotation != endRotation) {
      ...
      anim.getAnimator().addProp(Animator.TYPE_POINT, this, this.getCenter, this._setCenter, new Point(endCenter.x, endCenter.y));
    }
  }
  ...
  if (this._label && oldObj.getLabel()) {
    ...
    if (startLabelX != endLabelX) {
      ...
      anim.getAnimator().addProp(Animator.TYPE_NUMBER, this._label, this._label.getX, this._label.setX, endLabelX);
    }
    var startLabelY = oldObj.getLabel().getY();
    var endLabelY = this._label.getY();
    if (startLabelY != endLabelY) {
      this._label.setY(startLabelY);
      anim.getAnimator().addProp(Animator.TYPE_NUMBER, this._label, this._label.getY, this._label.setY, endLabelY);
    }
    // Hide old label
...
DvtMapObjPeer.prototype.animateDelete = function(handler, container) {
  ...
  if (label)
    fadeObjs.push(label);
  var anim = new AnimFadeOut(this._view.getCtx(), fadeObjs, this.getDataLayer().getAnimationDuration());
...
DvtMapObjPeer.prototype.animateInsert = function(handler) {
  ...
  var anim = new AnimFadeIn(this._view.getCtx(), fadeObjs, this.getDataLayer().getAnimationDuration());
...
Obj.createSubclass(DvtMapAreaPeer, DvtMapObjPeer);
...
Obj.createSubclass(DvtThematicMapAutomation, Automation);
...
Obj.createSubclass(DvtThematicMapDefaults, BaseComponentDefaults);
...
DvtThematicMapDefaults.prototype.calcAreaLayerOptions = function(userOptions, isCustomElement) {
  ...
  if (isCustomElement)
    defaultObj = JsonUtils.merge({'labelDisplay': 'off'}, defaultObj);
...
Obj.createSubclass(DvtThematicMapEventManager, EventManager);
...
DvtThematicMapEventManager.prototype.ProcessKeyboardEvent = function(event) {
  ...
  // Zoom to fit
  else if ((keyCode == KeyboardEvent.ZERO || keyCode == KeyboardEvent.NUMPAD_ZERO) && event.ctrlKey) {
    if (event.altKey)
...
DvtThematicMapEventManager.prototype.OnComponentTouchClick = function(event) {
  var consumed = this.GetEventInfo(event, PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY);
  if (consumed)
    return;
...
Obj.createSubclass(DvtMapDataLayer, Obj);
...
DvtMapDataLayer.prototype.getNavigableLinksForNodeId = function(markerId) {
  ...
  for (var i = 0; i < this._dataLinkCollection.length; i++) {
    ...
    if (startMarker && Obj.compareValues(ctx, startMarker.getId(), markerId))
      links.push(link);
    else if (endMarker && Obj.compareValues(ctx, endMarker.getId(), markerId))
      links.push(link);
  }
  return links;
};
...
Obj.createSubclass(DvtMapLinkPeer, DvtMapObjPeer);
...
DvtMapLinkPeer.prototype.animateUpdate = function(handler, oldObj) {
  ...
  if (endStroke.getColor() != startStroke.getColor()) {
    ...
    anim.getAnimator().addProp(Animator.TYPE_STROKE, this.Displayable, this.Displayable.getStroke, this.Displayable.setStroke, endStroke);
  }
  ...
  if (!origStart.equals(start) || !origEnd.equals(end)) {
    fadeOld = false;
    this.Displayable.setAlpha(0);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this.Displayable, this.Displayable.getAlpha, this.Displayable.setAlpha, 1);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, oldDisplayable, oldDisplayable.getAlpha, oldDisplayable.setAlpha, 0);
  }
...
Obj.createSubclass(DvtSelectablePath, Path);
...
DvtSelectablePath.prototype.CreateSelectedHoverStrokes = function() {
  if (!this.SelectedHoverInnerStroke) {
    this.SelectedHoverInnerStroke = new Stroke(this.HoverInnerStroke.getColor(), this.HoverInnerStroke.getAlpha(),
      DvtSelectablePath.HOVER_STROKE_WIDTH, this._bSupportsVectorEffects);
  }
  if (!this.SelectedHoverOuterStroke) {
    this.SelectedHoverOuterStroke = new Stroke(this.SelectedOuterStroke.getColor(), this.SelectedOuterStroke.getAlpha(),
...
DvtSelectablePath.prototype._updateStrokeZoomWidth = function(shape, fixedWidth) {
  if (!this._bSupportsVectorEffects) {
    ...
    if (stroke) {
      var updatedStroke = new Stroke(stroke.getColor(), stroke.getAlpha(), fixedWidth / this.Zoom, stroke.isFixedWidth());
...
DvtSelectablePath.prototype._adjustStrokeZoomWidth = function(stroke, fixedWidth) {
  if (!this._bSupportsVectorEffects) {
    return new Stroke(stroke.getColor(), stroke.getAlpha(), fixedWidth / this.Zoom, stroke.isFixedWidth());
  }
  return stroke;
};
...
Obj.createSubclass(DvtThematicMapProjections, Obj);
/**  @const  @private */
DvtThematicMapProjections._VIEWPORT_BOUNDS = new Rectangle(0, 0, 800, 500);
/**  @const  @private */
DvtThematicMapProjections._RADIUS = 6378206.4;
/**  @const  @private */
DvtThematicMapProjections._NEW_ZEALAND_RECT = new Rectangle(500, 200, 200, 200);
/**  @const  @private */
DvtThematicMapProjections._NEW_ZEALAND_BOUNDS = new Rectangle(163, - 49, 17, 17);
/**  @const  @private */
DvtThematicMapProjections._AFRICA_BOUNDS = new Rectangle(- 17.379205428479874, - 37.201510854305546, 68.66391442808313, 77.50071544582713);
/**  @const  @private */
DvtThematicMapProjections._ASIA_BOUNDS = new Rectangle(- 0.8436866097568272, - 0.7626456732012923, 1.8336308036296942, 1.5748427214611724);
/**  @const  @private */
DvtThematicMapProjections._AUSTRALIA_BOUNDS = new Rectangle(113.29667079927977, - 52.89550592498755, 65.25257389065216, 42.123114617504626);
/**  @const  @private */
DvtThematicMapProjections._EUROPE_BOUNDS = new Rectangle(- 0.47944476148667076, - 0.0014669405958800579, 0.7364925893845453, 0.6293972741802124);
/**  @const  @private */
DvtThematicMapProjections._N_AMERICA_BOUNDS = new Rectangle(- 0.6154469465354344, - 0.24589767758847714, 1.2448236795108683, 1.2631535127174947);
/**  @const  @private */
DvtThematicMapProjections._S_AMERICA_BOUNDS = new Rectangle(- 80.60817722658722, - 60.796273249672765, 46.608687602908056, 66.96595767361796);
/**  @const  @private */
DvtThematicMapProjections._APAC_BOUNDS = new Rectangle(68.20516856593524, - 52.89892708045518, 111.65739821771903, 116.55460214469134);
/**  @const  @private */
DvtThematicMapProjections._EMEA_BOUNDS = new Rectangle(- 24.543831069368586, - 37.202500659225905, 204.54283106936856, 164.9634493690208);
/**  @const  @private */
DvtThematicMapProjections._L_AMERICA_BOUNDS = new Rectangle(- 117.12451221229134, - 54.95921623126266, 82.33223251442891, 87.67786623127876);
/**  @const  @private */
DvtThematicMapProjections._USA_CANADA_BOUNDS = new Rectangle(- 0.6154656300926513, 0.0507209798775865, 1.0153104799231851, 0.966537441082997);
/**  @const  @private */
DvtThematicMapProjections._WORLD_BOUNDS = new Rectangle(- 171.9, - 62.6, 349.8, 150.8);
/**  @const  @private */
DvtThematicMapProjections._ALASKA1_RECT = new Rectangle(172, 51, 8, 3);
/**  @const  @private */
DvtThematicMapProjections._ALASKA2_RECT = new Rectangle(- 180, 51, 51, 21);
/**  @const  @private */
DvtThematicMapProjections._HAWAII_RECT = new Rectangle(- 178.5, 18.9, 35, 11);
/**  @const  @private */
DvtThematicMapProjections._USA_RECT = new Rectangle(- 124.8, 24.4, 58, 25.5);
/**  @const  @private */
DvtThematicMapProjections._ALASKA_BOUNDS = new Rectangle(- 187.5517578125, 59.82610321044922, 57.562225341796875, 43.83738708496094);
/**  @const  @private */
DvtThematicMapProjections._HAWAII_BOUNDS = new Rectangle(- 160.23606872558594, 18.91549301147461, 5.4374847412109375, 3.3189010620117188);
/**  @const  @private */
DvtThematicMapProjections._USA_BOUNDS = new Rectangle(- 2386803.25, - 1183550.5, 4514111, 2908402);
/**  @const  @private */
DvtThematicMapProjections._HAWAII_WINDOW = new Rectangle(165.0, 400.0, 100.0, 100.0);
/**  @const  @private */
DvtThematicMapProjections._ALASKA_WINDOW = new Rectangle(-75.0, 350.0, 240.0, 150.0);
/**  @const  @private */
...
DvtThematicMapProjections.project = function(x, y, basemap) {
  ...
  if (point)
    return new Point(point.x * 10, point.y * 10);// multiply by 10 because basemaps are 10x bigger
  else
    return null;
};
...
DvtThematicMapProjections._getUSAProjection = function(x, y) {
  ...
  if (DvtThematicMapProjections._ALASKA1_RECT.containsPoint(x, y) || DvtThematicMapProjections._ALASKA2_RECT.containsPoint(x, y)) {
    ...
    transformedPoint = DvtThematicMapProjections._applyAffineTransform(viewPortTransform, new Point(x, y));
  }
    ...
    transformedPoint = DvtThematicMapProjections._applyAffineTransform(viewPortTransform, DvtThematicMapProjections._getOrthographicProjection(new Point(- 95, 36), x, y));
  }
...
DvtThematicMapProjections._getOrthographicProjection = function(center, x, y) {
  ...
  return new Point(px * DvtThematicMapProjections._RADIUS, py * DvtThematicMapProjections._RADIUS);
};
...
DvtThematicMapProjections._applyAffineTransform = function(matrix, point) {
  return new Point(point.x * matrix.getA() + matrix.getTx(), point.y * matrix.getD() + matrix.getTy());
};
...
DvtThematicMapProjections._getInverseUSAProjection = function(x, y) {
  ...
  if (DvtThematicMapProjections._ALASKA_WINDOW.containsPoint(x, y)) {
    ...
    viewPortTransform = viewPortTransform.invert();
    var point = viewPortTransform.transformPoint(new Point(x, y));
    return DvtThematicMapProjections._getInverseOrthographicProjection(new Point(- 95, 36), point.x, point.y);
  }
  return new Point(x, y);
};
...
DvtThematicMapProjections._getInverseAlbersEqualAreaConicProjection = function(latOfOrigin, lonOfOrigin, sP1, sP2, x, y) {
  ...
  return new Point(DvtThematicMapProjections.toDegrees(px), DvtThematicMapProjections.toDegrees(py));
};
...
Obj.createSubclass(DvtThematicMapJsonParser, Obj);
...
DvtThematicMapJsonParser.prototype.ParseDataLayers = function(dataLayers, parentLayer, topLayerName, isAreaDataLayer) {
  ...
  for (var i = 0; i < dataLayers.length; i++) {
    ...
    if (selectionMode == 'single')
      dataLayer.setSelectionMode(SelectionHandler.TYPE_SINGLE);
    ...
    if (areas) {
      for (var j = 0; j < areas.length; j++) {
        if (hiddenCategories && ArrayUtils.hasAnyItem(hiddenCategories, areas[j]['categories'])) {
        ...
        if (isolatedRowKey) {
          if (!Obj.compareValues(ctx, isolatedRowKey, areas[j]['id']))
            continue;
          else
    ...
    if (markers) {
      ...
      for (var j = 0; j < markers.length; j++) {
        if (hiddenCategories && ArrayUtils.hasAnyItem(hiddenCategories, markers[j]['categories'])) {
          // placeholder null object for automation
        ...
        if (isolatedRowKey) {
          if (!Obj.compareValues(ctx, isolatedRowKey, markers[j]['id']))
            continue;
          else
            isolatedAreaId = areaId;
        }
    ...
    if (images) {
      for (var j = 0; j < images.length; j++) {
        ...
        if (isolatedRowKey) {
          if (!Obj.compareValues(ctx, isolatedRowKey, images[j]['id']))
            continue;
          else
            isolatedAreaId = areaId;
        }
    ...
    if (links) {
      ...
      for (var j = 0; j < links.length; j++) {
        if (hiddenCategories && ArrayUtils.hasAnyItem(hiddenCategories, links[j]['categories'])) {
          // placeholder null object for automation
          dataLayer.addAreaObject(null);
          continue;
        }
...
DvtThematicMapJsonParser.prototype._createArea = function(layer, dataLayer, data) {
  ...
  if (areaShape) {
    ...
    var hs = new Stroke(data['hoverColor'], 1, DvtSelectablePath.HOVER_STROKE_WIDTH, this._tmap.supportsVectorEffects());
    var sis = new Stroke(data['selectedInnerColor'], 1, DvtSelectablePath.SELECTED_INNER_STROKE_WIDTH, this._tmap.supportsVectorEffects());
    var sos = new Stroke(data['selectedOuterColor'], 1, DvtSelectablePath.SELECTED_OUTER_STROKE_WIDTH, this._tmap.supportsVectorEffects());
...
DvtThematicMapJsonParser.prototype._createMarker = function(layer, dataLayer, data, isParentAreaDataLayer) {
  ...
  if (this._mapProvider && !data['location'])
    center = new Point(data['x'], -data['y']);
  else
  ...
  var marker;
  if (data['source']) {
    marker = new ImageMarker(this._tmap.getCtx(), center.x, center.y, width, height, br,
    ...
    marker = new SimpleMarker(this._tmap.getCtx(), shapeType, center.x, center.y, width, height, br);
  }
...
DvtThematicMapJsonParser.prototype._createLink = function(dataLayer, data, idToMarkerMap, idToAreaMap) {
  ...
  var curve = new Path(this._tmap.getCtx(), DvtThematicMapUtils.calcCurve(dataLayer, startPt, endPt, startPt, endPt));
  curve.setFill(null);
  ...
  var shis = new Stroke(hoverColor, 1, strokeWidth);
  var shos = new Stroke(selectedColor, 1, strokeWidth + 2);
...
DvtThematicMapJsonParser.prototype._createLabel = function(layer, dataLayer, data, displayable, isParentAreaDataLayer) {
  ...
  if (labelText) {
    ...
    if (isArea)
      ...
      label = new OutputText(context, labelText, 0, 0);
    ...
    if (data['color'] && (label instanceof DvtMapLabel && (Agent.isHighContrast() || !fillColor))) {
...
DvtThematicMapJsonParser.prototype._styleDisplayable = function(style, displayable) {
  ...
  if (displayable instanceof SimpleMarker) {
    ...
    if (strokeType != 'none') {
      ...
      var stroke = new Stroke(style['borderColor'], 1, borderWidth,
        !this._tmap.isMarkerZoomBehaviorFixed(), Stroke.getDefaultDashProps(strokeType, borderWidth));
...
DvtThematicMapJsonParser._calcBubbleSizes = function(tmap, markers) {
  ...
  for (var i = 0; i < markers.length; i++) {
    ...
    markers[i]['_size'] = (value == null || value <= 0) ? 0 : LayoutUtils.getBubbleSize(value, minValue, maxValue, DvtThematicMapJsonParser._MIN_MARKER_SIZE, maxSize);
  }
};
...
Obj.createSubclass(DvtThematicMapKeyboardHandler, PanZoomCanvasKeyboardHandler);
...
DvtThematicMapKeyboardHandler.prototype.processKeyDown = function(event) {
  ...
  if (keyCode == KeyboardEvent.CLOSE_BRACKET) {
    ...
    if ((event.keyCode == KeyboardEvent.OPEN_ANGLED_BRACKET || KeyboardEvent.CLOSE_ANGLED_BRACKET) && event.altKey) {
 ...
 * @constructor
 */
const ThematicMap = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(ThematicMap, PanZoomComponent);

/**
 * @const
 */
ThematicMap.DEFAULT_MAX_ZOOM_FACTOR = 6;
 ...
 * @protected
 */
ThematicMap.prototype.Init = function(context, callback, callbackObj) {
  ThematicMap.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @private
 */
ThematicMap.prototype._createLayers = function() {
 ...
 * @private
 */
ThematicMap.prototype._updateAriaForLayers = function() {
 ...
 * @return {ThematicMap}
 */
ThematicMap.newInstance = function(context, callback, callbackObj) {
 ...
 * @override
 */
ThematicMap.prototype.SetOptions = function(options) {
  ThematicMap.superclass.SetOptions.call(this, options);
 ...
 */
ThematicMap.prototype.setStyleDefaults = function(defaults) {
 ...
 */
ThematicMap.prototype.getStyleDefaults = function() {
 ...
 * @return {number}
 */
ThematicMap.prototype.getMaxZoomFactor = function() {
  ...
  if (!this._zooming) {
    ...
    return ThematicMap.DEFAULT_MAX_ZOOM_FACTOR;
  }
};
 ...
 */
ThematicMap.prototype.isMarkerZoomBehaviorFixed = function() {
 ...
 */
ThematicMap.prototype.addLayer = function(layer) {
 ...
 * @return {DvtMapLayer}
 */
ThematicMap.prototype.getLayer = function(layerName) {
 ...
 */
ThematicMap.prototype.getAllLayers = function() {
  return this._layers;
};
 ...
 */
ThematicMap.prototype.getAreaLayerContainer = function() {
 ...
 */
ThematicMap.prototype.getDataAreaContainer = function() {
 ...
 */
ThematicMap.prototype.getDataPointContainer = function() {
 ...
 */
ThematicMap.prototype.getLabelContainer = function() {
 ...
 */
ThematicMap.prototype.getLinkContainer = function() {
 ...
 */
ThematicMap.prototype.setMapName = function(attr) {
 ...
 */
ThematicMap.prototype.setAnimationDuration = function(attr) {
 ...
 */
ThematicMap.prototype.getAnimationDuration = function() {
 ...
 * @return {string}
 */
ThematicMap.prototype.getDisplayTooltips = function() {
 ...
 * @param {boolean} attr
 */
ThematicMap.prototype.setInitialZooming = function(attr) {
 ...
 * @override
 */
ThematicMap.prototype.PreRender = function() {
  ThematicMap.superclass.PreRender.call(this);
 ...
 * @private
 */
ThematicMap.prototype._createHandlers = function() {
 ...
 * @override
 */
ThematicMap.prototype.Render = function() {
  ThematicMap.superclass.Render.call(this);
  ...
  if (!this._bRendered && !this._oldPzc) { // Case 1
    ...
    if (animOnDisplay) {
      this.Animation = BlackBoxAnimationHandler.getInAnimation(this.getCtx(), animOnDisplay, this._pzc, bounds, this._animationDuration);
    }
  }
    ...
    if (animOnMapChange) {
      this.Animation = BlackBoxAnimationHandler.getCombinedAnimation(this.getCtx(), animOnMapChange, this._oldPzc, this._pzc, bounds, this._animationDuration);
 ...
 * @private
 */
ThematicMap.prototype._calcMinZoom = function() {
 ...
 * @private
 */
ThematicMap.prototype._render = function(pzcContainer, cpContainer) {
 ...
 * @private
 */
ThematicMap.prototype._setInitialKeyboardFocus = function() {
 ...
 * @param {boolean} isAreaDataLayer True if we are parsing an area data layer
 */
ThematicMap.prototype.updateLayer = function(dataLayerOptions, parentLayer, isAreaDataLayer) {
 ...
 * @private
 */
ThematicMap.prototype._renderCompleted = function() {
 ...
 * @private
 */
ThematicMap.prototype._updateZoomLimits = function() {
 ...
 * @protected
 */
ThematicMap.prototype.OnUpdateLayerEnd = function() {
 ...
 * @return {string}
 */
ThematicMap.prototype.getMapName = function() {
 ...
 * @private
 */
ThematicMap.prototype._transformContainers = function(pzcMatrix) {
 ...
 * @private
 */
ThematicMap.prototype._constrainPanning = function(zoom) {
  var padding = this._pzc.getZoomToFitPadding();
  var pzcDim = this._pzc.getSize();
  var viewportDim = new Rectangle(padding, padding, pzcDim.w - 2 * padding, pzcDim.h - 2 * padding);
  var mapDim = this._areaLayer.getLayerDim();
 ...
 * @private
 */
ThematicMap.prototype._updateAnimator = function(event, bRecenterObjs) {
  ...
  if (animator) {
    ...
    if (bRecenterObjs) {
      ...
      animator.addProp(Animator.TYPE_MATRIX, this, this._getCenteredObjsMatrix, this._setCenteredObjsMatrix, mat);
    }
    var transMat = new Matrix(1, 0, 0, 1, mat.getTx(), mat.getTy());
    if (this.isMarkerZoomBehaviorFixed())
      animator.addProp(Animator.TYPE_MATRIX, this._dataPointLayers, this._dataPointLayers.getMatrix, this._dataPointLayers.setMatrix, transMat);
    animator.addProp(Animator.TYPE_MATRIX, this._labelLayers, this._labelLayers.getMatrix, this._labelLayers.setMatrix, transMat);
    animator.addProp(Animator.TYPE_MATRIX, this._linkLayers, this._linkLayers.getMatrix, this._linkLayers.setMatrix, transMat);
  }
};
 ...
 * @protected
 */
ThematicMap.prototype.HandleZoomEvent = function(event) {
  ...
  switch (subtype) {
    ...
    case 'zoomed':
      ...
      if (zoom != null) {
        ...
        event.animator = null;
        this.dispatchEvent(EventFactory.newThematicMapViewportChangeEvent(pzcMatrix.getTx(), pzcMatrix.getTy(), zoom));
 ...
 * @override
 */
ThematicMap.prototype.HandlePanEvent = function(event) {
  var subtype = event.subtype;
  if (subtype == 'elasticAnimBegin' || subtype == 'panning') {
    ...
    if (event.newX != null) {
      ...
      // null out animator for Flash. Temp fix until  is done.
      event.animator = null;
      this.dispatchEvent(EventFactory.newThematicMapViewportChangeEvent(pzcMatrix.getTx(), pzcMatrix.getTy(), this._pzc.getZoom()));
 ...
 * @param {boolean} attr
 */
ThematicMap.prototype.setZooming = function(attr) {
  this._zooming = attr;
};
 ...
 * @param {boolean} attr
 */
ThematicMap.prototype.setPanning = function(attr) {
  this._panning = attr;
};
 ...
 * @private
 */
ThematicMap.prototype._zoomData = function() {
  ...
  if (!this._pzc.isZoomingEnabled()) {
    ...
    this._pzc.setMaxZoom(maxZoom * ThematicMap.DEFAULT_MAX_ZOOM_FACTOR);
  }

  var animation;
  if (!Agent.isEnvironmentTest())
    animation = new Animator(this.getCtx(), .3);
 ...
 * @private
 */
ThematicMap.prototype._zoomToFitBounds = function(bounds) {
  var animator = new Animator(this.getCtx(), .3);
 ...
 */
ThematicMap.prototype.fitRegion = function(toFit) {
  if (!toFit)
 ...
 */
ThematicMap.prototype.fitSelectedRegions = function() {
 ...
 */
ThematicMap.prototype.fitMap = function() {
  var animator = new Animator(this.getCtx(), .3);
 ...
 */
ThematicMap.prototype.getNavigableAreas = function() {
  var areas = [];
  if (this._areaLayer) {
 ...
 */
ThematicMap.prototype.getNavigableMarkers = function() {
  var markers = [];
  for (var i = 0; i < this._layers.length; i++) {
 ...
 */
ThematicMap.prototype.getNavigableLinks = function() {
  var navigable = [];
  for (var i = 0; i < this._layers.length; i++) {
    var dataLayers = this._layers[i].getDataLayers();
    for (var id in dataLayers)
 ...
 * @private
 */
ThematicMap.prototype._setCenteredObjsMatrix = function(matrix) {
 ...
 * @private
 */
ThematicMap.prototype._getCenteredObjsMatrix = function() {
 ...
 */
ThematicMap.prototype.setClickInfo = function(clientId, layerName, obj) {
 ...
 * @override
 */
ThematicMap.prototype.dispatchEvent = function(event) {
  var type = event['type'];
  if (type == 'selection') {
    this._handleSelectionEvent(event);
  }
  ThematicMap.superclass.dispatchEvent.call(this, event);
};
 ...
 * @private
 */
ThematicMap.prototype._handleSelectionEvent = function(event) {
  if (this._clickedDataLayerId) {
    ...
    if (this._clickedObject && !(this._clickedObject instanceof SimpleMarker || this._clickedObject instanceof ImageMarker))
 ...
 * @override
 */
ThematicMap.prototype.destroy = function() {
  var layers = this.getAllLayers();
  for (var i = 0; i < layers.length; i++)
    layers[i].destroy();

  // Always call superclass last for destroy
  ThematicMap.superclass.destroy.call(this);
};
 ...
 * @protected
 */
ThematicMap.prototype.OnAnimationEnd = function() {
  if (this._oldPzc) {
 ...
 */
ThematicMap.prototype.getAutomation = function() {
  if (!this.Automation)
 ...
 * @override
 */
ThematicMap.prototype.highlight = function(categories) {
  // Update the options
  this.Options['highlightedCategories'] = JsonUtils.clone(categories);

  // Perform the highlighting
  CategoryRolloverHandler.highlight(categories,
 ...
 * @param {boolean} hovered True to show hover effect
 */
ThematicMap.prototype.processDefaultHoverEffect = function(id, hovered) {
 ...
 */
ThematicMap.prototype.processDefaultSelectionEffect = function(id, selected) {
  var dataItem = this._getDataItemById(id);
  if (dataItem)
 ...
 * @param {boolean} focused True to show keyboard focus effect
 */
ThematicMap.prototype.processDefaultFocusEffect = function(id, focused) {
  var dataItem = this._getDataItemById(id);
  if (dataItem)
 ...
 */
ThematicMap.prototype.handleTouchStart = function() {
 ...
 */
ThematicMap.prototype.handleTouchEnd = function() {
 ...
 * @private
 */
ThematicMap.prototype._getDataItemById = function(id) {
  var ctx = this.getCtx();
  for (var i = 0; i < this._layers.length; i++) {
    var dataLayers = this._layers[i].getDataLayers();
    for (var dlId in dataLayers) {
      ...
      for (var j = 0; j < dataObjs.length; j++) {
        if (Obj.compareValues(ctx, dataObjs[j].getId(), id))
          return dataObjs[j];
      }
    }
  }
 ...
 * @private
 */
ThematicMap.prototype._getAreaFromDataLayer = function(areaName, dataLayer) {
 ...
 */
ThematicMap.prototype.storeTouchEventContent = function(content, marker) {
  ...
  if (content.namespaceURI === ToolkitUtils.SVG_NS) {
    ...
    content.forEach(function(node) {ToolkitUtils.appendChildElem(this._touchEventLayer.getElem(), node);}.bind(this));
  }
};
 ...
 */
ThematicMap.prototype._clearTouchEventContent = function() {
