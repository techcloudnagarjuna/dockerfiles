 ...
 * @ignore
 */
import { VComponent, flattenChildren as flattenChildren$1 } from 'ojs/ojvcomponent';
export { classPropToObject, dynamicDefault, h, listener } from 'ojs/ojvcomponent';
import { h, mountCustomElement, patchCustomElement, mountCustomElementContent, patchCustomElementContent, removeChildren, removeListeners, patchRef } from 'ojs/ojvdom';
import 'ojs/ojcustomelement';
import oj from 'ojs/ojcore-base';
import { DefaultsUtils } from 'ojs/ojdefaultsutils';
import Context from 'ojs/ojcontext';
import { ElementState, CustomElementUtils, AttributeUtils, ElementUtils } from 'ojs/ojcustomelement-utils';
...
class ElementVComponent extends VComponent {
}
const flattenChildren = flattenChildren$1;
...
class VComponentState extends ElementState {
    ...
    _getTemplateEnginePromise() {
        return import('ojs/ojtemplateengine').then((eng) => {
...
oj.CollectionUtils.copyInto(VirtualElementBridge.proto, {
  ...
  GetAttributes: function (metadata) {
    ...
    if (rootPropsMap) {
      Object.keys(rootPropsMap).forEach(function (prop) {
        attrs.push(AttributeUtils.getGlobalAttrForProp(prop));
      });
    }
    return attrs;
  },
  ...
  HandleAttributeChanged: function (element, attr, oldValue, newValue) {
    ...
    if (vcomp && rootPropsMap) {
      var prop = AttributeUtils.getGlobalPropForAttr(attr) || attr;
  ...
  _queueFireEventsTask: function (customEvent, element) {
    ...
    if (!this._eventsQueued) {
      ...
      const busyCallback = busyContext.addBusyState({
        description: CustomElementUtils.getElementInfo(element) + ' event queued.'
      });
  ...
  _getVComponentProps: function () {
    const staticDefaults = DefaultsUtils.getStaticDefaults(this._CONSTRUCTOR, this.METADATA, true);
  ...
  _initializeControlledProps: function (element) {
    ...
    if (rootPropsMap) {
      ...
      Object.keys(rootPropsMap).forEach(function (prop) {
        var attr = AttributeUtils.getGlobalAttrForProp(prop);
  ...
  _getDefaultValue: function (property) {
    ...
    return DefaultsUtils.getFrozenDefault(property, this._CONSTRUCTOR, this.METADATA);
  },
  ...
  _initSlotMapAndRootProps: function (element) {
    ...
    if (!this._slotMap) {
      ...
      this._uniqueId = ElementUtils.getUniqueId(element.id);

      this._slotMap = CustomElementUtils.getSlotMap(element);
...
function customElement(tagName) {
    return function (constructor) {
        ...
        constructor.prototype.mount = function (props, content, uncontrolledRootProps) {
            ...
            const mountNode = (this._ref = mountCustomElement(this._vnode, uncontrolledRootProps));
        ...
        constructor.prototype.patch = function (props, content, uncontrolledRootProps, oldUncontrolledRootProps) {
            ...
            this._patching = true;
            try {
                patchCustomElement(this._vnode, oldVnode, uncontrolledRootProps, oldUncontrolledRootProps);
            }
            finally {
        ...
        constructor.prototype.mountContent = function (props, content, rootElem, controlledRootProps) {
            ...
            this._patching = true;
            try {
                mountCustomElementContent(this._vnode, controlledRootProps);
            }
            finally {
                this._patching = false;
            }
        ...
        constructor.prototype.patchContent = function (props, controlledRootProps, content) {
            ...
            this._patching = true;
            try {
                patchCustomElementContent(this._vnode, oldVnode, controlledRootProps);
            }
            finally {
                this._patching = false;
            }
        ...
        constructor.prototype.notifyUnmounted = function () {
            ...
            removeChildren(this._ref, this._vnode.content);
            if (this.isCustomElementFirst()) {
                removeListeners(this._ref, this._vnode.props);
            }
            this.unmounted();
            patchRef(this._vnode.ref, null);
        };
        function _isValidRootProp(prop, metadata) {
            ...
            return (allowedProps[prop] ||
                ...
                AttributeUtils.isEventListenerProperty(prop));
        }
...
export { ElementVComponent, customElement, event, flattenChildren, method, readOnly, rootProperty };
