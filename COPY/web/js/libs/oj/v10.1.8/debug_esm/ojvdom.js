 ...
 * @ignore
 */
import { ElementUtils, AttributeUtils } from 'ojs/ojcustomelement-utils';
...
function h(type, props, ...args) {
    ...
    if (typeof type === 'string') {
        isSVG = type === 'svg';
        isCustomElement = !isSVG && ElementUtils.isValidCustomElementName(type);
    }
    else {
...
function maybePatchListener(el, key, value, oldValue) {
    if (value || oldValue) {
        if (AttributeUtils.isEventListenerProperty(key)) {
            patchListener(el, AttributeUtils.eventListenerPropertyToEventType(key), value, oldValue);
            return true;
        }
    }
    return false;
...
function maybePatchAttribute(el, key, value, isVComponent) {
    if (requiresDomPropertyPatching(el, key, isVComponent)) {
        const attr = AttributeUtils.getNativeAttr(key);
        if (key === 'draggable') {
            if (value == null) {
...
function requiresDomPropertyPatching(el, propName, isVComponent) {
    if (isVComponent) {
        return AttributeUtils.isGlobalOrData(propName);
    }
    ...
    if (c._text != null) {
        ...
        if (typeof type === 'string') {
            ...
            if (c.isComponent) {
                ...
                instance._uniqueId = ElementUtils.getUniqueId(props.id);
...
function sortControlled(constr, props, isCustomElement) {
    const staticDefaults = DefaultsUtils.getStaticDefaults(constr, constr.metadata, true);
    const splitProps = {
    ...
    for (let propName in props) {
        ...
        if (value !== undefined && isValidProp(constr, propName)) {
            if (!isCustomElement ||
                !AttributeUtils.isGlobalOrData(propName) ||
...
function patchCustomElementContent(newch, oldch, controlledRootProps) {
    ...
    for (let prop in oldProps) {
        if (AttributeUtils.isEventListenerProperty(prop) || prop === 'class' || prop === 'style') {
...
function isValidProp(constr, prop) {
    ...
    if ((_b = (_a = meta === null || meta === void 0 ? void 0 : meta.properties) === null || _a === void 0 ? void 0 : _a[prop]) === null || _b === void 0 ? void 0 : _b.readOnly) {
        warn(`Read-only property '${prop}' cannot be set on ${meta.name}.`);
        return false;
    }
    return true;
}
...
export { LISTENER_OPTIONS_SYMBOL, appendChildrenForTemplate, classPropToObject, flattenContent, getMountedNode, h, mount, mountCustomElement, mountCustomElementContent, mountForTemplate, mounted, patch, patchCustomElement, patchCustomElementContent, patchDOMForTemplate, patchRef, removeChildren, removeListeners, unmount };
