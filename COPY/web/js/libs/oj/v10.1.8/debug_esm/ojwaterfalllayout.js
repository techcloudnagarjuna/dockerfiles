...
let WaterfallLayout = WaterfallLayout_1 = class WaterfallLayout extends ElementVComponent {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        return (h("oj-waterfall-layout", { ref: this.setRootElement, style: this._getRootElementStyle() },
            h("div", { onClick: this._handleClick, onKeydown: this._handleKeyDown, onTouchstart: this._touchStartHandler, onFocusin: this._handleFocusIn, onFocusout: this._handleFocusOut, role: 'grid', "aria-label": this.props['aria-label'], "aria-labelledby": this.props['aria-labelledby'] },
    ...
    _getOptionDefaults() {
        if (this.defaultOptions == null) {
            this.defaultOptions = parseJSONFromFontFamily('oj-waterfalllayout-option-defaults');
        }
    ...
    _applyEntranceAnimation() {
        ...
        items.forEach((item, index) => {
            ...
            promises.push(fadeIn(item, { delay: delay, duration: duration }));
            promise.then(() => {
    ...
    _applyExitAnimation() {
        ...
        items.forEach((item) => {
            const duration = '300ms';
            promises.push(slideOut(item, { offsetY: '300px', duration: duration, persist: 'all' }));
            promises.push(fadeOut(item, { duration: duration, persist: 'all' }));
        });
        return Promise.all(promises);
    }
    ...
    _renderInitialSkeletons(positions) {
        ...
        if (positions == null) {
            ...
            return h("div", { role: 'row' }, skeletons);
        }
    }
...
__decorate([
    listener()
], WaterfallLayout.prototype, "_handleFocusIn", null);
__decorate([
    listener()
], WaterfallLayout.prototype, "_handleFocusOut", null);
__decorate([
    listener()
], WaterfallLayout.prototype, "_handleClick", null);
__decorate([
    listener()
], WaterfallLayout.prototype, "_handleKeyDown", null);
__decorate([
    listener({ passive: true })
], WaterfallLayout.prototype, "_touchStartHandler", null);
__decorate([
    listener()
], WaterfallLayout.prototype, "scrollListener", null);
